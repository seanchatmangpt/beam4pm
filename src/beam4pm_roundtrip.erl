%% GENERATED by ggen from the admitted bpm:RecordType graph. Do not edit.
%%
%% Cross-language roundtrip sample fixtures: one deterministic sample per
%% admitted record type and variant (full | minimal), written and verified
%% through beam4pm_codec so any drift between the Erlang and Elixir codecs
%% surfaces as a real fixture mismatch, not an assumed equivalence.

-module(beam4pm_roundtrip).

-export([record_names/0, sample/2, write_samples/1, verify_samples/2]).

-type variant() :: full | minimal.

-spec record_names() -> [atom()].
record_names() ->
    [
        acceptance_criteria_nonweakening,
        account_discovery,
        account_master_match,
        account_parent_scope,
        account_value_realization,
        action_eligibility_decision,
        action_pin_evidence,
        activation_event,
        add_on_bundle,
        addon_activation,
        admissible_action_set,
        adoption_milestone,
        agent_assignment,
        agent_capability_advertisement,
        alignment_move,
        annual_subscription,
        anomaly_detection_observation,
        anti_repeat_refusal,
        anti_repeat_signature,
        approval_separation_evidence,
        architecture_readiness,
        architecture_review_evidence,
        artifact_digest_evidence,
        artifact_digest_observation,
        astar_plan_candidate,
        attestation_verification_evidence,
        audit_chain_evidence,
        authority_ceiling,
        autonomic_actuation_receipt,
        autonomic_actuation_replay,
        autonomic_actuation_selection,
        autonomic_authority_admission,
        autonomic_authority_escalation,
        autonomic_backpressure_admission,
        autonomic_caller_local_binding,
        autonomic_canary_admission,
        autonomic_cancellation_receipt,
        autonomic_canonical_repair_route,
        autonomic_capability_token,
        autonomic_circuit_breaker_transition,
        autonomic_compensation_verification,
        autonomic_crash_recovery,
        autonomic_cross_consumer_receipt_refusal,
        autonomic_deterministic_receipt_replay,
        autonomic_failure_classification,
        autonomic_forged_receipt_refusal,
        autonomic_generated_surface_refusal,
        autonomic_idempotence_fence,
        autonomic_incident_recovery,
        autonomic_least_authority_grant,
        autonomic_model_authority_refusal,
        autonomic_mutable_pack_refusal,
        autonomic_output_ownership_check,
        autonomic_pack_sha_authority,
        autonomic_plan_construction,
        autonomic_planner_authority_refusal,
        autonomic_rca_hypothesis,
        autonomic_receipt_authority_binding,
        autonomic_receipt_chain_link,
        autonomic_receipt_completeness_check,
        autonomic_receipt_subject_binding,
        autonomic_repair_reexecution,
        autonomic_repair_selection,
        autonomic_replay_divergence_refusal,
        autonomic_retry_backoff,
        autonomic_retry_budget,
        autonomic_rollback_transition,
        autonomic_saga_compensation,
        autonomic_second_run_identity,
        autonomic_self_healing_completion_receipt,
        autonomic_stale_action_refusal,
        autonomic_stale_receipt_refusal,
        autonomic_state_vector,
        autonomic_subject_compare_and_swap,
        autonomic_supervisor_restart,
        autonomic_timeout_budget,
        autonomic_transition_execution,
        autonomic_transition_verification,
        autonomic_upgrade_transition,
        availability_observation,
        availability_slo_evidence,
        backup_restore_evidence,
        baseline_metric,
        beam_search_candidate,
        belief_state_snapshot,
        belief_state_update,
        belief_update_rule,
        beneficial_owner_evidence,
        billable_usage_identity,
        billing_account,
        billing_reconciliation,
        booking_readiness,
        bounded_work_selection_receipt,
        brce_actuation_receipt,
        brce_actuation_request,
        budget_period_alignment,
        bundle_conflict,
        bundle_dependency,
        burst_pricing_policy,
        business_continuity_evidence,
        business_outcome_measurement,
        business_unit_allocation,
        buying_committee,
        caller_local_checkout_observation,
        caller_local_consumer,
        caller_local_crown_identity,
        canary_decision,
        canary_evidence,
        cancellation_policy,
        canonical_source_authority_observation,
        capability_bundle,
        capability_gap,
        capability_gap_learning,
        capsule_availability,
        capsule_identity,
        case_object_binding,
        case_stats,
        catalog_release,
        causal_lineage_observation,
        challenger_candidate_evaluation,
        change_control_evidence,
        change_order_authority,
        changed_surface_inference,
        channel_agreement,
        chargeback_rule,
        cluster_quorum_state,
        co_term_policy,
        commercial_approval,
        commercial_artifact_crown_evidence,
        commercial_exception,
        commercial_execution_receipt,
        commercial_forecast,
        commercial_outcome,
        commercial_quote,
        commercial_quote_line,
        commercial_value_realization,
        commit_check_state_observation,
        committed_spend,
        committed_spend_admission,
        compatibility_contract,
        compound_task_expansion,
        concurrency_pricing_policy,
        configuration_export,
        configuration_import,
        conformance_result,
        consequential_state_invalidation,
        constraint_set_binding,
        consumer_equivalence_learning_guard,
        consumer_equivalence_proof,
        consumer_pack_pin_observation,
        consumption_pool,
        consumption_subscription,
        container_manifest_digest_observation,
        container_platform_digest_observation,
        contingency_branch,
        contracting_entity_identity,
        cost_center_allocation,
        cost_latency_reliability_tradeoff,
        cost_of_delay_score,
        cost_to_serve_measurement,
        counterfactual_frontier,
        counterfactual_replay,
        crash_recovery_receipt,
        credit_expiry_policy,
        credit_risk_admission,
        cross_sell_fit,
        crown_applicable_gate_coverage,
        crown_artifact_pullback_smoke,
        crown_attestation_signer,
        crown_autonomic_republish,
        crown_capsule_toolchain,
        crown_cas_promotion,
        crown_check_relevance,
        crown_child_publish_observation,
        crown_consumer_smoke,
        crown_convergence_proof,
        crown_cosign_certificate,
        crown_default_head_sensor,
        crown_dependency_edge,
        crown_execution_mode,
        crown_fanin_convergence,
        crown_fanout_batch,
        crown_federated_phase_receipt,
        crown_freshness_window,
        crown_generated_source_ownership,
        crown_gitlink_reconciliation,
        crown_immutable_sha_tag,
        crown_known_good_rollback,
        crown_latency_observation,
        crown_lock_reconciliation,
        crown_manufacturer_identity,
        crown_marketplace_pack_pin,
        crown_multiarch_platform_set,
        crown_oci_manifest_binding,
        crown_package_pin_reconciliation,
        crown_partial_checkpoint,
        crown_path_skip_refusal,
        crown_planner_identity,
        crown_process_runtime_identity,
        crown_promotion_race,
        crown_provenance_binding,
        crown_receipt_output_ownership,
        crown_recursive_fixed_point,
        crown_resume_token,
        crown_runtime_identity,
        crown_sbom_subject_binding,
        crown_second_pass_identity,
        crown_security_scan,
        crown_source_capsule,
        crown_stale_refusal,
        crown_supply_chain_policy,
        crown_topological_order,
        crown_validation_pack,
        crown_workflow_run_receipt,
        crown_zero_unreceipted_writes,
        currency_policy,
        curriculum_generation,
        customer_health,
        customer_managed_key_evidence,
        customer_signal_observation,
        data_egress_evidence,
        data_migration_scope_admission,
        data_processing_addendum_state,
        data_readiness,
        data_residency_policy,
        data_volume_pricing_policy,
        deal_desk_packet,
        decision_compression_observation,
        decision_information_preservation,
        deletion_proof_evidence,
        demo_run,
        demo_scenario,
        dependency_dag,
        dependency_inventory_evidence,
        dependency_pin_observation,
        deployment_entitlement,
        developer_readiness,
        deviation_repair_option,
        dfg_edge,
        disaster_recovery_evidence,
        discount_schedule,
        discovery_hypothesis,
        distributed_work_queue_observation,
        dominance_witness,
        dynamic_replan_trigger,
        edition_definition,
        edition_downgrade_path,
        edition_upgrade_path,
        enterprise_agreement,
        enterprise_order,
        enterprise_order_line,
        entitlement_denial_receipt,
        entitlement_event,
        entitlement_evidence,
        entitlement_grant,
        entitlement_revocation,
        entitlement_runtime_check,
        entitlement_state,
        entropy_reduction_score,
        environment_failure_separation,
        environment_identity,
        environment_pricing_policy,
        environment_profile,
        environment_signal_observation,
        error_budget_state,
        evaluation_seed_binding,
        event_log,
        event_triggered_planning,
        event_type,
        event_volume_pricing_policy,
        evidence_freshness_evidence,
        evidence_training_sample,
        exact_subject_binding,
        exact_world_state_admission,
        exception_authority,
        executive_business_review,
        executive_sponsor,
        expansion_opportunity,
        expansion_option,
        expansion_receipt,
        expansion_signal,
        experiment_learning_receipt,
        failed_challenger_retention,
        failure_label,
        fairness_assumption,
        federated_dogfood_learning_crown,
        forged_receipt_refusal,
        frontier_benchmark,
        frontier_evidence,
        frontier_opportunity,
        frontier_source_release,
        funding_approval_chain,
        fx_conversion_policy,
        generated_hypothesis,
        generated_output_ownership_observation,
        generated_source_route,
        goal_set_binding,
        hddl_method_candidate,
        hddl_task_network,
        heuristic_arc,
        historical_episode_replay,
        hypothesis_priority_update,
        immutable_pack_selection,
        implementation_fee_admission,
        incident_acknowledgement,
        incident_detection_event,
        incident_response_evidence,
        indemnity_scope_admission,
        information_partition_observation,
        initial_state_digest,
        insurance_requirement,
        integration_readiness,
        integration_scope_admission,
        invoice_entity_identity,
        invoice_line_item,
        invoice_schedule,
        irreversibility_budget,
        k8s_object_ref,
        late_arriving_usage,
        latency_budget_observation,
        leakage_finding,
        learning_episode,
        lease_expiry_replan,
        least_authority_evidence,
        legal_blocker,
        liability_cap_admission,
        license_evidence,
        log_trace,
        machine_actionable_delta,
        manufacture_receipt_presence_observation,
        manufacture_receipt_validity_observation,
        master_service_agreement_binding,
        master_service_agreement_state,
        mcts_plan_candidate,
        meta_router,
        meter_definition,
        meter_dimension,
        meter_rollup,
        metered_usage_sample,
        method_precondition_gate,
        migration_contract,
        migration_readiness,
        minimum_commitment_schedule,
        minimum_term_admission,
        multiarch_evidence,
        mutable_identity_refusal_evidence,
        mutual_information_score,
        negative_fixture_generation,
        next_event_prediction_contract,
        next_lawful_actuation,
        no_authority_learning_guard,
        node_failover_event,
        nondeterministic_effect_contract,
        nonproduction_discount_policy,
        normalized_event_observation,
        novelty_reward,
        novelty_score,
        object_attribute_change,
        object_type,
        object_volume_pricing_policy,
        objection,
        objection_resolution,
        observation_deduplication_decision,
        observation_entropy_estimate,
        observation_freshness_assessment,
        observation_partition,
        observation_projection_update,
        observation_staleness_invalidation,
        oc_declare_constraint,
        ocel_attribute,
        ocel_event,
        ocel_object,
        ocel_planning_event,
        ocel_relationship,
        oci_manifest_evidence,
        offline_bundle_evidence,
        operator_readiness,
        opportunity_currency_contract,
        opportunity_value_range,
        optimization_plan_candidate,
        option_generation,
        order_form_admission,
        order_form_version,
        orthogonality_reward,
        orthogonality_score,
        outcome_branch_set,
        outcome_label,
        output_ownership_gate,
        overage_invoice,
        overage_policy,
        package_release_observation,
        paid_workload_outcome_receipt,
        pareto_filter,
        path_schema,
        path_schema_query,
        payment_terms,
        payment_terms_admission,
        performance_slo_evidence,
        petri_arc,
        petri_place,
        petri_transition,
        plan_handoff_receipt,
        plan_lineage,
        plan_memory,
        planner_bid,
        planner_capability_profile,
        planner_identity,
        planner_lease,
        planner_payoff_observation,
        planner_policy_comparison,
        planner_portfolio,
        planner_routing_update,
        planning_action,
        planning_blackboard_claim,
        planning_blackboard_conflict,
        planning_blackboard_fact,
        planning_blackboard_resolution,
        planning_case_identity,
        planning_conformance_alignment,
        planning_problem_admission,
        planning_state,
        poc_exit_criteria,
        poc_risk,
        poc_scope,
        poc_timeline,
        policy_binding,
        policy_decision,
        policy_graph_edge,
        policy_graph_node,
        policy_payoff_observation,
        powl_choice_graph_edge,
        powl_choice_operator,
        powl_freq,
        powl_leaf,
        powl_loop_operator,
        powl_parallel_operator,
        powl_partial_order_edge,
        powl_partial_order_plan,
        powl_projection,
        powl_sequence_operator,
        ppddl_projection,
        prefix_alignment_frontier,
        premium_connector_pricing,
        prepaid_credit_balance,
        price_book_version,
        pricing_basis_contract,
        primitive_task_binding,
        privacy_classification_evidence,
        private_offer,
        private_registry_evidence,
        process_variant,
        process_volume_pricing_policy,
        procurement_acceptance_evidence,
        procurement_blocker,
        procurement_channel_selection,
        procurement_readiness,
        production_readiness,
        promotion_decision,
        promotion_threshold,
        proof_of_value_budget,
        proof_of_value_exit_gate,
        proof_of_value_package,
        propagation_score,
        proration_policy,
        provenance_binding_evidence,
        provenance_binding_observation,
        psro_population,
        purchase_order_binding,
        purchase_order_requirement,
        purchasing_entity_identity,
        queue_snapshot,
        quota_burst_allowance,
        quota_override,
        quota_policy,
        ramp_commitment,
        rate_card_entry,
        rate_distortion_budget,
        reachability_analysis,
        receipt_learning_compilation,
        receipt_replay_evidence,
        receipt_replay_request,
        receipt_required_gate,
        receipt_signature,
        receipt_subject_binding,
        receipt_verification,
        recovery_plan,
        recovery_point_receipt,
        recovery_subtask,
        recovery_time_receipt,
        refund_policy,
        refusal_boundary_observation,
        refusal_threshold,
        region_pricing_policy,
        regression_detector,
        regression_refusal,
        remaining_time_estimate_contract,
        remediation_sla_evidence,
        renewal_evidence,
        renewal_health,
        renewal_option,
        renewal_risk,
        renewal_term_admission,
        repair_effectiveness_measurement,
        replay_environment_identity,
        repository_ancestry_observation,
        repository_default_branch_observation,
        repository_exact_head_observation,
        repository_worktree_state_observation,
        reproducible_build_evidence,
        reseller_authorization,
        reserve_work_promotion,
        residency_evidence,
        resource_allocation,
        resource_capacity_plan,
        retention_policy_evidence,
        retention_pricing_policy,
        revenue_attribution,
        revenue_contract_admission,
        revenue_schedule_assumption,
        reversibility_weight,
        rework_cost,
        rfp_response_evidence,
        role_compatibility,
        rollback_checkpoint,
        rollback_decision,
        rollback_evidence,
        rollback_outcome_learning,
        rolling_upgrade_plan,
        root_cause_pattern,
        root_cause_reuse_decision,
        runtime_health_observation,
        runtime_policy_decision,
        sanctions_screening_result,
        sandbox_entitlement,
        saturation_detection,
        sbom_inventory_evidence,
        scheduling_priority_score,
        seat_pricing_policy,
        second_pass_byte_identity_observation,
        second_run_identity_objective,
        secret_boundary_evidence,
        security_addendum_state,
        security_blocker,
        security_readiness,
        seeded_evaluation,
        semantic_drift_observation,
        service_credit,
        service_credit_admission,
        service_credit_ledger,
        service_health_snapshot,
        service_level_objective,
        service_slo_contract,
        service_span,
        shadow_challenger_execution,
        showback_allocation,
        signature_evidence,
        sku_definition,
        sla_offer_admission,
        sojourn_time,
        solution_fit,
        span_edge,
        spend_drawdown,
        stakeholder_map,
        stale_plan_refusal,
        stale_receipt_refusal,
        stale_subject_refusal_evidence,
        standing_state_observation,
        stopping_criterion,
        strong_cyclic_plan_candidate,
        strong_plan_candidate,
        subject_failure_separation,
        submodule_lock_observation,
        submodule_registration_observation,
        success_plan,
        supervisor_restart_policy,
        support_contract,
        support_diagnostic_bundle,
        support_escalation_evidence,
        support_readiness,
        support_sla_evidence,
        support_tier_admission,
        support_window_evidence,
        sync_time,
        target_metric,
        task_decomposition_proof,
        tax_jurisdiction_evidence,
        tax_jurisdiction_rule,
        technical_blocker,
        temporal_order_observation,
        tenant_account,
        tenant_data_partition,
        tenant_key_scope,
        tenant_project,
        tenant_resource_quota,
        tenant_runtime_boundary,
        term_subscription,
        termination_right_admission,
        time_to_value,
        token_replay_state,
        toolchain_identity,
        toolchain_identity_observation,
        training_readiness,
        training_scope_admission,
        trajectory_window,
        trial_entitlement,
        true_up_policy,
        type_edge,
        uncertainty_aware_selection,
        uncertainty_observation,
        unit_economics_snapshot,
        unsupported_capability_evidence,
        upgrade_evidence,
        upsell_readiness,
        usage_aggregation_window,
        usage_correction,
        usage_event,
        usage_plan,
        usage_reconciliation_receipt,
        usage_signal,
        validation_capsule_drift_observation,
        validation_capsule_identity_observation,
        value_baseline,
        value_driver,
        value_of_information_estimate,
        value_of_information_score,
        value_realization,
        value_realization_feedback,
        value_receipt,
        value_telemetry_sample,
        vendor_registration_state,
        vendor_risk_evidence,
        verification_depth_update,
        version_lifecycle_evidence,
        volume_tier_admission,
        vulnerability_scan_evidence,
        weak_plan_candidate,
        wip_limit_gate,
        workflow_definition_digest_observation,
        workflow_job_state_observation,
        workflow_run_state_observation,
        workload_backpressure_signal,
        workload_cancellation_receipt,
        workload_execution_identity,
        workload_idempotency_key,
        workload_queue_depth,
        workload_retry_policy,
        workload_timeout_budget
    ].

%% Deterministic sample values, keyed by bpm:fieldType -- each is the pack
%% vocabulary's bpm:sampleErlang literal for that bpm:FieldType:
%% string -> <<"sample_" field_name>>, datetime -> an ISO 8601 UTC binary
%% carrying six microsecond digits (the same fixture the Ash-leg tests use,
%% so a sub-second-truncating projection is caught), integer -> 42,
%% float -> 3.5, boolean -> true, atom -> sample_atom,
%% list_string -> [<<"alpha">>, <<"beta">>], map -> #{<<"k">> => <<"v">>}.
%% minimal = required fields only; full = every field.
-spec sample(atom(), variant()) -> {ok, tuple()}.
sample(acceptance_criteria_nonweakening, full) ->
    beam4pm_types:new_acceptance_criteria_nonweakening(#{
        assessment_id => <<"sample_assessment_id">>,
        acceptance_contract_id => <<"sample_acceptance_contract_id">>,
        prior_digest => <<"sample_prior_digest">>,
        candidate_digest => <<"sample_candidate_digest">>,
        strength_result => <<"sample_strength_result">>,
        refusal_code => <<"sample_refusal_code">>
    });
sample(acceptance_criteria_nonweakening, minimal) ->
    beam4pm_types:new_acceptance_criteria_nonweakening(#{
        assessment_id => <<"sample_assessment_id">>,
        acceptance_contract_id => <<"sample_acceptance_contract_id">>,
        prior_digest => <<"sample_prior_digest">>,
        candidate_digest => <<"sample_candidate_digest">>,
        strength_result => <<"sample_strength_result">>,
        refusal_code => <<"sample_refusal_code">>
    });
sample(account_discovery, full) ->
    beam4pm_types:new_account_discovery(#{
        account_discovery_id => <<"sample_account_discovery_id">>,
        account_id => <<"sample_account_id">>,
        discovery_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(account_discovery, minimal) ->
    beam4pm_types:new_account_discovery(#{
        account_discovery_id => <<"sample_account_discovery_id">>,
        account_id => <<"sample_account_id">>,
        discovery_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(account_master_match, full) ->
    beam4pm_types:new_account_master_match(#{
        source_account_id => <<"sample_source_account_id">>,
        canonical_account_id => <<"sample_canonical_account_id">>,
        match_evidence_hash => <<"sample_match_evidence_hash">>
    });
sample(account_master_match, minimal) ->
    beam4pm_types:new_account_master_match(#{
        source_account_id => <<"sample_source_account_id">>,
        canonical_account_id => <<"sample_canonical_account_id">>,
        match_evidence_hash => <<"sample_match_evidence_hash">>
    });
sample(account_parent_scope, full) ->
    beam4pm_types:new_account_parent_scope(#{
        account_id => <<"sample_account_id">>,
        parent_account_id => <<"sample_parent_account_id">>,
        scope_evidence_hash => <<"sample_scope_evidence_hash">>
    });
sample(account_parent_scope, minimal) ->
    beam4pm_types:new_account_parent_scope(#{
        account_id => <<"sample_account_id">>,
        parent_account_id => <<"sample_parent_account_id">>,
        scope_evidence_hash => <<"sample_scope_evidence_hash">>
    });
sample(account_value_realization, full) ->
    beam4pm_types:new_account_value_realization(#{
        tenant_id => <<"sample_tenant_id">>,
        account_id => <<"sample_account_id">>,
        realization_hash => <<"sample_realization_hash">>
    });
sample(account_value_realization, minimal) ->
    beam4pm_types:new_account_value_realization(#{
        tenant_id => <<"sample_tenant_id">>,
        account_id => <<"sample_account_id">>,
        realization_hash => <<"sample_realization_hash">>
    });
sample(action_eligibility_decision, full) ->
    beam4pm_types:new_action_eligibility_decision(#{
        decision_id => <<"sample_decision_id">>,
        action_id => <<"sample_action_id">>,
        standing => <<"sample_standing">>
    });
sample(action_eligibility_decision, minimal) ->
    beam4pm_types:new_action_eligibility_decision(#{
        decision_id => <<"sample_decision_id">>,
        action_id => <<"sample_action_id">>,
        standing => <<"sample_standing">>
    });
sample(action_pin_evidence, full) ->
    beam4pm_types:new_action_pin_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        action_sha => <<"sample_action_sha">>,
        observed_result => sample_atom
    });
sample(action_pin_evidence, minimal) ->
    beam4pm_types:new_action_pin_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        action_sha => <<"sample_action_sha">>,
        observed_result => sample_atom
    });
sample(activation_event, full) ->
    beam4pm_types:new_activation_event(#{
        activation_event_id => <<"sample_activation_event_id">>,
        account_id => <<"sample_account_id">>,
        activation_type => <<"sample_activation_type">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(activation_event, minimal) ->
    beam4pm_types:new_activation_event(#{
        activation_event_id => <<"sample_activation_event_id">>,
        account_id => <<"sample_account_id">>,
        activation_type => <<"sample_activation_type">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(add_on_bundle, full) ->
    beam4pm_types:new_add_on_bundle(#{
        add_on_id => <<"sample_add_on_id">>,
        name => <<"sample_name">>,
        capability_ids => [<<"alpha">>, <<"beta">>],
        status => sample_atom
    });
sample(add_on_bundle, minimal) ->
    beam4pm_types:new_add_on_bundle(#{
        add_on_id => <<"sample_add_on_id">>,
        name => <<"sample_name">>,
        capability_ids => [<<"alpha">>, <<"beta">>],
        status => sample_atom
    });
sample(addon_activation, full) ->
    beam4pm_types:new_addon_activation(#{
        addon_activation_id => <<"sample_addon_activation_id">>,
        account_id => <<"sample_account_id">>,
        addon_id => <<"sample_addon_id">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(addon_activation, minimal) ->
    beam4pm_types:new_addon_activation(#{
        addon_activation_id => <<"sample_addon_activation_id">>,
        account_id => <<"sample_account_id">>,
        addon_id => <<"sample_addon_id">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(admissible_action_set, full) ->
    beam4pm_types:new_admissible_action_set(#{
        state_id => <<"sample_state_id">>,
        constraint_hash => <<"sample_constraint_hash">>,
        action_set_hash => <<"sample_action_set_hash">>
    });
sample(admissible_action_set, minimal) ->
    beam4pm_types:new_admissible_action_set(#{
        state_id => <<"sample_state_id">>,
        constraint_hash => <<"sample_constraint_hash">>,
        action_set_hash => <<"sample_action_set_hash">>
    });
sample(adoption_milestone, full) ->
    beam4pm_types:new_adoption_milestone(#{
        adoption_milestone_id => <<"sample_adoption_milestone_id">>,
        account_id => <<"sample_account_id">>,
        milestone_name => <<"sample_milestone_name">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(adoption_milestone, minimal) ->
    beam4pm_types:new_adoption_milestone(#{
        adoption_milestone_id => <<"sample_adoption_milestone_id">>,
        account_id => <<"sample_account_id">>,
        milestone_name => <<"sample_milestone_name">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(agent_assignment, full) ->
    beam4pm_types:new_agent_assignment(#{
        agent_id => <<"sample_agent_id">>,
        policy_id => <<"sample_policy_id">>,
        assignment_hash => <<"sample_assignment_hash">>
    });
sample(agent_assignment, minimal) ->
    beam4pm_types:new_agent_assignment(#{
        agent_id => <<"sample_agent_id">>,
        policy_id => <<"sample_policy_id">>,
        assignment_hash => <<"sample_assignment_hash">>
    });
sample(agent_capability_advertisement, full) ->
    beam4pm_types:new_agent_capability_advertisement(#{
        agent_subject_sha => <<"sample_agent_subject_sha">>,
        capability_digest => <<"sample_capability_digest">>,
        valid_until => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(agent_capability_advertisement, minimal) ->
    beam4pm_types:new_agent_capability_advertisement(#{
        agent_subject_sha => <<"sample_agent_subject_sha">>,
        capability_digest => <<"sample_capability_digest">>,
        valid_until => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(alignment_move, full) ->
    beam4pm_types:new_alignment_move(#{
        move_type => sample_atom,
        cost => 42
    });
sample(alignment_move, minimal) ->
    beam4pm_types:new_alignment_move(#{
        move_type => sample_atom,
        cost => 42
    });
sample(annual_subscription, full) ->
    beam4pm_types:new_annual_subscription(#{
        subscription_id => <<"sample_subscription_id">>,
        sku => <<"sample_sku">>,
        seat_count => 42,
        renews_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(annual_subscription, minimal) ->
    beam4pm_types:new_annual_subscription(#{
        subscription_id => <<"sample_subscription_id">>,
        sku => <<"sample_sku">>,
        seat_count => 42,
        renews_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(anomaly_detection_observation, full) ->
    beam4pm_types:new_anomaly_detection_observation(#{
        subject_id => <<"sample_subject_id">>,
        baseline_digest => <<"sample_baseline_digest">>,
        observation_digest => <<"sample_observation_digest">>,
        anomaly_score => 3.5
    });
sample(anomaly_detection_observation, minimal) ->
    beam4pm_types:new_anomaly_detection_observation(#{
        subject_id => <<"sample_subject_id">>,
        baseline_digest => <<"sample_baseline_digest">>,
        observation_digest => <<"sample_observation_digest">>,
        anomaly_score => 3.5
    });
sample(anti_repeat_refusal, full) ->
    beam4pm_types:new_anti_repeat_refusal(#{
        refusal_id => <<"sample_refusal_id">>,
        candidate_action_id => <<"sample_candidate_action_id">>,
        matching_signature_id => <<"sample_matching_signature_id">>,
        recurrence_risk => 3.5,
        alternative_required => true
    });
sample(anti_repeat_refusal, minimal) ->
    beam4pm_types:new_anti_repeat_refusal(#{
        refusal_id => <<"sample_refusal_id">>,
        candidate_action_id => <<"sample_candidate_action_id">>,
        matching_signature_id => <<"sample_matching_signature_id">>,
        recurrence_risk => 3.5,
        alternative_required => true
    });
sample(anti_repeat_signature, full) ->
    beam4pm_types:new_anti_repeat_signature(#{
        signature_id => <<"sample_signature_id">>,
        failure_class => <<"sample_failure_class">>,
        causal_features_digest => <<"sample_causal_features_digest">>,
        repair_family => <<"sample_repair_family">>,
        first_seen_episode => <<"sample_first_seen_episode">>
    });
sample(anti_repeat_signature, minimal) ->
    beam4pm_types:new_anti_repeat_signature(#{
        signature_id => <<"sample_signature_id">>,
        failure_class => <<"sample_failure_class">>,
        causal_features_digest => <<"sample_causal_features_digest">>,
        repair_family => <<"sample_repair_family">>,
        first_seen_episode => <<"sample_first_seen_episode">>
    });
sample(approval_separation_evidence, full) ->
    beam4pm_types:new_approval_separation_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        approver_identity => <<"sample_approver_identity">>,
        observed_result => sample_atom
    });
sample(approval_separation_evidence, minimal) ->
    beam4pm_types:new_approval_separation_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        approver_identity => <<"sample_approver_identity">>,
        observed_result => sample_atom
    });
sample(architecture_readiness, full) ->
    beam4pm_types:new_architecture_readiness(#{
        architecture_readiness_id => <<"sample_architecture_readiness_id">>,
        account_id => <<"sample_account_id">>,
        architecture_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(architecture_readiness, minimal) ->
    beam4pm_types:new_architecture_readiness(#{
        architecture_readiness_id => <<"sample_architecture_readiness_id">>,
        account_id => <<"sample_account_id">>,
        architecture_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(architecture_review_evidence, full) ->
    beam4pm_types:new_architecture_review_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        review_decision => <<"sample_review_decision">>,
        observed_result => sample_atom
    });
sample(architecture_review_evidence, minimal) ->
    beam4pm_types:new_architecture_review_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        review_decision => <<"sample_review_decision">>,
        observed_result => sample_atom
    });
sample(artifact_digest_evidence, full) ->
    beam4pm_types:new_artifact_digest_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        digest => <<"sample_digest">>,
        observed_result => sample_atom
    });
sample(artifact_digest_evidence, minimal) ->
    beam4pm_types:new_artifact_digest_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        digest => <<"sample_digest">>,
        observed_result => sample_atom
    });
sample(artifact_digest_observation, full) ->
    beam4pm_types:new_artifact_digest_observation(#{
        artifact_id => <<"sample_artifact_id">>,
        artifact_sha256 => <<"sample_artifact_sha256">>,
        producer_run_id => <<"sample_producer_run_id">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(artifact_digest_observation, minimal) ->
    beam4pm_types:new_artifact_digest_observation(#{
        artifact_id => <<"sample_artifact_id">>,
        artifact_sha256 => <<"sample_artifact_sha256">>,
        producer_run_id => <<"sample_producer_run_id">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(astar_plan_candidate, full) ->
    beam4pm_types:new_astar_plan_candidate(#{
        plan_id => <<"sample_plan_id">>,
        heuristic_id => <<"sample_heuristic_id">>,
        path_hash => <<"sample_path_hash">>
    });
sample(astar_plan_candidate, minimal) ->
    beam4pm_types:new_astar_plan_candidate(#{
        plan_id => <<"sample_plan_id">>,
        heuristic_id => <<"sample_heuristic_id">>,
        path_hash => <<"sample_path_hash">>
    });
sample(attestation_verification_evidence, full) ->
    beam4pm_types:new_attestation_verification_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        predicate_type => <<"sample_predicate_type">>,
        observed_result => sample_atom
    });
sample(attestation_verification_evidence, minimal) ->
    beam4pm_types:new_attestation_verification_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        predicate_type => <<"sample_predicate_type">>,
        observed_result => sample_atom
    });
sample(audit_chain_evidence, full) ->
    beam4pm_types:new_audit_chain_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        previous_receipt_hash => <<"sample_previous_receipt_hash">>,
        observed_result => sample_atom
    });
sample(audit_chain_evidence, minimal) ->
    beam4pm_types:new_audit_chain_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        previous_receipt_hash => <<"sample_previous_receipt_hash">>,
        observed_result => sample_atom
    });
sample(authority_ceiling, full) ->
    beam4pm_types:new_authority_ceiling(#{
        action_id => <<"sample_action_id">>,
        grant_id => <<"sample_grant_id">>,
        ceiling => <<"sample_ceiling">>
    });
sample(authority_ceiling, minimal) ->
    beam4pm_types:new_authority_ceiling(#{
        action_id => <<"sample_action_id">>,
        grant_id => <<"sample_grant_id">>,
        ceiling => <<"sample_ceiling">>
    });
sample(autonomic_actuation_receipt, full) ->
    beam4pm_types:new_autonomic_actuation_receipt(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_actuation_receipt, minimal) ->
    beam4pm_types:new_autonomic_actuation_receipt(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_actuation_replay, full) ->
    beam4pm_types:new_autonomic_actuation_replay(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_actuation_replay, minimal) ->
    beam4pm_types:new_autonomic_actuation_replay(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_actuation_selection, full) ->
    beam4pm_types:new_autonomic_actuation_selection(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_actuation_selection, minimal) ->
    beam4pm_types:new_autonomic_actuation_selection(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_authority_admission, full) ->
    beam4pm_types:new_autonomic_authority_admission(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_authority_admission, minimal) ->
    beam4pm_types:new_autonomic_authority_admission(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_authority_escalation, full) ->
    beam4pm_types:new_autonomic_authority_escalation(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_authority_escalation, minimal) ->
    beam4pm_types:new_autonomic_authority_escalation(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_backpressure_admission, full) ->
    beam4pm_types:new_autonomic_backpressure_admission(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_backpressure_admission, minimal) ->
    beam4pm_types:new_autonomic_backpressure_admission(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_caller_local_binding, full) ->
    beam4pm_types:new_autonomic_caller_local_binding(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_caller_local_binding, minimal) ->
    beam4pm_types:new_autonomic_caller_local_binding(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_canary_admission, full) ->
    beam4pm_types:new_autonomic_canary_admission(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_canary_admission, minimal) ->
    beam4pm_types:new_autonomic_canary_admission(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_cancellation_receipt, full) ->
    beam4pm_types:new_autonomic_cancellation_receipt(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_cancellation_receipt, minimal) ->
    beam4pm_types:new_autonomic_cancellation_receipt(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_canonical_repair_route, full) ->
    beam4pm_types:new_autonomic_canonical_repair_route(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_canonical_repair_route, minimal) ->
    beam4pm_types:new_autonomic_canonical_repair_route(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_capability_token, full) ->
    beam4pm_types:new_autonomic_capability_token(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_capability_token, minimal) ->
    beam4pm_types:new_autonomic_capability_token(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_circuit_breaker_transition, full) ->
    beam4pm_types:new_autonomic_circuit_breaker_transition(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_circuit_breaker_transition, minimal) ->
    beam4pm_types:new_autonomic_circuit_breaker_transition(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_compensation_verification, full) ->
    beam4pm_types:new_autonomic_compensation_verification(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_compensation_verification, minimal) ->
    beam4pm_types:new_autonomic_compensation_verification(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_crash_recovery, full) ->
    beam4pm_types:new_autonomic_crash_recovery(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_crash_recovery, minimal) ->
    beam4pm_types:new_autonomic_crash_recovery(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_cross_consumer_receipt_refusal, full) ->
    beam4pm_types:new_autonomic_cross_consumer_receipt_refusal(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_cross_consumer_receipt_refusal, minimal) ->
    beam4pm_types:new_autonomic_cross_consumer_receipt_refusal(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_deterministic_receipt_replay, full) ->
    beam4pm_types:new_autonomic_deterministic_receipt_replay(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_deterministic_receipt_replay, minimal) ->
    beam4pm_types:new_autonomic_deterministic_receipt_replay(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_failure_classification, full) ->
    beam4pm_types:new_autonomic_failure_classification(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_failure_classification, minimal) ->
    beam4pm_types:new_autonomic_failure_classification(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_forged_receipt_refusal, full) ->
    beam4pm_types:new_autonomic_forged_receipt_refusal(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_forged_receipt_refusal, minimal) ->
    beam4pm_types:new_autonomic_forged_receipt_refusal(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_generated_surface_refusal, full) ->
    beam4pm_types:new_autonomic_generated_surface_refusal(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_generated_surface_refusal, minimal) ->
    beam4pm_types:new_autonomic_generated_surface_refusal(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_idempotence_fence, full) ->
    beam4pm_types:new_autonomic_idempotence_fence(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_idempotence_fence, minimal) ->
    beam4pm_types:new_autonomic_idempotence_fence(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_incident_recovery, full) ->
    beam4pm_types:new_autonomic_incident_recovery(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_incident_recovery, minimal) ->
    beam4pm_types:new_autonomic_incident_recovery(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_least_authority_grant, full) ->
    beam4pm_types:new_autonomic_least_authority_grant(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_least_authority_grant, minimal) ->
    beam4pm_types:new_autonomic_least_authority_grant(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_model_authority_refusal, full) ->
    beam4pm_types:new_autonomic_model_authority_refusal(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_model_authority_refusal, minimal) ->
    beam4pm_types:new_autonomic_model_authority_refusal(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_mutable_pack_refusal, full) ->
    beam4pm_types:new_autonomic_mutable_pack_refusal(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_mutable_pack_refusal, minimal) ->
    beam4pm_types:new_autonomic_mutable_pack_refusal(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_output_ownership_check, full) ->
    beam4pm_types:new_autonomic_output_ownership_check(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_output_ownership_check, minimal) ->
    beam4pm_types:new_autonomic_output_ownership_check(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_pack_sha_authority, full) ->
    beam4pm_types:new_autonomic_pack_sha_authority(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_pack_sha_authority, minimal) ->
    beam4pm_types:new_autonomic_pack_sha_authority(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_plan_construction, full) ->
    beam4pm_types:new_autonomic_plan_construction(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_plan_construction, minimal) ->
    beam4pm_types:new_autonomic_plan_construction(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_planner_authority_refusal, full) ->
    beam4pm_types:new_autonomic_planner_authority_refusal(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_planner_authority_refusal, minimal) ->
    beam4pm_types:new_autonomic_planner_authority_refusal(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_rca_hypothesis, full) ->
    beam4pm_types:new_autonomic_rca_hypothesis(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_rca_hypothesis, minimal) ->
    beam4pm_types:new_autonomic_rca_hypothesis(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_receipt_authority_binding, full) ->
    beam4pm_types:new_autonomic_receipt_authority_binding(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_receipt_authority_binding, minimal) ->
    beam4pm_types:new_autonomic_receipt_authority_binding(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_receipt_chain_link, full) ->
    beam4pm_types:new_autonomic_receipt_chain_link(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_receipt_chain_link, minimal) ->
    beam4pm_types:new_autonomic_receipt_chain_link(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_receipt_completeness_check, full) ->
    beam4pm_types:new_autonomic_receipt_completeness_check(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_receipt_completeness_check, minimal) ->
    beam4pm_types:new_autonomic_receipt_completeness_check(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_receipt_subject_binding, full) ->
    beam4pm_types:new_autonomic_receipt_subject_binding(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_receipt_subject_binding, minimal) ->
    beam4pm_types:new_autonomic_receipt_subject_binding(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_repair_reexecution, full) ->
    beam4pm_types:new_autonomic_repair_reexecution(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_repair_reexecution, minimal) ->
    beam4pm_types:new_autonomic_repair_reexecution(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_repair_selection, full) ->
    beam4pm_types:new_autonomic_repair_selection(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_repair_selection, minimal) ->
    beam4pm_types:new_autonomic_repair_selection(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_replay_divergence_refusal, full) ->
    beam4pm_types:new_autonomic_replay_divergence_refusal(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_replay_divergence_refusal, minimal) ->
    beam4pm_types:new_autonomic_replay_divergence_refusal(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_retry_backoff, full) ->
    beam4pm_types:new_autonomic_retry_backoff(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_retry_backoff, minimal) ->
    beam4pm_types:new_autonomic_retry_backoff(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_retry_budget, full) ->
    beam4pm_types:new_autonomic_retry_budget(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_retry_budget, minimal) ->
    beam4pm_types:new_autonomic_retry_budget(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_rollback_transition, full) ->
    beam4pm_types:new_autonomic_rollback_transition(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_rollback_transition, minimal) ->
    beam4pm_types:new_autonomic_rollback_transition(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_saga_compensation, full) ->
    beam4pm_types:new_autonomic_saga_compensation(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_saga_compensation, minimal) ->
    beam4pm_types:new_autonomic_saga_compensation(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_second_run_identity, full) ->
    beam4pm_types:new_autonomic_second_run_identity(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_second_run_identity, minimal) ->
    beam4pm_types:new_autonomic_second_run_identity(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_self_healing_completion_receipt, full) ->
    beam4pm_types:new_autonomic_self_healing_completion_receipt(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_self_healing_completion_receipt, minimal) ->
    beam4pm_types:new_autonomic_self_healing_completion_receipt(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_stale_action_refusal, full) ->
    beam4pm_types:new_autonomic_stale_action_refusal(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_stale_action_refusal, minimal) ->
    beam4pm_types:new_autonomic_stale_action_refusal(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_stale_receipt_refusal, full) ->
    beam4pm_types:new_autonomic_stale_receipt_refusal(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_stale_receipt_refusal, minimal) ->
    beam4pm_types:new_autonomic_stale_receipt_refusal(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_state_vector, full) ->
    beam4pm_types:new_autonomic_state_vector(#{
        state_vector_id => <<"sample_state_vector_id">>,
        subject_id => <<"sample_subject_id">>,
        dimension_digest => <<"sample_dimension_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(autonomic_state_vector, minimal) ->
    beam4pm_types:new_autonomic_state_vector(#{
        state_vector_id => <<"sample_state_vector_id">>,
        subject_id => <<"sample_subject_id">>,
        dimension_digest => <<"sample_dimension_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(autonomic_subject_compare_and_swap, full) ->
    beam4pm_types:new_autonomic_subject_compare_and_swap(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_subject_compare_and_swap, minimal) ->
    beam4pm_types:new_autonomic_subject_compare_and_swap(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_supervisor_restart, full) ->
    beam4pm_types:new_autonomic_supervisor_restart(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_supervisor_restart, minimal) ->
    beam4pm_types:new_autonomic_supervisor_restart(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_timeout_budget, full) ->
    beam4pm_types:new_autonomic_timeout_budget(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_timeout_budget, minimal) ->
    beam4pm_types:new_autonomic_timeout_budget(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_transition_execution, full) ->
    beam4pm_types:new_autonomic_transition_execution(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_transition_execution, minimal) ->
    beam4pm_types:new_autonomic_transition_execution(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_transition_verification, full) ->
    beam4pm_types:new_autonomic_transition_verification(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_transition_verification, minimal) ->
    beam4pm_types:new_autonomic_transition_verification(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_upgrade_transition, full) ->
    beam4pm_types:new_autonomic_upgrade_transition(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(autonomic_upgrade_transition, minimal) ->
    beam4pm_types:new_autonomic_upgrade_transition(#{
        actuation_id => <<"sample_actuation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        authority_receipt_sha => <<"sample_authority_receipt_sha">>,
        state_digest => <<"sample_state_digest">>
    });
sample(availability_observation, full) ->
    beam4pm_types:new_availability_observation(#{
        tenant_id => <<"sample_tenant_id">>,
        slo_id => <<"sample_slo_id">>,
        observation_hash => <<"sample_observation_hash">>
    });
sample(availability_observation, minimal) ->
    beam4pm_types:new_availability_observation(#{
        tenant_id => <<"sample_tenant_id">>,
        slo_id => <<"sample_slo_id">>,
        observation_hash => <<"sample_observation_hash">>
    });
sample(availability_slo_evidence, full) ->
    beam4pm_types:new_availability_slo_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        availability_percent => 3.5,
        observed_result => sample_atom
    });
sample(availability_slo_evidence, minimal) ->
    beam4pm_types:new_availability_slo_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        availability_percent => 3.5,
        observed_result => sample_atom
    });
sample(backup_restore_evidence, full) ->
    beam4pm_types:new_backup_restore_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        backup_digest => <<"sample_backup_digest">>,
        observed_result => sample_atom
    });
sample(backup_restore_evidence, minimal) ->
    beam4pm_types:new_backup_restore_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        backup_digest => <<"sample_backup_digest">>,
        observed_result => sample_atom
    });
sample(baseline_metric, full) ->
    beam4pm_types:new_baseline_metric(#{
        baseline_metric_id => <<"sample_baseline_metric_id">>,
        account_id => <<"sample_account_id">>,
        baseline_value => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(baseline_metric, minimal) ->
    beam4pm_types:new_baseline_metric(#{
        baseline_metric_id => <<"sample_baseline_metric_id">>,
        account_id => <<"sample_account_id">>,
        baseline_value => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(beam_search_candidate, full) ->
    beam4pm_types:new_beam_search_candidate(#{
        plan_id => <<"sample_plan_id">>,
        beam_width => <<"sample_beam_width">>,
        frontier_hash => <<"sample_frontier_hash">>
    });
sample(beam_search_candidate, minimal) ->
    beam4pm_types:new_beam_search_candidate(#{
        plan_id => <<"sample_plan_id">>,
        beam_width => <<"sample_beam_width">>,
        frontier_hash => <<"sample_frontier_hash">>
    });
sample(belief_state_snapshot, full) ->
    beam4pm_types:new_belief_state_snapshot(#{
        belief_state_id => <<"sample_belief_state_id">>,
        subject_id => <<"sample_subject_id">>,
        posterior_digest => <<"sample_posterior_digest">>,
        uncertainty_status => <<"sample_uncertainty_status">>
    });
sample(belief_state_snapshot, minimal) ->
    beam4pm_types:new_belief_state_snapshot(#{
        belief_state_id => <<"sample_belief_state_id">>,
        subject_id => <<"sample_subject_id">>,
        posterior_digest => <<"sample_posterior_digest">>,
        uncertainty_status => <<"sample_uncertainty_status">>
    });
sample(belief_state_update, full) ->
    beam4pm_types:new_belief_state_update(#{
        prior_belief_id => <<"sample_prior_belief_id">>,
        evidence_digest => <<"sample_evidence_digest">>,
        posterior_belief_id => <<"sample_posterior_belief_id">>,
        update_rule => <<"sample_update_rule">>
    });
sample(belief_state_update, minimal) ->
    beam4pm_types:new_belief_state_update(#{
        prior_belief_id => <<"sample_prior_belief_id">>,
        evidence_digest => <<"sample_evidence_digest">>,
        posterior_belief_id => <<"sample_posterior_belief_id">>,
        update_rule => <<"sample_update_rule">>
    });
sample(belief_update_rule, full) ->
    beam4pm_types:new_belief_update_rule(#{
        rule_id => <<"sample_rule_id">>,
        observation_schema => <<"sample_observation_schema">>,
        update_digest => <<"sample_update_digest">>
    });
sample(belief_update_rule, minimal) ->
    beam4pm_types:new_belief_update_rule(#{
        rule_id => <<"sample_rule_id">>,
        observation_schema => <<"sample_observation_schema">>,
        update_digest => <<"sample_update_digest">>
    });
sample(beneficial_owner_evidence, full) ->
    beam4pm_types:new_beneficial_owner_evidence(#{
        account_id => <<"sample_account_id">>,
        owner_id => <<"sample_owner_id">>,
        evidence_hash => <<"sample_evidence_hash">>
    });
sample(beneficial_owner_evidence, minimal) ->
    beam4pm_types:new_beneficial_owner_evidence(#{
        account_id => <<"sample_account_id">>,
        owner_id => <<"sample_owner_id">>,
        evidence_hash => <<"sample_evidence_hash">>
    });
sample(billable_usage_identity, full) ->
    beam4pm_types:new_billable_usage_identity(#{
        tenant_id => <<"sample_tenant_id">>,
        billable_usage_id => <<"sample_billable_usage_id">>,
        identity_hash => <<"sample_identity_hash">>
    });
sample(billable_usage_identity, minimal) ->
    beam4pm_types:new_billable_usage_identity(#{
        tenant_id => <<"sample_tenant_id">>,
        billable_usage_id => <<"sample_billable_usage_id">>,
        identity_hash => <<"sample_identity_hash">>
    });
sample(billing_account, full) ->
    beam4pm_types:new_billing_account(#{
        billing_account_id => <<"sample_billing_account_id">>,
        account_id => <<"sample_account_id">>,
        currency => <<"sample_currency">>,
        invoice_profile => <<"sample_invoice_profile">>
    });
sample(billing_account, minimal) ->
    beam4pm_types:new_billing_account(#{
        billing_account_id => <<"sample_billing_account_id">>,
        account_id => <<"sample_account_id">>,
        currency => <<"sample_currency">>,
        invoice_profile => <<"sample_invoice_profile">>
    });
sample(billing_reconciliation, full) ->
    beam4pm_types:new_billing_reconciliation(#{
        entitlement_id => <<"sample_entitlement_id">>,
        metric_name => <<"sample_metric_name">>,
        total_quantity => 3.5,
        applied_event_ids => [<<"alpha">>, <<"beta">>],
        period_start => <<"2026-08-29T12:00:00.123456Z">>,
        period_end => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(billing_reconciliation, minimal) ->
    beam4pm_types:new_billing_reconciliation(#{
        entitlement_id => <<"sample_entitlement_id">>,
        metric_name => <<"sample_metric_name">>,
        total_quantity => 3.5,
        applied_event_ids => [<<"alpha">>, <<"beta">>],
        period_start => <<"2026-08-29T12:00:00.123456Z">>,
        period_end => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(booking_readiness, full) ->
    beam4pm_types:new_booking_readiness(#{
        opportunity_id => <<"sample_opportunity_id">>,
        readiness_id => <<"sample_readiness_id">>,
        decision => <<"sample_decision">>
    });
sample(booking_readiness, minimal) ->
    beam4pm_types:new_booking_readiness(#{
        opportunity_id => <<"sample_opportunity_id">>,
        readiness_id => <<"sample_readiness_id">>,
        decision => <<"sample_decision">>
    });
sample(bounded_work_selection_receipt, full) ->
    beam4pm_types:new_bounded_work_selection_receipt(#{
        receipt_id => <<"sample_receipt_id">>,
        selected_work_item_id => <<"sample_selected_work_item_id">>,
        frontier_digest => <<"sample_frontier_digest">>
    });
sample(bounded_work_selection_receipt, minimal) ->
    beam4pm_types:new_bounded_work_selection_receipt(#{
        receipt_id => <<"sample_receipt_id">>,
        selected_work_item_id => <<"sample_selected_work_item_id">>,
        frontier_digest => <<"sample_frontier_digest">>
    });
sample(brce_actuation_receipt, full) ->
    beam4pm_types:new_brce_actuation_receipt(#{
        tenant_id => <<"sample_tenant_id">>,
        request_id => <<"sample_request_id">>,
        receipt_hash => <<"sample_receipt_hash">>
    });
sample(brce_actuation_receipt, minimal) ->
    beam4pm_types:new_brce_actuation_receipt(#{
        tenant_id => <<"sample_tenant_id">>,
        request_id => <<"sample_request_id">>,
        receipt_hash => <<"sample_receipt_hash">>
    });
sample(brce_actuation_request, full) ->
    beam4pm_types:new_brce_actuation_request(#{
        tenant_id => <<"sample_tenant_id">>,
        request_id => <<"sample_request_id">>,
        authority_hash => <<"sample_authority_hash">>
    });
sample(brce_actuation_request, minimal) ->
    beam4pm_types:new_brce_actuation_request(#{
        tenant_id => <<"sample_tenant_id">>,
        request_id => <<"sample_request_id">>,
        authority_hash => <<"sample_authority_hash">>
    });
sample(budget_period_alignment, full) ->
    beam4pm_types:new_budget_period_alignment(#{
        opportunity_id => <<"sample_opportunity_id">>,
        budget_period => <<"sample_budget_period">>,
        alignment_result => <<"sample_alignment_result">>
    });
sample(budget_period_alignment, minimal) ->
    beam4pm_types:new_budget_period_alignment(#{
        opportunity_id => <<"sample_opportunity_id">>,
        budget_period => <<"sample_budget_period">>,
        alignment_result => <<"sample_alignment_result">>
    });
sample(bundle_conflict, full) ->
    beam4pm_types:new_bundle_conflict(#{
        bundle_conflict_id => <<"sample_bundle_conflict_id">>,
        account_id => <<"sample_account_id">>,
        conflicting_bundle_id => <<"sample_conflicting_bundle_id">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(bundle_conflict, minimal) ->
    beam4pm_types:new_bundle_conflict(#{
        bundle_conflict_id => <<"sample_bundle_conflict_id">>,
        account_id => <<"sample_account_id">>,
        conflicting_bundle_id => <<"sample_conflicting_bundle_id">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(bundle_dependency, full) ->
    beam4pm_types:new_bundle_dependency(#{
        bundle_dependency_id => <<"sample_bundle_dependency_id">>,
        account_id => <<"sample_account_id">>,
        required_bundle_id => <<"sample_required_bundle_id">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(bundle_dependency, minimal) ->
    beam4pm_types:new_bundle_dependency(#{
        bundle_dependency_id => <<"sample_bundle_dependency_id">>,
        account_id => <<"sample_account_id">>,
        required_bundle_id => <<"sample_required_bundle_id">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(burst_pricing_policy, full) ->
    beam4pm_types:new_burst_pricing_policy(#{
        burst_pricing_policy_id => <<"sample_burst_pricing_policy_id">>,
        account_id => <<"sample_account_id">>,
        burst_multiplier => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(burst_pricing_policy, minimal) ->
    beam4pm_types:new_burst_pricing_policy(#{
        burst_pricing_policy_id => <<"sample_burst_pricing_policy_id">>,
        account_id => <<"sample_account_id">>,
        burst_multiplier => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(business_continuity_evidence, full) ->
    beam4pm_types:new_business_continuity_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        continuity_mode => <<"sample_continuity_mode">>,
        observed_result => sample_atom
    });
sample(business_continuity_evidence, minimal) ->
    beam4pm_types:new_business_continuity_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        continuity_mode => <<"sample_continuity_mode">>,
        observed_result => sample_atom
    });
sample(business_outcome_measurement, full) ->
    beam4pm_types:new_business_outcome_measurement(#{
        tenant_id => <<"sample_tenant_id">>,
        outcome_id => <<"sample_outcome_id">>,
        measurement_hash => <<"sample_measurement_hash">>
    });
sample(business_outcome_measurement, minimal) ->
    beam4pm_types:new_business_outcome_measurement(#{
        tenant_id => <<"sample_tenant_id">>,
        outcome_id => <<"sample_outcome_id">>,
        measurement_hash => <<"sample_measurement_hash">>
    });
sample(business_unit_allocation, full) ->
    beam4pm_types:new_business_unit_allocation(#{
        business_unit_allocation_id => <<"sample_business_unit_allocation_id">>,
        account_id => <<"sample_account_id">>,
        business_unit_id => <<"sample_business_unit_id">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(business_unit_allocation, minimal) ->
    beam4pm_types:new_business_unit_allocation(#{
        business_unit_allocation_id => <<"sample_business_unit_allocation_id">>,
        account_id => <<"sample_account_id">>,
        business_unit_id => <<"sample_business_unit_id">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(buying_committee, full) ->
    beam4pm_types:new_buying_committee(#{
        buying_committee_id => <<"sample_buying_committee_id">>,
        account_id => <<"sample_account_id">>,
        committee_coverage => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(buying_committee, minimal) ->
    beam4pm_types:new_buying_committee(#{
        buying_committee_id => <<"sample_buying_committee_id">>,
        account_id => <<"sample_account_id">>,
        committee_coverage => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(caller_local_checkout_observation, full) ->
    beam4pm_types:new_caller_local_checkout_observation(#{
        consumer_repository_id => <<"sample_consumer_repository_id">>,
        checkout_sha => <<"sample_checkout_sha">>,
        checkout_path => <<"sample_checkout_path">>,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(caller_local_checkout_observation, minimal) ->
    beam4pm_types:new_caller_local_checkout_observation(#{
        consumer_repository_id => <<"sample_consumer_repository_id">>,
        checkout_sha => <<"sample_checkout_sha">>,
        checkout_path => <<"sample_checkout_path">>,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(caller_local_consumer, full) ->
    beam4pm_types:new_caller_local_consumer(#{
        consumer_id => <<"sample_consumer_id">>,
        subject_sha => <<"sample_subject_sha">>,
        consumer_hash => <<"sample_consumer_hash">>
    });
sample(caller_local_consumer, minimal) ->
    beam4pm_types:new_caller_local_consumer(#{
        consumer_id => <<"sample_consumer_id">>,
        subject_sha => <<"sample_subject_sha">>,
        consumer_hash => <<"sample_consumer_hash">>
    });
sample(caller_local_crown_identity, full) ->
    beam4pm_types:new_caller_local_crown_identity(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        consumer_subject_sha => <<"sample_consumer_subject_sha">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(caller_local_crown_identity, minimal) ->
    beam4pm_types:new_caller_local_crown_identity(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        consumer_subject_sha => <<"sample_consumer_subject_sha">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(canary_decision, full) ->
    beam4pm_types:new_canary_decision(#{
        canary_decision_id => <<"sample_canary_decision_id">>,
        account_id => <<"sample_account_id">>,
        canary_result => <<"sample_canary_result">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(canary_decision, minimal) ->
    beam4pm_types:new_canary_decision(#{
        canary_decision_id => <<"sample_canary_decision_id">>,
        account_id => <<"sample_account_id">>,
        canary_result => <<"sample_canary_result">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(canary_evidence, full) ->
    beam4pm_types:new_canary_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        canary_percentage => 3.5,
        observed_result => sample_atom
    });
sample(canary_evidence, minimal) ->
    beam4pm_types:new_canary_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        canary_percentage => 3.5,
        observed_result => sample_atom
    });
sample(cancellation_policy, full) ->
    beam4pm_types:new_cancellation_policy(#{
        cancellation_policy_id => <<"sample_cancellation_policy_id">>,
        account_id => <<"sample_account_id">>,
        notice_days => 42,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(cancellation_policy, minimal) ->
    beam4pm_types:new_cancellation_policy(#{
        cancellation_policy_id => <<"sample_cancellation_policy_id">>,
        account_id => <<"sample_account_id">>,
        notice_days => 42,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(canonical_source_authority_observation, full) ->
    beam4pm_types:new_canonical_source_authority_observation(#{
        source_path => <<"sample_source_path">>,
        authority_class => <<"sample_authority_class">>,
        mutation_allowed => true,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(canonical_source_authority_observation, minimal) ->
    beam4pm_types:new_canonical_source_authority_observation(#{
        source_path => <<"sample_source_path">>,
        authority_class => <<"sample_authority_class">>,
        mutation_allowed => true,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(capability_bundle, full) ->
    beam4pm_types:new_capability_bundle(#{
        bundle_id => <<"sample_bundle_id">>,
        name => <<"sample_name">>,
        capability_ids => [<<"alpha">>, <<"beta">>],
        version => <<"sample_version">>
    });
sample(capability_bundle, minimal) ->
    beam4pm_types:new_capability_bundle(#{
        bundle_id => <<"sample_bundle_id">>,
        name => <<"sample_name">>,
        capability_ids => [<<"alpha">>, <<"beta">>],
        version => <<"sample_version">>
    });
sample(capability_gap, full) ->
    beam4pm_types:new_capability_gap(#{
        capability_gap_id => <<"sample_capability_gap_id">>,
        account_id => <<"sample_account_id">>,
        gap_severity => <<"sample_gap_severity">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(capability_gap, minimal) ->
    beam4pm_types:new_capability_gap(#{
        capability_gap_id => <<"sample_capability_gap_id">>,
        account_id => <<"sample_account_id">>,
        gap_severity => <<"sample_gap_severity">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(capability_gap_learning, full) ->
    beam4pm_types:new_capability_gap_learning(#{
        learning_id => <<"sample_learning_id">>,
        observed_capability_id => <<"sample_observed_capability_id">>,
        required_capability_id => <<"sample_required_capability_id">>,
        gap_type => <<"sample_gap_type">>,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(capability_gap_learning, minimal) ->
    beam4pm_types:new_capability_gap_learning(#{
        learning_id => <<"sample_learning_id">>,
        observed_capability_id => <<"sample_observed_capability_id">>,
        required_capability_id => <<"sample_required_capability_id">>,
        gap_type => <<"sample_gap_type">>,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(capsule_availability, full) ->
    beam4pm_types:new_capsule_availability(#{
        capsule_id => <<"sample_capsule_id">>,
        capsule_digest => <<"sample_capsule_digest">>,
        availability => <<"sample_availability">>
    });
sample(capsule_availability, minimal) ->
    beam4pm_types:new_capsule_availability(#{
        capsule_id => <<"sample_capsule_id">>,
        capsule_digest => <<"sample_capsule_digest">>,
        availability => <<"sample_availability">>
    });
sample(capsule_identity, full) ->
    beam4pm_types:new_capsule_identity(#{
        tenant_id => <<"sample_tenant_id">>,
        capsule_id => <<"sample_capsule_id">>,
        identity_hash => <<"sample_identity_hash">>
    });
sample(capsule_identity, minimal) ->
    beam4pm_types:new_capsule_identity(#{
        tenant_id => <<"sample_tenant_id">>,
        capsule_id => <<"sample_capsule_id">>,
        identity_hash => <<"sample_identity_hash">>
    });
sample(case_object_binding, full) ->
    beam4pm_types:new_case_object_binding(#{
        case_id => <<"sample_case_id">>,
        object_id => <<"sample_object_id">>,
        binding_role => <<"sample_binding_role">>
    });
sample(case_object_binding, minimal) ->
    beam4pm_types:new_case_object_binding(#{
        case_id => <<"sample_case_id">>,
        object_id => <<"sample_object_id">>,
        binding_role => <<"sample_binding_role">>
    });
sample(case_stats, full) ->
    beam4pm_types:new_case_stats(#{
        case_id => <<"sample_case_id">>,
        event_count => 42,
        duration_seconds => 3.5
    });
sample(case_stats, minimal) ->
    beam4pm_types:new_case_stats(#{
        case_id => <<"sample_case_id">>,
        event_count => 42
    });
sample(catalog_release, full) ->
    beam4pm_types:new_catalog_release(#{
        release_id => <<"sample_release_id">>,
        version => <<"sample_version">>,
        sku_ids => [<<"alpha">>, <<"beta">>],
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(catalog_release, minimal) ->
    beam4pm_types:new_catalog_release(#{
        release_id => <<"sample_release_id">>,
        version => <<"sample_version">>,
        sku_ids => [<<"alpha">>, <<"beta">>],
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(causal_lineage_observation, full) ->
    beam4pm_types:new_causal_lineage_observation(#{
        cause_observation_id => <<"sample_cause_observation_id">>,
        effect_observation_id => <<"sample_effect_observation_id">>,
        causal_basis => <<"sample_causal_basis">>,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(causal_lineage_observation, minimal) ->
    beam4pm_types:new_causal_lineage_observation(#{
        cause_observation_id => <<"sample_cause_observation_id">>,
        effect_observation_id => <<"sample_effect_observation_id">>,
        causal_basis => <<"sample_causal_basis">>,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(challenger_candidate_evaluation, full) ->
    beam4pm_types:new_challenger_candidate_evaluation(#{
        evaluation_id => <<"sample_evaluation_id">>,
        challenger_policy_id => <<"sample_challenger_policy_id">>,
        episode_set_digest => <<"sample_episode_set_digest">>,
        evaluation_seed => <<"sample_evaluation_seed">>,
        score => 3.5,
        refusal_code => <<"sample_refusal_code">>
    });
sample(challenger_candidate_evaluation, minimal) ->
    beam4pm_types:new_challenger_candidate_evaluation(#{
        evaluation_id => <<"sample_evaluation_id">>,
        challenger_policy_id => <<"sample_challenger_policy_id">>,
        episode_set_digest => <<"sample_episode_set_digest">>,
        evaluation_seed => <<"sample_evaluation_seed">>,
        score => 3.5,
        refusal_code => <<"sample_refusal_code">>
    });
sample(change_control_evidence, full) ->
    beam4pm_types:new_change_control_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        change_request_id => <<"sample_change_request_id">>,
        observed_result => sample_atom
    });
sample(change_control_evidence, minimal) ->
    beam4pm_types:new_change_control_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        change_request_id => <<"sample_change_request_id">>,
        observed_result => sample_atom
    });
sample(change_order_authority, full) ->
    beam4pm_types:new_change_order_authority(#{
        opportunity_id => <<"sample_opportunity_id">>,
        authority_id => <<"sample_authority_id">>,
        evidence_hash => <<"sample_evidence_hash">>
    });
sample(change_order_authority, minimal) ->
    beam4pm_types:new_change_order_authority(#{
        opportunity_id => <<"sample_opportunity_id">>,
        authority_id => <<"sample_authority_id">>,
        evidence_hash => <<"sample_evidence_hash">>
    });
sample(changed_surface_inference, full) ->
    beam4pm_types:new_changed_surface_inference(#{
        prior_tree_sha => <<"sample_prior_tree_sha">>,
        current_tree_sha => <<"sample_current_tree_sha">>,
        changed_surface_digest => <<"sample_changed_surface_digest">>,
        inference_status => <<"sample_inference_status">>
    });
sample(changed_surface_inference, minimal) ->
    beam4pm_types:new_changed_surface_inference(#{
        prior_tree_sha => <<"sample_prior_tree_sha">>,
        current_tree_sha => <<"sample_current_tree_sha">>,
        changed_surface_digest => <<"sample_changed_surface_digest">>,
        inference_status => <<"sample_inference_status">>
    });
sample(channel_agreement, full) ->
    beam4pm_types:new_channel_agreement(#{
        agreement_id => <<"sample_agreement_id">>,
        partner_id => <<"sample_partner_id">>,
        territory => <<"sample_territory">>,
        valid_until => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(channel_agreement, minimal) ->
    beam4pm_types:new_channel_agreement(#{
        agreement_id => <<"sample_agreement_id">>,
        partner_id => <<"sample_partner_id">>,
        territory => <<"sample_territory">>,
        valid_until => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(chargeback_rule, full) ->
    beam4pm_types:new_chargeback_rule(#{
        rule_id => <<"sample_rule_id">>,
        cost_center => <<"sample_cost_center">>,
        metric_name => <<"sample_metric_name">>,
        rate => 3.5
    });
sample(chargeback_rule, minimal) ->
    beam4pm_types:new_chargeback_rule(#{
        rule_id => <<"sample_rule_id">>,
        cost_center => <<"sample_cost_center">>,
        metric_name => <<"sample_metric_name">>,
        rate => 3.5
    });
sample(cluster_quorum_state, full) ->
    beam4pm_types:new_cluster_quorum_state(#{
        tenant_id => <<"sample_tenant_id">>,
        cluster_id => <<"sample_cluster_id">>,
        quorum_hash => <<"sample_quorum_hash">>
    });
sample(cluster_quorum_state, minimal) ->
    beam4pm_types:new_cluster_quorum_state(#{
        tenant_id => <<"sample_tenant_id">>,
        cluster_id => <<"sample_cluster_id">>,
        quorum_hash => <<"sample_quorum_hash">>
    });
sample(co_term_policy, full) ->
    beam4pm_types:new_co_term_policy(#{
        co_term_policy_id => <<"sample_co_term_policy_id">>,
        account_id => <<"sample_account_id">>,
        coterm_date => <<"2026-08-29T12:00:00.123456Z">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(co_term_policy, minimal) ->
    beam4pm_types:new_co_term_policy(#{
        co_term_policy_id => <<"sample_co_term_policy_id">>,
        account_id => <<"sample_account_id">>,
        coterm_date => <<"2026-08-29T12:00:00.123456Z">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(commercial_approval, full) ->
    beam4pm_types:new_commercial_approval(#{
        approval_id => <<"sample_approval_id">>,
        quote_id => <<"sample_quote_id">>,
        authority => <<"sample_authority">>,
        status => sample_atom
    });
sample(commercial_approval, minimal) ->
    beam4pm_types:new_commercial_approval(#{
        approval_id => <<"sample_approval_id">>,
        quote_id => <<"sample_quote_id">>,
        authority => <<"sample_authority">>,
        status => sample_atom
    });
sample(commercial_artifact_crown_evidence, full) ->
    beam4pm_types:new_commercial_artifact_crown_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        artifact_digest => <<"sample_artifact_digest">>,
        observed_result => sample_atom
    });
sample(commercial_artifact_crown_evidence, minimal) ->
    beam4pm_types:new_commercial_artifact_crown_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        artifact_digest => <<"sample_artifact_digest">>,
        observed_result => sample_atom
    });
sample(commercial_exception, full) ->
    beam4pm_types:new_commercial_exception(#{
        opportunity_id => <<"sample_opportunity_id">>,
        exception_id => <<"sample_exception_id">>,
        exception_state => <<"sample_exception_state">>
    });
sample(commercial_exception, minimal) ->
    beam4pm_types:new_commercial_exception(#{
        opportunity_id => <<"sample_opportunity_id">>,
        exception_id => <<"sample_exception_id">>,
        exception_state => <<"sample_exception_state">>
    });
sample(commercial_execution_receipt, full) ->
    beam4pm_types:new_commercial_execution_receipt(#{
        receipt_id => <<"sample_receipt_id">>,
        subject_id => <<"sample_subject_id">>,
        operation => <<"sample_operation">>,
        evidence_hash => <<"sample_evidence_hash">>
    });
sample(commercial_execution_receipt, minimal) ->
    beam4pm_types:new_commercial_execution_receipt(#{
        receipt_id => <<"sample_receipt_id">>,
        subject_id => <<"sample_subject_id">>,
        operation => <<"sample_operation">>,
        evidence_hash => <<"sample_evidence_hash">>
    });
sample(commercial_forecast, full) ->
    beam4pm_types:new_commercial_forecast(#{
        forecast_id => <<"sample_forecast_id">>,
        account_id => <<"sample_account_id">>,
        amount => 3.5,
        confidence => 3.5
    });
sample(commercial_forecast, minimal) ->
    beam4pm_types:new_commercial_forecast(#{
        forecast_id => <<"sample_forecast_id">>,
        account_id => <<"sample_account_id">>,
        amount => 3.5,
        confidence => 3.5
    });
sample(commercial_outcome, full) ->
    beam4pm_types:new_commercial_outcome(#{
        commercial_outcome_id => <<"sample_commercial_outcome_id">>,
        account_id => <<"sample_account_id">>,
        outcome_value => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(commercial_outcome, minimal) ->
    beam4pm_types:new_commercial_outcome(#{
        commercial_outcome_id => <<"sample_commercial_outcome_id">>,
        account_id => <<"sample_account_id">>,
        outcome_value => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(commercial_quote, full) ->
    beam4pm_types:new_commercial_quote(#{
        quote_id => <<"sample_quote_id">>,
        account_id => <<"sample_account_id">>,
        currency => <<"sample_currency">>,
        status => sample_atom
    });
sample(commercial_quote, minimal) ->
    beam4pm_types:new_commercial_quote(#{
        quote_id => <<"sample_quote_id">>,
        account_id => <<"sample_account_id">>,
        currency => <<"sample_currency">>,
        status => sample_atom
    });
sample(commercial_quote_line, full) ->
    beam4pm_types:new_commercial_quote_line(#{
        quote_id => <<"sample_quote_id">>,
        sku => <<"sample_sku">>,
        quantity => 42,
        unit_price => 3.5
    });
sample(commercial_quote_line, minimal) ->
    beam4pm_types:new_commercial_quote_line(#{
        quote_id => <<"sample_quote_id">>,
        sku => <<"sample_sku">>,
        quantity => 42,
        unit_price => 3.5
    });
sample(commercial_value_realization, full) ->
    beam4pm_types:new_commercial_value_realization(#{
        realization_id => <<"sample_realization_id">>,
        baseline_id => <<"sample_baseline_id">>,
        realized_value => 3.5,
        measured_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(commercial_value_realization, minimal) ->
    beam4pm_types:new_commercial_value_realization(#{
        realization_id => <<"sample_realization_id">>,
        baseline_id => <<"sample_baseline_id">>,
        realized_value => 3.5,
        measured_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(commit_check_state_observation, full) ->
    beam4pm_types:new_commit_check_state_observation(#{
        commit_sha => <<"sample_commit_sha">>,
        check_name => <<"sample_check_name">>,
        check_status => <<"sample_check_status">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(commit_check_state_observation, minimal) ->
    beam4pm_types:new_commit_check_state_observation(#{
        commit_sha => <<"sample_commit_sha">>,
        check_name => <<"sample_check_name">>,
        check_status => <<"sample_check_status">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(committed_spend, full) ->
    beam4pm_types:new_committed_spend(#{
        commitment_id => <<"sample_commitment_id">>,
        amount => 3.5,
        currency => <<"sample_currency">>,
        expires_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(committed_spend, minimal) ->
    beam4pm_types:new_committed_spend(#{
        commitment_id => <<"sample_commitment_id">>,
        amount => 3.5,
        currency => <<"sample_currency">>,
        expires_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(committed_spend_admission, full) ->
    beam4pm_types:new_committed_spend_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        commitment_id => <<"sample_commitment_id">>,
        decision => <<"sample_decision">>
    });
sample(committed_spend_admission, minimal) ->
    beam4pm_types:new_committed_spend_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        commitment_id => <<"sample_commitment_id">>,
        decision => <<"sample_decision">>
    });
sample(compatibility_contract, full) ->
    beam4pm_types:new_compatibility_contract(#{
        contract_id => <<"sample_contract_id">>,
        product_version => <<"sample_product_version">>,
        schema_version => <<"sample_schema_version">>,
        api_version => <<"sample_api_version">>
    });
sample(compatibility_contract, minimal) ->
    beam4pm_types:new_compatibility_contract(#{
        contract_id => <<"sample_contract_id">>,
        product_version => <<"sample_product_version">>,
        schema_version => <<"sample_schema_version">>,
        api_version => <<"sample_api_version">>
    });
sample(compound_task_expansion, full) ->
    beam4pm_types:new_compound_task_expansion(#{
        task_id => <<"sample_task_id">>,
        selected_method_id => <<"sample_selected_method_id">>,
        alternative_digest => <<"sample_alternative_digest">>
    });
sample(compound_task_expansion, minimal) ->
    beam4pm_types:new_compound_task_expansion(#{
        task_id => <<"sample_task_id">>,
        selected_method_id => <<"sample_selected_method_id">>,
        alternative_digest => <<"sample_alternative_digest">>
    });
sample(concurrency_pricing_policy, full) ->
    beam4pm_types:new_concurrency_pricing_policy(#{
        concurrency_pricing_policy_id => <<"sample_concurrency_pricing_policy_id">>,
        account_id => <<"sample_account_id">>,
        included_concurrency => 42,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(concurrency_pricing_policy, minimal) ->
    beam4pm_types:new_concurrency_pricing_policy(#{
        concurrency_pricing_policy_id => <<"sample_concurrency_pricing_policy_id">>,
        account_id => <<"sample_account_id">>,
        included_concurrency => 42,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(configuration_export, full) ->
    beam4pm_types:new_configuration_export(#{
        export_id => <<"sample_export_id">>,
        tenant_id => <<"sample_tenant_id">>,
        configuration_hash => <<"sample_configuration_hash">>,
        exported_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(configuration_export, minimal) ->
    beam4pm_types:new_configuration_export(#{
        export_id => <<"sample_export_id">>,
        tenant_id => <<"sample_tenant_id">>,
        configuration_hash => <<"sample_configuration_hash">>,
        exported_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(configuration_import, full) ->
    beam4pm_types:new_configuration_import(#{
        import_id => <<"sample_import_id">>,
        tenant_id => <<"sample_tenant_id">>,
        configuration_hash => <<"sample_configuration_hash">>,
        status => sample_atom
    });
sample(configuration_import, minimal) ->
    beam4pm_types:new_configuration_import(#{
        import_id => <<"sample_import_id">>,
        tenant_id => <<"sample_tenant_id">>,
        configuration_hash => <<"sample_configuration_hash">>,
        status => sample_atom
    });
sample(conformance_result, full) ->
    beam4pm_types:new_conformance_result(#{
        trace_id => <<"sample_trace_id">>,
        fitness => 3.5,
        precision => 3.5
    });
sample(conformance_result, minimal) ->
    beam4pm_types:new_conformance_result(#{
        trace_id => <<"sample_trace_id">>,
        fitness => 3.5
    });
sample(consequential_state_invalidation, full) ->
    beam4pm_types:new_consequential_state_invalidation(#{
        transition_id => <<"sample_transition_id">>,
        affected_state_digest => <<"sample_affected_state_digest">>,
        invalidation_reason => <<"sample_invalidation_reason">>,
        invalidated_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(consequential_state_invalidation, minimal) ->
    beam4pm_types:new_consequential_state_invalidation(#{
        transition_id => <<"sample_transition_id">>,
        affected_state_digest => <<"sample_affected_state_digest">>,
        invalidation_reason => <<"sample_invalidation_reason">>,
        invalidated_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(constraint_set_binding, full) ->
    beam4pm_types:new_constraint_set_binding(#{
        problem_id => <<"sample_problem_id">>,
        constraint_digest => <<"sample_constraint_digest">>,
        binding_mode => <<"sample_binding_mode">>
    });
sample(constraint_set_binding, minimal) ->
    beam4pm_types:new_constraint_set_binding(#{
        problem_id => <<"sample_problem_id">>,
        constraint_digest => <<"sample_constraint_digest">>,
        binding_mode => <<"sample_binding_mode">>
    });
sample(consumer_equivalence_learning_guard, full) ->
    beam4pm_types:new_consumer_equivalence_learning_guard(#{
        guard_id => <<"sample_guard_id">>,
        source_consumer_id => <<"sample_source_consumer_id">>,
        target_consumer_id => <<"sample_target_consumer_id">>,
        equivalence_evidence_digest => <<"sample_equivalence_evidence_digest">>,
        decision => <<"sample_decision">>,
        refusal_code => <<"sample_refusal_code">>
    });
sample(consumer_equivalence_learning_guard, minimal) ->
    beam4pm_types:new_consumer_equivalence_learning_guard(#{
        guard_id => <<"sample_guard_id">>,
        source_consumer_id => <<"sample_source_consumer_id">>,
        target_consumer_id => <<"sample_target_consumer_id">>,
        equivalence_evidence_digest => <<"sample_equivalence_evidence_digest">>,
        decision => <<"sample_decision">>,
        refusal_code => <<"sample_refusal_code">>
    });
sample(consumer_equivalence_proof, full) ->
    beam4pm_types:new_consumer_equivalence_proof(#{
        consumer_set_id => <<"sample_consumer_set_id">>,
        equivalence_proof_hash => <<"sample_equivalence_proof_hash">>,
        standing => <<"sample_standing">>
    });
sample(consumer_equivalence_proof, minimal) ->
    beam4pm_types:new_consumer_equivalence_proof(#{
        consumer_set_id => <<"sample_consumer_set_id">>,
        equivalence_proof_hash => <<"sample_equivalence_proof_hash">>,
        standing => <<"sample_standing">>
    });
sample(consumer_pack_pin_observation, full) ->
    beam4pm_types:new_consumer_pack_pin_observation(#{
        consumer_repository_id => <<"sample_consumer_repository_id">>,
        pack_id => <<"sample_pack_id">>,
        pack_sha => <<"sample_pack_sha">>,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(consumer_pack_pin_observation, minimal) ->
    beam4pm_types:new_consumer_pack_pin_observation(#{
        consumer_repository_id => <<"sample_consumer_repository_id">>,
        pack_id => <<"sample_pack_id">>,
        pack_sha => <<"sample_pack_sha">>,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(consumption_pool, full) ->
    beam4pm_types:new_consumption_pool(#{
        pool_id => <<"sample_pool_id">>,
        account_id => <<"sample_account_id">>,
        unit => <<"sample_unit">>,
        remaining_quantity => 3.5
    });
sample(consumption_pool, minimal) ->
    beam4pm_types:new_consumption_pool(#{
        pool_id => <<"sample_pool_id">>,
        account_id => <<"sample_account_id">>,
        unit => <<"sample_unit">>,
        remaining_quantity => 3.5
    });
sample(consumption_subscription, full) ->
    beam4pm_types:new_consumption_subscription(#{
        subscription_id => <<"sample_subscription_id">>,
        account_id => <<"sample_account_id">>,
        plan_id => <<"sample_plan_id">>,
        status => sample_atom
    });
sample(consumption_subscription, minimal) ->
    beam4pm_types:new_consumption_subscription(#{
        subscription_id => <<"sample_subscription_id">>,
        account_id => <<"sample_account_id">>,
        plan_id => <<"sample_plan_id">>,
        status => sample_atom
    });
sample(container_manifest_digest_observation, full) ->
    beam4pm_types:new_container_manifest_digest_observation(#{
        image_repository => <<"sample_image_repository">>,
        tag => <<"sample_tag">>,
        index_digest => <<"sample_index_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(container_manifest_digest_observation, minimal) ->
    beam4pm_types:new_container_manifest_digest_observation(#{
        image_repository => <<"sample_image_repository">>,
        tag => <<"sample_tag">>,
        index_digest => <<"sample_index_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(container_platform_digest_observation, full) ->
    beam4pm_types:new_container_platform_digest_observation(#{
        index_digest => <<"sample_index_digest">>,
        platform => <<"sample_platform">>,
        platform_digest => <<"sample_platform_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(container_platform_digest_observation, minimal) ->
    beam4pm_types:new_container_platform_digest_observation(#{
        index_digest => <<"sample_index_digest">>,
        platform => <<"sample_platform">>,
        platform_digest => <<"sample_platform_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(contingency_branch, full) ->
    beam4pm_types:new_contingency_branch(#{
        branch_id => <<"sample_branch_id">>,
        observation_predicate => <<"sample_observation_predicate">>,
        next_node_id => <<"sample_next_node_id">>
    });
sample(contingency_branch, minimal) ->
    beam4pm_types:new_contingency_branch(#{
        branch_id => <<"sample_branch_id">>,
        observation_predicate => <<"sample_observation_predicate">>,
        next_node_id => <<"sample_next_node_id">>
    });
sample(contracting_entity_identity, full) ->
    beam4pm_types:new_contracting_entity_identity(#{
        opportunity_id => <<"sample_opportunity_id">>,
        contracting_entity_id => <<"sample_contracting_entity_id">>,
        identity_evidence_hash => <<"sample_identity_evidence_hash">>
    });
sample(contracting_entity_identity, minimal) ->
    beam4pm_types:new_contracting_entity_identity(#{
        opportunity_id => <<"sample_opportunity_id">>,
        contracting_entity_id => <<"sample_contracting_entity_id">>,
        identity_evidence_hash => <<"sample_identity_evidence_hash">>
    });
sample(cost_center_allocation, full) ->
    beam4pm_types:new_cost_center_allocation(#{
        cost_center_allocation_id => <<"sample_cost_center_allocation_id">>,
        account_id => <<"sample_account_id">>,
        cost_center_id => <<"sample_cost_center_id">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(cost_center_allocation, minimal) ->
    beam4pm_types:new_cost_center_allocation(#{
        cost_center_allocation_id => <<"sample_cost_center_allocation_id">>,
        account_id => <<"sample_account_id">>,
        cost_center_id => <<"sample_cost_center_id">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(cost_latency_reliability_tradeoff, full) ->
    beam4pm_types:new_cost_latency_reliability_tradeoff(#{
        assessment_id => <<"sample_assessment_id">>,
        candidate_policy_id => <<"sample_candidate_policy_id">>,
        cost_score => 3.5,
        latency_score => 3.5,
        reliability_score => 3.5,
        utility_score => 3.5
    });
sample(cost_latency_reliability_tradeoff, minimal) ->
    beam4pm_types:new_cost_latency_reliability_tradeoff(#{
        assessment_id => <<"sample_assessment_id">>,
        candidate_policy_id => <<"sample_candidate_policy_id">>,
        cost_score => 3.5,
        latency_score => 3.5,
        reliability_score => 3.5,
        utility_score => 3.5
    });
sample(cost_of_delay_score, full) ->
    beam4pm_types:new_cost_of_delay_score(#{
        option_id => <<"sample_option_id">>,
        horizon => <<"sample_horizon">>,
        score => <<"sample_score">>
    });
sample(cost_of_delay_score, minimal) ->
    beam4pm_types:new_cost_of_delay_score(#{
        option_id => <<"sample_option_id">>,
        horizon => <<"sample_horizon">>,
        score => <<"sample_score">>
    });
sample(cost_to_serve_measurement, full) ->
    beam4pm_types:new_cost_to_serve_measurement(#{
        tenant_id => <<"sample_tenant_id">>,
        billing_period_id => <<"sample_billing_period_id">>,
        measurement_hash => <<"sample_measurement_hash">>
    });
sample(cost_to_serve_measurement, minimal) ->
    beam4pm_types:new_cost_to_serve_measurement(#{
        tenant_id => <<"sample_tenant_id">>,
        billing_period_id => <<"sample_billing_period_id">>,
        measurement_hash => <<"sample_measurement_hash">>
    });
sample(counterfactual_frontier, full) ->
    beam4pm_types:new_counterfactual_frontier(#{
        option_set_id => <<"sample_option_set_id">>,
        world_model_hash => <<"sample_world_model_hash">>,
        frontier_hash => <<"sample_frontier_hash">>
    });
sample(counterfactual_frontier, minimal) ->
    beam4pm_types:new_counterfactual_frontier(#{
        option_set_id => <<"sample_option_set_id">>,
        world_model_hash => <<"sample_world_model_hash">>,
        frontier_hash => <<"sample_frontier_hash">>
    });
sample(counterfactual_replay, full) ->
    beam4pm_types:new_counterfactual_replay(#{
        replay_id => <<"sample_replay_id">>,
        source_episode_id => <<"sample_source_episode_id">>,
        intervention_digest => <<"sample_intervention_digest">>,
        seed => <<"sample_seed">>,
        predicted_outcome => <<"sample_predicted_outcome">>,
        comparison_digest => <<"sample_comparison_digest">>
    });
sample(counterfactual_replay, minimal) ->
    beam4pm_types:new_counterfactual_replay(#{
        replay_id => <<"sample_replay_id">>,
        source_episode_id => <<"sample_source_episode_id">>,
        intervention_digest => <<"sample_intervention_digest">>,
        seed => <<"sample_seed">>,
        predicted_outcome => <<"sample_predicted_outcome">>,
        comparison_digest => <<"sample_comparison_digest">>
    });
sample(crash_recovery_receipt, full) ->
    beam4pm_types:new_crash_recovery_receipt(#{
        tenant_id => <<"sample_tenant_id">>,
        crash_id => <<"sample_crash_id">>,
        receipt_hash => <<"sample_receipt_hash">>
    });
sample(crash_recovery_receipt, minimal) ->
    beam4pm_types:new_crash_recovery_receipt(#{
        tenant_id => <<"sample_tenant_id">>,
        crash_id => <<"sample_crash_id">>,
        receipt_hash => <<"sample_receipt_hash">>
    });
sample(credit_expiry_policy, full) ->
    beam4pm_types:new_credit_expiry_policy(#{
        credit_expiry_policy_id => <<"sample_credit_expiry_policy_id">>,
        account_id => <<"sample_account_id">>,
        expires_at => <<"2026-08-29T12:00:00.123456Z">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(credit_expiry_policy, minimal) ->
    beam4pm_types:new_credit_expiry_policy(#{
        credit_expiry_policy_id => <<"sample_credit_expiry_policy_id">>,
        account_id => <<"sample_account_id">>,
        expires_at => <<"2026-08-29T12:00:00.123456Z">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(credit_risk_admission, full) ->
    beam4pm_types:new_credit_risk_admission(#{
        account_id => <<"sample_account_id">>,
        risk_band => <<"sample_risk_band">>,
        decision => <<"sample_decision">>
    });
sample(credit_risk_admission, minimal) ->
    beam4pm_types:new_credit_risk_admission(#{
        account_id => <<"sample_account_id">>,
        risk_band => <<"sample_risk_band">>,
        decision => <<"sample_decision">>
    });
sample(cross_sell_fit, full) ->
    beam4pm_types:new_cross_sell_fit(#{
        cross_sell_fit_id => <<"sample_cross_sell_fit_id">>,
        account_id => <<"sample_account_id">>,
        cross_sell_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(cross_sell_fit, minimal) ->
    beam4pm_types:new_cross_sell_fit(#{
        cross_sell_fit_id => <<"sample_cross_sell_fit_id">>,
        account_id => <<"sample_account_id">>,
        cross_sell_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(crown_applicable_gate_coverage, full) ->
    beam4pm_types:new_crown_applicable_gate_coverage(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        applicable_gate_set_digest => <<"sample_applicable_gate_set_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_applicable_gate_coverage, minimal) ->
    beam4pm_types:new_crown_applicable_gate_coverage(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        applicable_gate_set_digest => <<"sample_applicable_gate_set_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_artifact_pullback_smoke, full) ->
    beam4pm_types:new_crown_artifact_pullback_smoke(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        pullback_digest => <<"sample_pullback_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_artifact_pullback_smoke, minimal) ->
    beam4pm_types:new_crown_artifact_pullback_smoke(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        pullback_digest => <<"sample_pullback_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_attestation_signer, full) ->
    beam4pm_types:new_crown_attestation_signer(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        signer_identity => <<"sample_signer_identity">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_attestation_signer, minimal) ->
    beam4pm_types:new_crown_attestation_signer(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        signer_identity => <<"sample_signer_identity">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_autonomic_republish, full) ->
    beam4pm_types:new_crown_autonomic_republish(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        republished_crown_digest => <<"sample_republished_crown_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_autonomic_republish, minimal) ->
    beam4pm_types:new_crown_autonomic_republish(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        republished_crown_digest => <<"sample_republished_crown_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_capsule_toolchain, full) ->
    beam4pm_types:new_crown_capsule_toolchain(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        toolchain_digest => <<"sample_toolchain_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_capsule_toolchain, minimal) ->
    beam4pm_types:new_crown_capsule_toolchain(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        toolchain_digest => <<"sample_toolchain_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_cas_promotion, full) ->
    beam4pm_types:new_crown_cas_promotion(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        expected_previous_digest => <<"sample_expected_previous_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_cas_promotion, minimal) ->
    beam4pm_types:new_crown_cas_promotion(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        expected_previous_digest => <<"sample_expected_previous_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_check_relevance, full) ->
    beam4pm_types:new_crown_check_relevance(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        relevance_proof_digest => <<"sample_relevance_proof_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_check_relevance, minimal) ->
    beam4pm_types:new_crown_check_relevance(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        relevance_proof_digest => <<"sample_relevance_proof_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_child_publish_observation, full) ->
    beam4pm_types:new_crown_child_publish_observation(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        child_publish_run_id => <<"sample_child_publish_run_id">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_child_publish_observation, minimal) ->
    beam4pm_types:new_crown_child_publish_observation(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        child_publish_run_id => <<"sample_child_publish_run_id">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_consumer_smoke, full) ->
    beam4pm_types:new_crown_consumer_smoke(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        consumer_smoke_digest => <<"sample_consumer_smoke_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_consumer_smoke, minimal) ->
    beam4pm_types:new_crown_consumer_smoke(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        consumer_smoke_digest => <<"sample_consumer_smoke_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_convergence_proof, full) ->
    beam4pm_types:new_crown_convergence_proof(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        convergence_proof_digest => <<"sample_convergence_proof_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_convergence_proof, minimal) ->
    beam4pm_types:new_crown_convergence_proof(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        convergence_proof_digest => <<"sample_convergence_proof_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_cosign_certificate, full) ->
    beam4pm_types:new_crown_cosign_certificate(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        certificate_identity => <<"sample_certificate_identity">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_cosign_certificate, minimal) ->
    beam4pm_types:new_crown_cosign_certificate(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        certificate_identity => <<"sample_certificate_identity">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_default_head_sensor, full) ->
    beam4pm_types:new_crown_default_head_sensor(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        default_head_sha => <<"sample_default_head_sha">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_default_head_sensor, minimal) ->
    beam4pm_types:new_crown_default_head_sensor(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        default_head_sha => <<"sample_default_head_sha">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_dependency_edge, full) ->
    beam4pm_types:new_crown_dependency_edge(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        downstream_consumer_id => <<"sample_downstream_consumer_id">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_dependency_edge, minimal) ->
    beam4pm_types:new_crown_dependency_edge(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        downstream_consumer_id => <<"sample_downstream_consumer_id">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_execution_mode, full) ->
    beam4pm_types:new_crown_execution_mode(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        execution_mode => <<"sample_execution_mode">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_execution_mode, minimal) ->
    beam4pm_types:new_crown_execution_mode(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        execution_mode => <<"sample_execution_mode">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_fanin_convergence, full) ->
    beam4pm_types:new_crown_fanin_convergence(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        fanin_set_digest => <<"sample_fanin_set_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_fanin_convergence, minimal) ->
    beam4pm_types:new_crown_fanin_convergence(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        fanin_set_digest => <<"sample_fanin_set_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_fanout_batch, full) ->
    beam4pm_types:new_crown_fanout_batch(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        fanout_set_digest => <<"sample_fanout_set_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_fanout_batch, minimal) ->
    beam4pm_types:new_crown_fanout_batch(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        fanout_set_digest => <<"sample_fanout_set_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_federated_phase_receipt, full) ->
    beam4pm_types:new_crown_federated_phase_receipt(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        selected_option_digest => <<"sample_selected_option_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_federated_phase_receipt, minimal) ->
    beam4pm_types:new_crown_federated_phase_receipt(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        selected_option_digest => <<"sample_selected_option_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_freshness_window, full) ->
    beam4pm_types:new_crown_freshness_window(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        fresh_until => <<"sample_fresh_until">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_freshness_window, minimal) ->
    beam4pm_types:new_crown_freshness_window(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        fresh_until => <<"sample_fresh_until">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_generated_source_ownership, full) ->
    beam4pm_types:new_crown_generated_source_ownership(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        canonical_source_path => <<"sample_canonical_source_path">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_generated_source_ownership, minimal) ->
    beam4pm_types:new_crown_generated_source_ownership(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        canonical_source_path => <<"sample_canonical_source_path">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_gitlink_reconciliation, full) ->
    beam4pm_types:new_crown_gitlink_reconciliation(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        gitlink_commit_sha => <<"sample_gitlink_commit_sha">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_gitlink_reconciliation, minimal) ->
    beam4pm_types:new_crown_gitlink_reconciliation(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        gitlink_commit_sha => <<"sample_gitlink_commit_sha">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_immutable_sha_tag, full) ->
    beam4pm_types:new_crown_immutable_sha_tag(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        immutable_tag => <<"sample_immutable_tag">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_immutable_sha_tag, minimal) ->
    beam4pm_types:new_crown_immutable_sha_tag(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        immutable_tag => <<"sample_immutable_tag">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_known_good_rollback, full) ->
    beam4pm_types:new_crown_known_good_rollback(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        rollback_crown_digest => <<"sample_rollback_crown_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_known_good_rollback, minimal) ->
    beam4pm_types:new_crown_known_good_rollback(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        rollback_crown_digest => <<"sample_rollback_crown_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_latency_observation, full) ->
    beam4pm_types:new_crown_latency_observation(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        latency_millis => <<"sample_latency_millis">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_latency_observation, minimal) ->
    beam4pm_types:new_crown_latency_observation(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        latency_millis => <<"sample_latency_millis">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_lock_reconciliation, full) ->
    beam4pm_types:new_crown_lock_reconciliation(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        lock_commit_sha => <<"sample_lock_commit_sha">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_lock_reconciliation, minimal) ->
    beam4pm_types:new_crown_lock_reconciliation(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        lock_commit_sha => <<"sample_lock_commit_sha">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_manufacturer_identity, full) ->
    beam4pm_types:new_crown_manufacturer_identity(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        manufacturer_digest => <<"sample_manufacturer_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_manufacturer_identity, minimal) ->
    beam4pm_types:new_crown_manufacturer_identity(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        manufacturer_digest => <<"sample_manufacturer_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_marketplace_pack_pin, full) ->
    beam4pm_types:new_crown_marketplace_pack_pin(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        pack_commit_sha => <<"sample_pack_commit_sha">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_marketplace_pack_pin, minimal) ->
    beam4pm_types:new_crown_marketplace_pack_pin(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        pack_commit_sha => <<"sample_pack_commit_sha">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_multiarch_platform_set, full) ->
    beam4pm_types:new_crown_multiarch_platform_set(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        platform_set_digest => <<"sample_platform_set_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_multiarch_platform_set, minimal) ->
    beam4pm_types:new_crown_multiarch_platform_set(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        platform_set_digest => <<"sample_platform_set_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_oci_manifest_binding, full) ->
    beam4pm_types:new_crown_oci_manifest_binding(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        oci_index_digest => <<"sample_oci_index_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_oci_manifest_binding, minimal) ->
    beam4pm_types:new_crown_oci_manifest_binding(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        oci_index_digest => <<"sample_oci_index_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_package_pin_reconciliation, full) ->
    beam4pm_types:new_crown_package_pin_reconciliation(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        package_version_digest => <<"sample_package_version_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_package_pin_reconciliation, minimal) ->
    beam4pm_types:new_crown_package_pin_reconciliation(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        package_version_digest => <<"sample_package_version_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_partial_checkpoint, full) ->
    beam4pm_types:new_crown_partial_checkpoint(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        checkpoint_digest => <<"sample_checkpoint_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_partial_checkpoint, minimal) ->
    beam4pm_types:new_crown_partial_checkpoint(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        checkpoint_digest => <<"sample_checkpoint_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_path_skip_refusal, full) ->
    beam4pm_types:new_crown_path_skip_refusal(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        skipped_gate_id => <<"sample_skipped_gate_id">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_path_skip_refusal, minimal) ->
    beam4pm_types:new_crown_path_skip_refusal(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        skipped_gate_id => <<"sample_skipped_gate_id">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_planner_identity, full) ->
    beam4pm_types:new_crown_planner_identity(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        planner_digest => <<"sample_planner_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_planner_identity, minimal) ->
    beam4pm_types:new_crown_planner_identity(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        planner_digest => <<"sample_planner_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_process_runtime_identity, full) ->
    beam4pm_types:new_crown_process_runtime_identity(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        process_runtime_digest => <<"sample_process_runtime_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_process_runtime_identity, minimal) ->
    beam4pm_types:new_crown_process_runtime_identity(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        process_runtime_digest => <<"sample_process_runtime_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_promotion_race, full) ->
    beam4pm_types:new_crown_promotion_race(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        observed_previous_digest => <<"sample_observed_previous_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_promotion_race, minimal) ->
    beam4pm_types:new_crown_promotion_race(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        observed_previous_digest => <<"sample_observed_previous_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_provenance_binding, full) ->
    beam4pm_types:new_crown_provenance_binding(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        provenance_digest => <<"sample_provenance_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_provenance_binding, minimal) ->
    beam4pm_types:new_crown_provenance_binding(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        provenance_digest => <<"sample_provenance_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_receipt_output_ownership, full) ->
    beam4pm_types:new_crown_receipt_output_ownership(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        output_owner => <<"sample_output_owner">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_receipt_output_ownership, minimal) ->
    beam4pm_types:new_crown_receipt_output_ownership(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        output_owner => <<"sample_output_owner">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_recursive_fixed_point, full) ->
    beam4pm_types:new_crown_recursive_fixed_point(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        fixed_point_digest => <<"sample_fixed_point_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_recursive_fixed_point, minimal) ->
    beam4pm_types:new_crown_recursive_fixed_point(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        fixed_point_digest => <<"sample_fixed_point_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_resume_token, full) ->
    beam4pm_types:new_crown_resume_token(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        resume_token_digest => <<"sample_resume_token_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_resume_token, minimal) ->
    beam4pm_types:new_crown_resume_token(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        resume_token_digest => <<"sample_resume_token_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_runtime_identity, full) ->
    beam4pm_types:new_crown_runtime_identity(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        runtime_digest => <<"sample_runtime_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_runtime_identity, minimal) ->
    beam4pm_types:new_crown_runtime_identity(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        runtime_digest => <<"sample_runtime_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_sbom_subject_binding, full) ->
    beam4pm_types:new_crown_sbom_subject_binding(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        sbom_digest => <<"sample_sbom_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_sbom_subject_binding, minimal) ->
    beam4pm_types:new_crown_sbom_subject_binding(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        sbom_digest => <<"sample_sbom_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_second_pass_identity, full) ->
    beam4pm_types:new_crown_second_pass_identity(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        second_pass_digest => <<"sample_second_pass_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_second_pass_identity, minimal) ->
    beam4pm_types:new_crown_second_pass_identity(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        second_pass_digest => <<"sample_second_pass_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_security_scan, full) ->
    beam4pm_types:new_crown_security_scan(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        scan_report_digest => <<"sample_scan_report_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_security_scan, minimal) ->
    beam4pm_types:new_crown_security_scan(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        scan_report_digest => <<"sample_scan_report_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_source_capsule, full) ->
    beam4pm_types:new_crown_source_capsule(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        capsule_digest => <<"sample_capsule_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_source_capsule, minimal) ->
    beam4pm_types:new_crown_source_capsule(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        capsule_digest => <<"sample_capsule_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_stale_refusal, full) ->
    beam4pm_types:new_crown_stale_refusal(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        observed_age_seconds => <<"sample_observed_age_seconds">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_stale_refusal, minimal) ->
    beam4pm_types:new_crown_stale_refusal(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        observed_age_seconds => <<"sample_observed_age_seconds">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_supply_chain_policy, full) ->
    beam4pm_types:new_crown_supply_chain_policy(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        policy_decision_digest => <<"sample_policy_decision_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_supply_chain_policy, minimal) ->
    beam4pm_types:new_crown_supply_chain_policy(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        policy_decision_digest => <<"sample_policy_decision_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_topological_order, full) ->
    beam4pm_types:new_crown_topological_order(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        topological_rank => <<"sample_topological_rank">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_topological_order, minimal) ->
    beam4pm_types:new_crown_topological_order(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        topological_rank => <<"sample_topological_rank">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_validation_pack, full) ->
    beam4pm_types:new_crown_validation_pack(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        validation_pack_sha => <<"sample_validation_pack_sha">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_validation_pack, minimal) ->
    beam4pm_types:new_crown_validation_pack(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        validation_pack_sha => <<"sample_validation_pack_sha">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_workflow_run_receipt, full) ->
    beam4pm_types:new_crown_workflow_run_receipt(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        workflow_run_id => <<"sample_workflow_run_id">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_workflow_run_receipt, minimal) ->
    beam4pm_types:new_crown_workflow_run_receipt(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        workflow_run_id => <<"sample_workflow_run_id">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_zero_unreceipted_writes, full) ->
    beam4pm_types:new_crown_zero_unreceipted_writes(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        write_set_digest => <<"sample_write_set_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(crown_zero_unreceipted_writes, minimal) ->
    beam4pm_types:new_crown_zero_unreceipted_writes(#{
        propagation_id => <<"sample_propagation_id">>,
        subject_sha => <<"sample_subject_sha">>,
        write_set_digest => <<"sample_write_set_digest">>,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(currency_policy, full) ->
    beam4pm_types:new_currency_policy(#{
        currency_policy_id => <<"sample_currency_policy_id">>,
        account_id => <<"sample_account_id">>,
        currency_code => <<"sample_currency_code">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(currency_policy, minimal) ->
    beam4pm_types:new_currency_policy(#{
        currency_policy_id => <<"sample_currency_policy_id">>,
        account_id => <<"sample_account_id">>,
        currency_code => <<"sample_currency_code">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(curriculum_generation, full) ->
    beam4pm_types:new_curriculum_generation(#{
        curriculum_id => <<"sample_curriculum_id">>,
        source_gap_set_digest => <<"sample_source_gap_set_digest">>,
        ordered_hypotheses_digest => <<"sample_ordered_hypotheses_digest">>,
        diversity_score => 3.5,
        falsifier_coverage => 3.5
    });
sample(curriculum_generation, minimal) ->
    beam4pm_types:new_curriculum_generation(#{
        curriculum_id => <<"sample_curriculum_id">>,
        source_gap_set_digest => <<"sample_source_gap_set_digest">>,
        ordered_hypotheses_digest => <<"sample_ordered_hypotheses_digest">>,
        diversity_score => 3.5,
        falsifier_coverage => 3.5
    });
sample(customer_health, full) ->
    beam4pm_types:new_customer_health(#{
        customer_health_id => <<"sample_customer_health_id">>,
        account_id => <<"sample_account_id">>,
        health_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(customer_health, minimal) ->
    beam4pm_types:new_customer_health(#{
        customer_health_id => <<"sample_customer_health_id">>,
        account_id => <<"sample_account_id">>,
        health_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(customer_managed_key_evidence, full) ->
    beam4pm_types:new_customer_managed_key_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        key_identifier => <<"sample_key_identifier">>,
        observed_result => sample_atom
    });
sample(customer_managed_key_evidence, minimal) ->
    beam4pm_types:new_customer_managed_key_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        key_identifier => <<"sample_key_identifier">>,
        observed_result => sample_atom
    });
sample(customer_signal_observation, full) ->
    beam4pm_types:new_customer_signal_observation(#{
        customer_id => <<"sample_customer_id">>,
        signal_type => <<"sample_signal_type">>,
        signal_digest => <<"sample_signal_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(customer_signal_observation, minimal) ->
    beam4pm_types:new_customer_signal_observation(#{
        customer_id => <<"sample_customer_id">>,
        signal_type => <<"sample_signal_type">>,
        signal_digest => <<"sample_signal_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(data_egress_evidence, full) ->
    beam4pm_types:new_data_egress_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        egress_bytes => 42,
        observed_result => sample_atom
    });
sample(data_egress_evidence, minimal) ->
    beam4pm_types:new_data_egress_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        egress_bytes => 42,
        observed_result => sample_atom
    });
sample(data_migration_scope_admission, full) ->
    beam4pm_types:new_data_migration_scope_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        migration_scope_id => <<"sample_migration_scope_id">>,
        decision => <<"sample_decision">>
    });
sample(data_migration_scope_admission, minimal) ->
    beam4pm_types:new_data_migration_scope_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        migration_scope_id => <<"sample_migration_scope_id">>,
        decision => <<"sample_decision">>
    });
sample(data_processing_addendum_state, full) ->
    beam4pm_types:new_data_processing_addendum_state(#{
        opportunity_id => <<"sample_opportunity_id">>,
        addendum_id => <<"sample_addendum_id">>,
        addendum_state => <<"sample_addendum_state">>
    });
sample(data_processing_addendum_state, minimal) ->
    beam4pm_types:new_data_processing_addendum_state(#{
        opportunity_id => <<"sample_opportunity_id">>,
        addendum_id => <<"sample_addendum_id">>,
        addendum_state => <<"sample_addendum_state">>
    });
sample(data_readiness, full) ->
    beam4pm_types:new_data_readiness(#{
        data_readiness_id => <<"sample_data_readiness_id">>,
        account_id => <<"sample_account_id">>,
        data_quality_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(data_readiness, minimal) ->
    beam4pm_types:new_data_readiness(#{
        data_readiness_id => <<"sample_data_readiness_id">>,
        account_id => <<"sample_account_id">>,
        data_quality_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(data_residency_policy, full) ->
    beam4pm_types:new_data_residency_policy(#{
        policy_id => <<"sample_policy_id">>,
        tenant_id => <<"sample_tenant_id">>,
        allowed_regions => [<<"alpha">>, <<"beta">>],
        status => sample_atom
    });
sample(data_residency_policy, minimal) ->
    beam4pm_types:new_data_residency_policy(#{
        policy_id => <<"sample_policy_id">>,
        tenant_id => <<"sample_tenant_id">>,
        allowed_regions => [<<"alpha">>, <<"beta">>],
        status => sample_atom
    });
sample(data_volume_pricing_policy, full) ->
    beam4pm_types:new_data_volume_pricing_policy(#{
        data_volume_pricing_policy_id => <<"sample_data_volume_pricing_policy_id">>,
        account_id => <<"sample_account_id">>,
        unit_gb_price => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(data_volume_pricing_policy, minimal) ->
    beam4pm_types:new_data_volume_pricing_policy(#{
        data_volume_pricing_policy_id => <<"sample_data_volume_pricing_policy_id">>,
        account_id => <<"sample_account_id">>,
        unit_gb_price => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(deal_desk_packet, full) ->
    beam4pm_types:new_deal_desk_packet(#{
        opportunity_id => <<"sample_opportunity_id">>,
        packet_id => <<"sample_packet_id">>,
        evidence_hash => <<"sample_evidence_hash">>
    });
sample(deal_desk_packet, minimal) ->
    beam4pm_types:new_deal_desk_packet(#{
        opportunity_id => <<"sample_opportunity_id">>,
        packet_id => <<"sample_packet_id">>,
        evidence_hash => <<"sample_evidence_hash">>
    });
sample(decision_compression_observation, full) ->
    beam4pm_types:new_decision_compression_observation(#{
        compression_id => <<"sample_compression_id">>,
        input_state_digest => <<"sample_input_state_digest">>,
        output_delta_digest => <<"sample_output_delta_digest">>,
        loss_bound => <<"sample_loss_bound">>
    });
sample(decision_compression_observation, minimal) ->
    beam4pm_types:new_decision_compression_observation(#{
        compression_id => <<"sample_compression_id">>,
        input_state_digest => <<"sample_input_state_digest">>,
        output_delta_digest => <<"sample_output_delta_digest">>,
        loss_bound => <<"sample_loss_bound">>
    });
sample(decision_information_preservation, full) ->
    beam4pm_types:new_decision_information_preservation(#{
        preservation_id => <<"sample_preservation_id">>,
        decision_id => <<"sample_decision_id">>,
        source_partition_digest => <<"sample_source_partition_digest">>,
        projection_digest => <<"sample_projection_digest">>,
        preserved_question_set_digest => <<"sample_preserved_question_set_digest">>,
        loss_score => 3.5
    });
sample(decision_information_preservation, minimal) ->
    beam4pm_types:new_decision_information_preservation(#{
        preservation_id => <<"sample_preservation_id">>,
        decision_id => <<"sample_decision_id">>,
        source_partition_digest => <<"sample_source_partition_digest">>,
        projection_digest => <<"sample_projection_digest">>,
        preserved_question_set_digest => <<"sample_preserved_question_set_digest">>,
        loss_score => 3.5
    });
sample(deletion_proof_evidence, full) ->
    beam4pm_types:new_deletion_proof_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        deletion_receipt_id => <<"sample_deletion_receipt_id">>,
        observed_result => sample_atom
    });
sample(deletion_proof_evidence, minimal) ->
    beam4pm_types:new_deletion_proof_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        deletion_receipt_id => <<"sample_deletion_receipt_id">>,
        observed_result => sample_atom
    });
sample(demo_run, full) ->
    beam4pm_types:new_demo_run(#{
        demo_run_id => <<"sample_demo_run_id">>,
        account_id => <<"sample_account_id">>,
        demo_result => <<"sample_demo_result">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(demo_run, minimal) ->
    beam4pm_types:new_demo_run(#{
        demo_run_id => <<"sample_demo_run_id">>,
        account_id => <<"sample_account_id">>,
        demo_result => <<"sample_demo_result">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(demo_scenario, full) ->
    beam4pm_types:new_demo_scenario(#{
        demo_scenario_id => <<"sample_demo_scenario_id">>,
        account_id => <<"sample_account_id">>,
        scenario_name => <<"sample_scenario_name">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(demo_scenario, minimal) ->
    beam4pm_types:new_demo_scenario(#{
        demo_scenario_id => <<"sample_demo_scenario_id">>,
        account_id => <<"sample_account_id">>,
        scenario_name => <<"sample_scenario_name">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(dependency_dag, full) ->
    beam4pm_types:new_dependency_dag(#{
        dag_id => <<"sample_dag_id">>,
        node_set_hash => <<"sample_node_set_hash">>,
        edge_set_hash => <<"sample_edge_set_hash">>
    });
sample(dependency_dag, minimal) ->
    beam4pm_types:new_dependency_dag(#{
        dag_id => <<"sample_dag_id">>,
        node_set_hash => <<"sample_node_set_hash">>,
        edge_set_hash => <<"sample_edge_set_hash">>
    });
sample(dependency_inventory_evidence, full) ->
    beam4pm_types:new_dependency_inventory_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        dependency_count => 42,
        observed_result => sample_atom
    });
sample(dependency_inventory_evidence, minimal) ->
    beam4pm_types:new_dependency_inventory_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        dependency_count => 42,
        observed_result => sample_atom
    });
sample(dependency_pin_observation, full) ->
    beam4pm_types:new_dependency_pin_observation(#{
        dependency_id => <<"sample_dependency_id">>,
        declared_ref => <<"sample_declared_ref">>,
        resolved_sha => <<"sample_resolved_sha">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(dependency_pin_observation, minimal) ->
    beam4pm_types:new_dependency_pin_observation(#{
        dependency_id => <<"sample_dependency_id">>,
        declared_ref => <<"sample_declared_ref">>,
        resolved_sha => <<"sample_resolved_sha">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(deployment_entitlement, full) ->
    beam4pm_types:new_deployment_entitlement(#{
        entitlement_id => <<"sample_entitlement_id">>,
        tenant_id => <<"sample_tenant_id">>,
        profile_id => <<"sample_profile_id">>,
        valid_until => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(deployment_entitlement, minimal) ->
    beam4pm_types:new_deployment_entitlement(#{
        entitlement_id => <<"sample_entitlement_id">>,
        tenant_id => <<"sample_tenant_id">>,
        profile_id => <<"sample_profile_id">>,
        valid_until => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(developer_readiness, full) ->
    beam4pm_types:new_developer_readiness(#{
        developer_readiness_id => <<"sample_developer_readiness_id">>,
        account_id => <<"sample_account_id">>,
        developer_readiness_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(developer_readiness, minimal) ->
    beam4pm_types:new_developer_readiness(#{
        developer_readiness_id => <<"sample_developer_readiness_id">>,
        account_id => <<"sample_account_id">>,
        developer_readiness_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(deviation_repair_option, full) ->
    beam4pm_types:new_deviation_repair_option(#{
        option_id => <<"sample_option_id">>,
        deviation_id => <<"sample_deviation_id">>,
        repair_plan_digest => <<"sample_repair_plan_digest">>
    });
sample(deviation_repair_option, minimal) ->
    beam4pm_types:new_deviation_repair_option(#{
        option_id => <<"sample_option_id">>,
        deviation_id => <<"sample_deviation_id">>,
        repair_plan_digest => <<"sample_repair_plan_digest">>
    });
sample(dfg_edge, full) ->
    beam4pm_types:new_dfg_edge(#{
        source_activity => <<"sample_source_activity">>,
        target_activity => <<"sample_target_activity">>,
        frequency => 42,
        edge_weight => 3.5
    });
sample(dfg_edge, minimal) ->
    beam4pm_types:new_dfg_edge(#{
        source_activity => <<"sample_source_activity">>,
        target_activity => <<"sample_target_activity">>,
        frequency => 42,
        edge_weight => 3.5
    });
sample(disaster_recovery_evidence, full) ->
    beam4pm_types:new_disaster_recovery_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        recovered_at => <<"2026-08-29T12:00:00.123456Z">>,
        observed_result => sample_atom
    });
sample(disaster_recovery_evidence, minimal) ->
    beam4pm_types:new_disaster_recovery_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        recovered_at => <<"2026-08-29T12:00:00.123456Z">>,
        observed_result => sample_atom
    });
sample(discount_schedule, full) ->
    beam4pm_types:new_discount_schedule(#{
        schedule_id => <<"sample_schedule_id">>,
        threshold => 3.5,
        discount_percent => 3.5,
        currency => <<"sample_currency">>
    });
sample(discount_schedule, minimal) ->
    beam4pm_types:new_discount_schedule(#{
        schedule_id => <<"sample_schedule_id">>,
        threshold => 3.5,
        discount_percent => 3.5,
        currency => <<"sample_currency">>
    });
sample(discovery_hypothesis, full) ->
    beam4pm_types:new_discovery_hypothesis(#{
        discovery_hypothesis_id => <<"sample_discovery_hypothesis_id">>,
        account_id => <<"sample_account_id">>,
        expected_value => <<"sample_expected_value">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(discovery_hypothesis, minimal) ->
    beam4pm_types:new_discovery_hypothesis(#{
        discovery_hypothesis_id => <<"sample_discovery_hypothesis_id">>,
        account_id => <<"sample_account_id">>,
        expected_value => <<"sample_expected_value">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(distributed_work_queue_observation, full) ->
    beam4pm_types:new_distributed_work_queue_observation(#{
        consumer_subject_sha => <<"sample_consumer_subject_sha">>,
        queue_digest => <<"sample_queue_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(distributed_work_queue_observation, minimal) ->
    beam4pm_types:new_distributed_work_queue_observation(#{
        consumer_subject_sha => <<"sample_consumer_subject_sha">>,
        queue_digest => <<"sample_queue_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(dominance_witness, full) ->
    beam4pm_types:new_dominance_witness(#{
        dominant_option_id => <<"sample_dominant_option_id">>,
        dominated_option_id => <<"sample_dominated_option_id">>,
        witness_hash => <<"sample_witness_hash">>
    });
sample(dominance_witness, minimal) ->
    beam4pm_types:new_dominance_witness(#{
        dominant_option_id => <<"sample_dominant_option_id">>,
        dominated_option_id => <<"sample_dominated_option_id">>,
        witness_hash => <<"sample_witness_hash">>
    });
sample(dynamic_replan_trigger, full) ->
    beam4pm_types:new_dynamic_replan_trigger(#{
        plan_id => <<"sample_plan_id">>,
        event_id => <<"sample_event_id">>,
        trigger_hash => <<"sample_trigger_hash">>
    });
sample(dynamic_replan_trigger, minimal) ->
    beam4pm_types:new_dynamic_replan_trigger(#{
        plan_id => <<"sample_plan_id">>,
        event_id => <<"sample_event_id">>,
        trigger_hash => <<"sample_trigger_hash">>
    });
sample(edition_definition, full) ->
    beam4pm_types:new_edition_definition(#{
        edition_id => <<"sample_edition_id">>,
        name => <<"sample_name">>,
        bundle_ids => [<<"alpha">>, <<"beta">>],
        support_tier => <<"sample_support_tier">>
    });
sample(edition_definition, minimal) ->
    beam4pm_types:new_edition_definition(#{
        edition_id => <<"sample_edition_id">>,
        name => <<"sample_name">>,
        bundle_ids => [<<"alpha">>, <<"beta">>],
        support_tier => <<"sample_support_tier">>
    });
sample(edition_downgrade_path, full) ->
    beam4pm_types:new_edition_downgrade_path(#{
        edition_downgrade_path_id => <<"sample_edition_downgrade_path_id">>,
        account_id => <<"sample_account_id">>,
        target_edition_id => <<"sample_target_edition_id">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(edition_downgrade_path, minimal) ->
    beam4pm_types:new_edition_downgrade_path(#{
        edition_downgrade_path_id => <<"sample_edition_downgrade_path_id">>,
        account_id => <<"sample_account_id">>,
        target_edition_id => <<"sample_target_edition_id">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(edition_upgrade_path, full) ->
    beam4pm_types:new_edition_upgrade_path(#{
        edition_upgrade_path_id => <<"sample_edition_upgrade_path_id">>,
        account_id => <<"sample_account_id">>,
        target_edition_id => <<"sample_target_edition_id">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(edition_upgrade_path, minimal) ->
    beam4pm_types:new_edition_upgrade_path(#{
        edition_upgrade_path_id => <<"sample_edition_upgrade_path_id">>,
        account_id => <<"sample_account_id">>,
        target_edition_id => <<"sample_target_edition_id">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(enterprise_agreement, full) ->
    beam4pm_types:new_enterprise_agreement(#{
        enterprise_agreement_id => <<"sample_enterprise_agreement_id">>,
        account_id => <<"sample_account_id">>,
        agreement_version => <<"sample_agreement_version">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(enterprise_agreement, minimal) ->
    beam4pm_types:new_enterprise_agreement(#{
        enterprise_agreement_id => <<"sample_enterprise_agreement_id">>,
        account_id => <<"sample_account_id">>,
        agreement_version => <<"sample_agreement_version">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(enterprise_order, full) ->
    beam4pm_types:new_enterprise_order(#{
        order_id => <<"sample_order_id">>,
        account_id => <<"sample_account_id">>,
        quote_id => <<"sample_quote_id">>,
        status => sample_atom
    });
sample(enterprise_order, minimal) ->
    beam4pm_types:new_enterprise_order(#{
        order_id => <<"sample_order_id">>,
        account_id => <<"sample_account_id">>,
        quote_id => <<"sample_quote_id">>,
        status => sample_atom
    });
sample(enterprise_order_line, full) ->
    beam4pm_types:new_enterprise_order_line(#{
        order_id => <<"sample_order_id">>,
        sku => <<"sample_sku">>,
        quantity => 42,
        unit_price => 3.5
    });
sample(enterprise_order_line, minimal) ->
    beam4pm_types:new_enterprise_order_line(#{
        order_id => <<"sample_order_id">>,
        sku => <<"sample_sku">>,
        quantity => 42,
        unit_price => 3.5
    });
sample(entitlement_denial_receipt, full) ->
    beam4pm_types:new_entitlement_denial_receipt(#{
        tenant_id => <<"sample_tenant_id">>,
        entitlement_id => <<"sample_entitlement_id">>,
        denial_hash => <<"sample_denial_hash">>
    });
sample(entitlement_denial_receipt, minimal) ->
    beam4pm_types:new_entitlement_denial_receipt(#{
        tenant_id => <<"sample_tenant_id">>,
        entitlement_id => <<"sample_entitlement_id">>,
        denial_hash => <<"sample_denial_hash">>
    });
sample(entitlement_event, full) ->
    beam4pm_types:new_entitlement_event(#{
        event_id => <<"sample_event_id">>,
        entitlement_id => <<"sample_entitlement_id">>,
        event_type => <<"sample_event_type">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>,
        payload => #{<<"k">> => <<"v">>}
    });
sample(entitlement_event, minimal) ->
    beam4pm_types:new_entitlement_event(#{
        event_id => <<"sample_event_id">>,
        entitlement_id => <<"sample_entitlement_id">>,
        event_type => <<"sample_event_type">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(entitlement_evidence, full) ->
    beam4pm_types:new_entitlement_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        entitlement_id => <<"sample_entitlement_id">>,
        observed_result => sample_atom
    });
sample(entitlement_evidence, minimal) ->
    beam4pm_types:new_entitlement_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        entitlement_id => <<"sample_entitlement_id">>,
        observed_result => sample_atom
    });
sample(entitlement_grant, full) ->
    beam4pm_types:new_entitlement_grant(#{
        grant_id => <<"sample_grant_id">>,
        tenant_id => <<"sample_tenant_id">>,
        capability_id => <<"sample_capability_id">>,
        valid_until => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(entitlement_grant, minimal) ->
    beam4pm_types:new_entitlement_grant(#{
        grant_id => <<"sample_grant_id">>,
        tenant_id => <<"sample_tenant_id">>,
        capability_id => <<"sample_capability_id">>,
        valid_until => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(entitlement_revocation, full) ->
    beam4pm_types:new_entitlement_revocation(#{
        revocation_id => <<"sample_revocation_id">>,
        grant_id => <<"sample_grant_id">>,
        reason => <<"sample_reason">>,
        revoked_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(entitlement_revocation, minimal) ->
    beam4pm_types:new_entitlement_revocation(#{
        revocation_id => <<"sample_revocation_id">>,
        grant_id => <<"sample_grant_id">>,
        reason => <<"sample_reason">>,
        revoked_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(entitlement_runtime_check, full) ->
    beam4pm_types:new_entitlement_runtime_check(#{
        tenant_id => <<"sample_tenant_id">>,
        entitlement_id => <<"sample_entitlement_id">>,
        check_hash => <<"sample_check_hash">>
    });
sample(entitlement_runtime_check, minimal) ->
    beam4pm_types:new_entitlement_runtime_check(#{
        tenant_id => <<"sample_tenant_id">>,
        entitlement_id => <<"sample_entitlement_id">>,
        check_hash => <<"sample_check_hash">>
    });
sample(entitlement_state, full) ->
    beam4pm_types:new_entitlement_state(#{
        entitlement_id => <<"sample_entitlement_id">>,
        status => <<"sample_status">>,
        last_applied_event_id => <<"sample_last_applied_event_id">>,
        updated_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(entitlement_state, minimal) ->
    beam4pm_types:new_entitlement_state(#{
        entitlement_id => <<"sample_entitlement_id">>,
        status => <<"sample_status">>,
        last_applied_event_id => <<"sample_last_applied_event_id">>,
        updated_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(entropy_reduction_score, full) ->
    beam4pm_types:new_entropy_reduction_score(#{
        action_id => <<"sample_action_id">>,
        prior_entropy => <<"sample_prior_entropy">>,
        expected_posterior_entropy => <<"sample_expected_posterior_entropy">>
    });
sample(entropy_reduction_score, minimal) ->
    beam4pm_types:new_entropy_reduction_score(#{
        action_id => <<"sample_action_id">>,
        prior_entropy => <<"sample_prior_entropy">>,
        expected_posterior_entropy => <<"sample_expected_posterior_entropy">>
    });
sample(environment_failure_separation, full) ->
    beam4pm_types:new_environment_failure_separation(#{
        assessment_id => <<"sample_assessment_id">>,
        exact_subject_sha => <<"sample_exact_subject_sha">>,
        capsule_digest => <<"sample_capsule_digest">>,
        failure_signal => <<"sample_failure_signal">>,
        classification_basis => <<"sample_classification_basis">>
    });
sample(environment_failure_separation, minimal) ->
    beam4pm_types:new_environment_failure_separation(#{
        assessment_id => <<"sample_assessment_id">>,
        exact_subject_sha => <<"sample_exact_subject_sha">>,
        capsule_digest => <<"sample_capsule_digest">>,
        failure_signal => <<"sample_failure_signal">>,
        classification_basis => <<"sample_classification_basis">>
    });
sample(environment_identity, full) ->
    beam4pm_types:new_environment_identity(#{
        tenant_id => <<"sample_tenant_id">>,
        environment_id => <<"sample_environment_id">>,
        identity_hash => <<"sample_identity_hash">>
    });
sample(environment_identity, minimal) ->
    beam4pm_types:new_environment_identity(#{
        tenant_id => <<"sample_tenant_id">>,
        environment_id => <<"sample_environment_id">>,
        identity_hash => <<"sample_identity_hash">>
    });
sample(environment_pricing_policy, full) ->
    beam4pm_types:new_environment_pricing_policy(#{
        environment_pricing_policy_id => <<"sample_environment_pricing_policy_id">>,
        account_id => <<"sample_account_id">>,
        environment_tier => <<"sample_environment_tier">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(environment_pricing_policy, minimal) ->
    beam4pm_types:new_environment_pricing_policy(#{
        environment_pricing_policy_id => <<"sample_environment_pricing_policy_id">>,
        account_id => <<"sample_account_id">>,
        environment_tier => <<"sample_environment_tier">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(environment_profile, full) ->
    beam4pm_types:new_environment_profile(#{
        profile_id => <<"sample_profile_id">>,
        environment => sample_atom,
        region => <<"sample_region">>,
        configuration_hash => <<"sample_configuration_hash">>
    });
sample(environment_profile, minimal) ->
    beam4pm_types:new_environment_profile(#{
        profile_id => <<"sample_profile_id">>,
        environment => sample_atom,
        region => <<"sample_region">>,
        configuration_hash => <<"sample_configuration_hash">>
    });
sample(environment_signal_observation, full) ->
    beam4pm_types:new_environment_signal_observation(#{
        environment_id => <<"sample_environment_id">>,
        signal_type => <<"sample_signal_type">>,
        signal_digest => <<"sample_signal_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(environment_signal_observation, minimal) ->
    beam4pm_types:new_environment_signal_observation(#{
        environment_id => <<"sample_environment_id">>,
        signal_type => <<"sample_signal_type">>,
        signal_digest => <<"sample_signal_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(error_budget_state, full) ->
    beam4pm_types:new_error_budget_state(#{
        tenant_id => <<"sample_tenant_id">>,
        slo_id => <<"sample_slo_id">>,
        state_hash => <<"sample_state_hash">>
    });
sample(error_budget_state, minimal) ->
    beam4pm_types:new_error_budget_state(#{
        tenant_id => <<"sample_tenant_id">>,
        slo_id => <<"sample_slo_id">>,
        state_hash => <<"sample_state_hash">>
    });
sample(evaluation_seed_binding, full) ->
    beam4pm_types:new_evaluation_seed_binding(#{
        binding_id => <<"sample_binding_id">>,
        evaluation_id => <<"sample_evaluation_id">>,
        seed => <<"sample_seed">>,
        result_digest => <<"sample_result_digest">>,
        source_digest => <<"sample_source_digest">>,
        deterministic => true
    });
sample(evaluation_seed_binding, minimal) ->
    beam4pm_types:new_evaluation_seed_binding(#{
        binding_id => <<"sample_binding_id">>,
        evaluation_id => <<"sample_evaluation_id">>,
        seed => <<"sample_seed">>,
        result_digest => <<"sample_result_digest">>,
        source_digest => <<"sample_source_digest">>,
        deterministic => true
    });
sample(event_log, full) ->
    beam4pm_types:new_event_log(#{
        log_id => <<"sample_log_id">>,
        name => <<"sample_name">>,
        description => <<"sample_description">>
    });
sample(event_log, minimal) ->
    beam4pm_types:new_event_log(#{
        log_id => <<"sample_log_id">>,
        name => <<"sample_name">>
    });
sample(event_triggered_planning, full) ->
    beam4pm_types:new_event_triggered_planning(#{
        event_id => <<"sample_event_id">>,
        world_state_hash => <<"sample_world_state_hash">>,
        episode_id => <<"sample_episode_id">>
    });
sample(event_triggered_planning, minimal) ->
    beam4pm_types:new_event_triggered_planning(#{
        event_id => <<"sample_event_id">>,
        world_state_hash => <<"sample_world_state_hash">>,
        episode_id => <<"sample_episode_id">>
    });
sample(event_type, full) ->
    beam4pm_types:new_event_type(#{
        type_name => <<"sample_type_name">>,
        attribute_names => [<<"alpha">>, <<"beta">>]
    });
sample(event_type, minimal) ->
    beam4pm_types:new_event_type(#{
        type_name => <<"sample_type_name">>
    });
sample(event_volume_pricing_policy, full) ->
    beam4pm_types:new_event_volume_pricing_policy(#{
        event_volume_pricing_policy_id => <<"sample_event_volume_pricing_policy_id">>,
        account_id => <<"sample_account_id">>,
        unit_event_price => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(event_volume_pricing_policy, minimal) ->
    beam4pm_types:new_event_volume_pricing_policy(#{
        event_volume_pricing_policy_id => <<"sample_event_volume_pricing_policy_id">>,
        account_id => <<"sample_account_id">>,
        unit_event_price => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(evidence_freshness_evidence, full) ->
    beam4pm_types:new_evidence_freshness_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>,
        observed_result => sample_atom
    });
sample(evidence_freshness_evidence, minimal) ->
    beam4pm_types:new_evidence_freshness_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>,
        observed_result => sample_atom
    });
sample(evidence_training_sample, full) ->
    beam4pm_types:new_evidence_training_sample(#{
        sample_id => <<"sample_sample_id">>,
        episode_id => <<"sample_episode_id">>,
        feature_digest => <<"sample_feature_digest">>,
        label_id => <<"sample_label_id">>,
        provenance_digest => <<"sample_provenance_digest">>
    });
sample(evidence_training_sample, minimal) ->
    beam4pm_types:new_evidence_training_sample(#{
        sample_id => <<"sample_sample_id">>,
        episode_id => <<"sample_episode_id">>,
        feature_digest => <<"sample_feature_digest">>,
        label_id => <<"sample_label_id">>,
        provenance_digest => <<"sample_provenance_digest">>
    });
sample(exact_subject_binding, full) ->
    beam4pm_types:new_exact_subject_binding(#{
        subject_id => <<"sample_subject_id">>,
        subject_sha => <<"sample_subject_sha">>,
        binding_hash => <<"sample_binding_hash">>
    });
sample(exact_subject_binding, minimal) ->
    beam4pm_types:new_exact_subject_binding(#{
        subject_id => <<"sample_subject_id">>,
        subject_sha => <<"sample_subject_sha">>,
        binding_hash => <<"sample_binding_hash">>
    });
sample(exact_world_state_admission, full) ->
    beam4pm_types:new_exact_world_state_admission(#{
        subject_sha => <<"sample_subject_sha">>,
        state_digest => <<"sample_state_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(exact_world_state_admission, minimal) ->
    beam4pm_types:new_exact_world_state_admission(#{
        subject_sha => <<"sample_subject_sha">>,
        state_digest => <<"sample_state_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(exception_authority, full) ->
    beam4pm_types:new_exception_authority(#{
        exception_id => <<"sample_exception_id">>,
        authority_id => <<"sample_authority_id">>,
        decision => <<"sample_decision">>
    });
sample(exception_authority, minimal) ->
    beam4pm_types:new_exception_authority(#{
        exception_id => <<"sample_exception_id">>,
        authority_id => <<"sample_authority_id">>,
        decision => <<"sample_decision">>
    });
sample(executive_business_review, full) ->
    beam4pm_types:new_executive_business_review(#{
        executive_business_review_id => <<"sample_executive_business_review_id">>,
        account_id => <<"sample_account_id">>,
        executive_outcome => <<"sample_executive_outcome">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(executive_business_review, minimal) ->
    beam4pm_types:new_executive_business_review(#{
        executive_business_review_id => <<"sample_executive_business_review_id">>,
        account_id => <<"sample_account_id">>,
        executive_outcome => <<"sample_executive_outcome">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(executive_sponsor, full) ->
    beam4pm_types:new_executive_sponsor(#{
        executive_sponsor_id => <<"sample_executive_sponsor_id">>,
        account_id => <<"sample_account_id">>,
        sponsor_commitment => <<"sample_sponsor_commitment">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(executive_sponsor, minimal) ->
    beam4pm_types:new_executive_sponsor(#{
        executive_sponsor_id => <<"sample_executive_sponsor_id">>,
        account_id => <<"sample_account_id">>,
        sponsor_commitment => <<"sample_sponsor_commitment">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(expansion_opportunity, full) ->
    beam4pm_types:new_expansion_opportunity(#{
        expansion_opportunity_id => <<"sample_expansion_opportunity_id">>,
        account_id => <<"sample_account_id">>,
        expansion_value => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(expansion_opportunity, minimal) ->
    beam4pm_types:new_expansion_opportunity(#{
        expansion_opportunity_id => <<"sample_expansion_opportunity_id">>,
        account_id => <<"sample_account_id">>,
        expansion_value => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(expansion_option, full) ->
    beam4pm_types:new_expansion_option(#{
        option_id => <<"sample_option_id">>,
        account_id => <<"sample_account_id">>,
        sku => <<"sample_sku">>,
        max_quantity => 42
    });
sample(expansion_option, minimal) ->
    beam4pm_types:new_expansion_option(#{
        option_id => <<"sample_option_id">>,
        account_id => <<"sample_account_id">>,
        sku => <<"sample_sku">>,
        max_quantity => 42
    });
sample(expansion_receipt, full) ->
    beam4pm_types:new_expansion_receipt(#{
        expansion_receipt_id => <<"sample_expansion_receipt_id">>,
        account_id => <<"sample_account_id">>,
        receipt_value => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(expansion_receipt, minimal) ->
    beam4pm_types:new_expansion_receipt(#{
        expansion_receipt_id => <<"sample_expansion_receipt_id">>,
        account_id => <<"sample_account_id">>,
        receipt_value => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(expansion_signal, full) ->
    beam4pm_types:new_expansion_signal(#{
        expansion_signal_id => <<"sample_expansion_signal_id">>,
        account_id => <<"sample_account_id">>,
        expansion_signal_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(expansion_signal, minimal) ->
    beam4pm_types:new_expansion_signal(#{
        expansion_signal_id => <<"sample_expansion_signal_id">>,
        account_id => <<"sample_account_id">>,
        expansion_signal_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(experiment_learning_receipt, full) ->
    beam4pm_types:new_experiment_learning_receipt(#{
        receipt_id => <<"sample_receipt_id">>,
        experiment_id => <<"sample_experiment_id">>,
        exact_subject_sha => <<"sample_exact_subject_sha">>,
        evidence_digest => <<"sample_evidence_digest">>,
        result_digest => <<"sample_result_digest">>,
        authority_ceiling => <<"sample_authority_ceiling">>
    });
sample(experiment_learning_receipt, minimal) ->
    beam4pm_types:new_experiment_learning_receipt(#{
        receipt_id => <<"sample_receipt_id">>,
        experiment_id => <<"sample_experiment_id">>,
        exact_subject_sha => <<"sample_exact_subject_sha">>,
        evidence_digest => <<"sample_evidence_digest">>,
        result_digest => <<"sample_result_digest">>,
        authority_ceiling => <<"sample_authority_ceiling">>
    });
sample(failed_challenger_retention, full) ->
    beam4pm_types:new_failed_challenger_retention(#{
        retention_id => <<"sample_retention_id">>,
        challenger_evaluation_id => <<"sample_challenger_evaluation_id">>,
        failure_evidence_digest => <<"sample_failure_evidence_digest">>,
        retained_at => <<"2026-08-29T12:00:00.123456Z">>,
        eligible_for_future => true
    });
sample(failed_challenger_retention, minimal) ->
    beam4pm_types:new_failed_challenger_retention(#{
        retention_id => <<"sample_retention_id">>,
        challenger_evaluation_id => <<"sample_challenger_evaluation_id">>,
        failure_evidence_digest => <<"sample_failure_evidence_digest">>,
        retained_at => <<"2026-08-29T12:00:00.123456Z">>,
        eligible_for_future => true
    });
sample(failure_label, full) ->
    beam4pm_types:new_failure_label(#{
        failure_id => <<"sample_failure_id">>,
        episode_id => <<"sample_episode_id">>,
        failure_class => <<"sample_failure_class">>,
        causal_scope => <<"sample_causal_scope">>,
        preserved_evidence_digest => <<"sample_preserved_evidence_digest">>
    });
sample(failure_label, minimal) ->
    beam4pm_types:new_failure_label(#{
        failure_id => <<"sample_failure_id">>,
        episode_id => <<"sample_episode_id">>,
        failure_class => <<"sample_failure_class">>,
        causal_scope => <<"sample_causal_scope">>,
        preserved_evidence_digest => <<"sample_preserved_evidence_digest">>
    });
sample(fairness_assumption, full) ->
    beam4pm_types:new_fairness_assumption(#{
        assumption_id => <<"sample_assumption_id">>,
        policy_digest => <<"sample_policy_digest">>,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(fairness_assumption, minimal) ->
    beam4pm_types:new_fairness_assumption(#{
        assumption_id => <<"sample_assumption_id">>,
        policy_digest => <<"sample_policy_digest">>,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(federated_dogfood_learning_crown, full) ->
    beam4pm_types:new_federated_dogfood_learning_crown(#{
        crown_id => <<"sample_crown_id">>,
        exact_subject_sha => <<"sample_exact_subject_sha">>,
        consumer_id => <<"sample_consumer_id">>,
        pack_sha => <<"sample_pack_sha">>,
        capsule_digest => <<"sample_capsule_digest">>,
        manufacture_receipt_digest => <<"sample_manufacture_receipt_digest">>,
        ownership_verification_digest => <<"sample_ownership_verification_digest">>,
        second_pass_identity_digest => <<"sample_second_pass_identity_digest">>,
        workflow_sequence => <<"sample_workflow_sequence">>,
        authority_ceiling => <<"sample_authority_ceiling">>
    });
sample(federated_dogfood_learning_crown, minimal) ->
    beam4pm_types:new_federated_dogfood_learning_crown(#{
        crown_id => <<"sample_crown_id">>,
        exact_subject_sha => <<"sample_exact_subject_sha">>,
        consumer_id => <<"sample_consumer_id">>,
        pack_sha => <<"sample_pack_sha">>,
        capsule_digest => <<"sample_capsule_digest">>,
        manufacture_receipt_digest => <<"sample_manufacture_receipt_digest">>,
        ownership_verification_digest => <<"sample_ownership_verification_digest">>,
        second_pass_identity_digest => <<"sample_second_pass_identity_digest">>,
        workflow_sequence => <<"sample_workflow_sequence">>,
        authority_ceiling => <<"sample_authority_ceiling">>
    });
sample(forged_receipt_refusal, full) ->
    beam4pm_types:new_forged_receipt_refusal(#{
        tenant_id => <<"sample_tenant_id">>,
        receipt_id => <<"sample_receipt_id">>,
        refusal_hash => <<"sample_refusal_hash">>
    });
sample(forged_receipt_refusal, minimal) ->
    beam4pm_types:new_forged_receipt_refusal(#{
        tenant_id => <<"sample_tenant_id">>,
        receipt_id => <<"sample_receipt_id">>,
        refusal_hash => <<"sample_refusal_hash">>
    });
sample(frontier_benchmark, full) ->
    beam4pm_types:new_frontier_benchmark(#{
        benchmark_id => <<"sample_benchmark_id">>,
        metric => <<"sample_metric">>,
        acceptance_predicate => <<"sample_acceptance_predicate">>,
        falsifier => <<"sample_falsifier">>
    });
sample(frontier_benchmark, minimal) ->
    beam4pm_types:new_frontier_benchmark(#{
        benchmark_id => <<"sample_benchmark_id">>,
        metric => <<"sample_metric">>,
        acceptance_predicate => <<"sample_acceptance_predicate">>,
        falsifier => <<"sample_falsifier">>
    });
sample(frontier_evidence, full) ->
    beam4pm_types:new_frontier_evidence(#{
        subject_identity => <<"sample_subject_identity">>,
        verifier_identity => <<"sample_verifier_identity">>,
        receipt_ref => <<"sample_receipt_ref">>,
        replay_ref => <<"sample_replay_ref">>,
        standing => sample_atom
    });
sample(frontier_evidence, minimal) ->
    beam4pm_types:new_frontier_evidence(#{
        subject_identity => <<"sample_subject_identity">>,
        verifier_identity => <<"sample_verifier_identity">>,
        receipt_ref => <<"sample_receipt_ref">>,
        replay_ref => <<"sample_replay_ref">>,
        standing => sample_atom
    });
sample(frontier_opportunity, full) ->
    beam4pm_types:new_frontier_opportunity(#{
        source_digest => <<"sample_source_digest">>,
        response_mode => sample_atom,
        target_repository => <<"sample_target_repository">>,
        required_capability => <<"sample_required_capability">>,
        benchmark_id => <<"sample_benchmark_id">>
    });
sample(frontier_opportunity, minimal) ->
    beam4pm_types:new_frontier_opportunity(#{
        source_digest => <<"sample_source_digest">>,
        response_mode => sample_atom,
        target_repository => <<"sample_target_repository">>,
        required_capability => <<"sample_required_capability">>,
        benchmark_id => <<"sample_benchmark_id">>
    });
sample(frontier_source_release, full) ->
    beam4pm_types:new_frontier_source_release(#{
        source_url => <<"sample_source_url">>,
        publisher => <<"sample_publisher">>,
        published_at => <<"2026-08-29T12:00:00.123456Z">>,
        content_digest => <<"sample_content_digest">>,
        claims => #{<<"k">> => <<"v">>}
    });
sample(frontier_source_release, minimal) ->
    beam4pm_types:new_frontier_source_release(#{
        source_url => <<"sample_source_url">>,
        publisher => <<"sample_publisher">>,
        published_at => <<"2026-08-29T12:00:00.123456Z">>,
        content_digest => <<"sample_content_digest">>,
        claims => #{<<"k">> => <<"v">>}
    });
sample(funding_approval_chain, full) ->
    beam4pm_types:new_funding_approval_chain(#{
        opportunity_id => <<"sample_opportunity_id">>,
        approval_chain_id => <<"sample_approval_chain_id">>,
        evidence_hash => <<"sample_evidence_hash">>
    });
sample(funding_approval_chain, minimal) ->
    beam4pm_types:new_funding_approval_chain(#{
        opportunity_id => <<"sample_opportunity_id">>,
        approval_chain_id => <<"sample_approval_chain_id">>,
        evidence_hash => <<"sample_evidence_hash">>
    });
sample(fx_conversion_policy, full) ->
    beam4pm_types:new_fx_conversion_policy(#{
        fx_conversion_policy_id => <<"sample_fx_conversion_policy_id">>,
        account_id => <<"sample_account_id">>,
        rate_source => <<"sample_rate_source">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(fx_conversion_policy, minimal) ->
    beam4pm_types:new_fx_conversion_policy(#{
        fx_conversion_policy_id => <<"sample_fx_conversion_policy_id">>,
        account_id => <<"sample_account_id">>,
        rate_source => <<"sample_rate_source">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(generated_hypothesis, full) ->
    beam4pm_types:new_generated_hypothesis(#{
        hypothesis_id => <<"sample_hypothesis_id">>,
        source_gap_id => <<"sample_source_gap_id">>,
        candidate_capability => <<"sample_candidate_capability">>,
        expected_value => 3.5,
        falsifier_contract => <<"sample_falsifier_contract">>
    });
sample(generated_hypothesis, minimal) ->
    beam4pm_types:new_generated_hypothesis(#{
        hypothesis_id => <<"sample_hypothesis_id">>,
        source_gap_id => <<"sample_source_gap_id">>,
        candidate_capability => <<"sample_candidate_capability">>,
        expected_value => 3.5,
        falsifier_contract => <<"sample_falsifier_contract">>
    });
sample(generated_output_ownership_observation, full) ->
    beam4pm_types:new_generated_output_ownership_observation(#{
        output_path => <<"sample_output_path">>,
        ownership_marker => <<"sample_ownership_marker">>,
        source_input_digest => <<"sample_source_input_digest">>,
        standing => <<"sample_standing">>
    });
sample(generated_output_ownership_observation, minimal) ->
    beam4pm_types:new_generated_output_ownership_observation(#{
        output_path => <<"sample_output_path">>,
        ownership_marker => <<"sample_ownership_marker">>,
        source_input_digest => <<"sample_source_input_digest">>,
        standing => <<"sample_standing">>
    });
sample(generated_source_route, full) ->
    beam4pm_types:new_generated_source_route(#{
        projection_id => <<"sample_projection_id">>,
        source_coordinate => <<"sample_source_coordinate">>,
        route => <<"sample_route">>
    });
sample(generated_source_route, minimal) ->
    beam4pm_types:new_generated_source_route(#{
        projection_id => <<"sample_projection_id">>,
        source_coordinate => <<"sample_source_coordinate">>,
        route => <<"sample_route">>
    });
sample(goal_set_binding, full) ->
    beam4pm_types:new_goal_set_binding(#{
        problem_id => <<"sample_problem_id">>,
        goal_set_digest => <<"sample_goal_set_digest">>,
        value_profile => <<"sample_value_profile">>
    });
sample(goal_set_binding, minimal) ->
    beam4pm_types:new_goal_set_binding(#{
        problem_id => <<"sample_problem_id">>,
        goal_set_digest => <<"sample_goal_set_digest">>,
        value_profile => <<"sample_value_profile">>
    });
sample(hddl_method_candidate, full) ->
    beam4pm_types:new_hddl_method_candidate(#{
        method_id => <<"sample_method_id">>,
        compound_task_id => <<"sample_compound_task_id">>,
        subnetwork_digest => <<"sample_subnetwork_digest">>
    });
sample(hddl_method_candidate, minimal) ->
    beam4pm_types:new_hddl_method_candidate(#{
        method_id => <<"sample_method_id">>,
        compound_task_id => <<"sample_compound_task_id">>,
        subnetwork_digest => <<"sample_subnetwork_digest">>
    });
sample(hddl_task_network, full) ->
    beam4pm_types:new_hddl_task_network(#{
        network_id => <<"sample_network_id">>,
        task_set_digest => <<"sample_task_set_digest">>,
        ordering_digest => <<"sample_ordering_digest">>
    });
sample(hddl_task_network, minimal) ->
    beam4pm_types:new_hddl_task_network(#{
        network_id => <<"sample_network_id">>,
        task_set_digest => <<"sample_task_set_digest">>,
        ordering_digest => <<"sample_ordering_digest">>
    });
sample(heuristic_arc, full) ->
    beam4pm_types:new_heuristic_arc(#{
        source_activity => <<"sample_source_activity">>,
        target_activity => <<"sample_target_activity">>,
        dependency_measure => 3.5
    });
sample(heuristic_arc, minimal) ->
    beam4pm_types:new_heuristic_arc(#{
        source_activity => <<"sample_source_activity">>,
        target_activity => <<"sample_target_activity">>,
        dependency_measure => 3.5
    });
sample(historical_episode_replay, full) ->
    beam4pm_types:new_historical_episode_replay(#{
        replay_id => <<"sample_replay_id">>,
        episode_id => <<"sample_episode_id">>,
        historical_subject_sha => <<"sample_historical_subject_sha">>,
        replay_seed => <<"sample_replay_seed">>,
        divergence_code => <<"sample_divergence_code">>
    });
sample(historical_episode_replay, minimal) ->
    beam4pm_types:new_historical_episode_replay(#{
        replay_id => <<"sample_replay_id">>,
        episode_id => <<"sample_episode_id">>,
        historical_subject_sha => <<"sample_historical_subject_sha">>,
        replay_seed => <<"sample_replay_seed">>,
        divergence_code => <<"sample_divergence_code">>
    });
sample(hypothesis_priority_update, full) ->
    beam4pm_types:new_hypothesis_priority_update(#{
        update_id => <<"sample_update_id">>,
        hypothesis_id => <<"sample_hypothesis_id">>,
        prior_priority => 3.5,
        new_priority => 3.5,
        payoff_evidence_digest => <<"sample_payoff_evidence_digest">>
    });
sample(hypothesis_priority_update, minimal) ->
    beam4pm_types:new_hypothesis_priority_update(#{
        update_id => <<"sample_update_id">>,
        hypothesis_id => <<"sample_hypothesis_id">>,
        prior_priority => 3.5,
        new_priority => 3.5,
        payoff_evidence_digest => <<"sample_payoff_evidence_digest">>
    });
sample(immutable_pack_selection, full) ->
    beam4pm_types:new_immutable_pack_selection(#{
        pack_id => <<"sample_pack_id">>,
        pack_sha => <<"sample_pack_sha">>,
        selection_hash => <<"sample_selection_hash">>
    });
sample(immutable_pack_selection, minimal) ->
    beam4pm_types:new_immutable_pack_selection(#{
        pack_id => <<"sample_pack_id">>,
        pack_sha => <<"sample_pack_sha">>,
        selection_hash => <<"sample_selection_hash">>
    });
sample(implementation_fee_admission, full) ->
    beam4pm_types:new_implementation_fee_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        fee_id => <<"sample_fee_id">>,
        decision => <<"sample_decision">>
    });
sample(implementation_fee_admission, minimal) ->
    beam4pm_types:new_implementation_fee_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        fee_id => <<"sample_fee_id">>,
        decision => <<"sample_decision">>
    });
sample(incident_acknowledgement, full) ->
    beam4pm_types:new_incident_acknowledgement(#{
        tenant_id => <<"sample_tenant_id">>,
        incident_id => <<"sample_incident_id">>,
        acknowledgement_hash => <<"sample_acknowledgement_hash">>
    });
sample(incident_acknowledgement, minimal) ->
    beam4pm_types:new_incident_acknowledgement(#{
        tenant_id => <<"sample_tenant_id">>,
        incident_id => <<"sample_incident_id">>,
        acknowledgement_hash => <<"sample_acknowledgement_hash">>
    });
sample(incident_detection_event, full) ->
    beam4pm_types:new_incident_detection_event(#{
        tenant_id => <<"sample_tenant_id">>,
        incident_id => <<"sample_incident_id">>,
        detection_hash => <<"sample_detection_hash">>
    });
sample(incident_detection_event, minimal) ->
    beam4pm_types:new_incident_detection_event(#{
        tenant_id => <<"sample_tenant_id">>,
        incident_id => <<"sample_incident_id">>,
        detection_hash => <<"sample_detection_hash">>
    });
sample(incident_response_evidence, full) ->
    beam4pm_types:new_incident_response_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        incident_id => <<"sample_incident_id">>,
        observed_result => sample_atom
    });
sample(incident_response_evidence, minimal) ->
    beam4pm_types:new_incident_response_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        incident_id => <<"sample_incident_id">>,
        observed_result => sample_atom
    });
sample(indemnity_scope_admission, full) ->
    beam4pm_types:new_indemnity_scope_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        indemnity_scope_id => <<"sample_indemnity_scope_id">>,
        decision => <<"sample_decision">>
    });
sample(indemnity_scope_admission, minimal) ->
    beam4pm_types:new_indemnity_scope_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        indemnity_scope_id => <<"sample_indemnity_scope_id">>,
        decision => <<"sample_decision">>
    });
sample(information_partition_observation, full) ->
    beam4pm_types:new_information_partition_observation(#{
        partition_id => <<"sample_partition_id">>,
        state_vector_id => <<"sample_state_vector_id">>,
        partition_key => <<"sample_partition_key">>,
        information_digest => <<"sample_information_digest">>
    });
sample(information_partition_observation, minimal) ->
    beam4pm_types:new_information_partition_observation(#{
        partition_id => <<"sample_partition_id">>,
        state_vector_id => <<"sample_state_vector_id">>,
        partition_key => <<"sample_partition_key">>,
        information_digest => <<"sample_information_digest">>
    });
sample(initial_state_digest, full) ->
    beam4pm_types:new_initial_state_digest(#{
        problem_id => <<"sample_problem_id">>,
        fact_digest => <<"sample_fact_digest">>,
        normalizer_version => <<"sample_normalizer_version">>
    });
sample(initial_state_digest, minimal) ->
    beam4pm_types:new_initial_state_digest(#{
        problem_id => <<"sample_problem_id">>,
        fact_digest => <<"sample_fact_digest">>,
        normalizer_version => <<"sample_normalizer_version">>
    });
sample(insurance_requirement, full) ->
    beam4pm_types:new_insurance_requirement(#{
        opportunity_id => <<"sample_opportunity_id">>,
        coverage_id => <<"sample_coverage_id">>,
        evidence_hash => <<"sample_evidence_hash">>
    });
sample(insurance_requirement, minimal) ->
    beam4pm_types:new_insurance_requirement(#{
        opportunity_id => <<"sample_opportunity_id">>,
        coverage_id => <<"sample_coverage_id">>,
        evidence_hash => <<"sample_evidence_hash">>
    });
sample(integration_readiness, full) ->
    beam4pm_types:new_integration_readiness(#{
        integration_readiness_id => <<"sample_integration_readiness_id">>,
        account_id => <<"sample_account_id">>,
        integration_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(integration_readiness, minimal) ->
    beam4pm_types:new_integration_readiness(#{
        integration_readiness_id => <<"sample_integration_readiness_id">>,
        account_id => <<"sample_account_id">>,
        integration_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(integration_scope_admission, full) ->
    beam4pm_types:new_integration_scope_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        integration_scope_id => <<"sample_integration_scope_id">>,
        decision => <<"sample_decision">>
    });
sample(integration_scope_admission, minimal) ->
    beam4pm_types:new_integration_scope_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        integration_scope_id => <<"sample_integration_scope_id">>,
        decision => <<"sample_decision">>
    });
sample(invoice_entity_identity, full) ->
    beam4pm_types:new_invoice_entity_identity(#{
        opportunity_id => <<"sample_opportunity_id">>,
        invoice_entity_id => <<"sample_invoice_entity_id">>,
        identity_evidence_hash => <<"sample_identity_evidence_hash">>
    });
sample(invoice_entity_identity, minimal) ->
    beam4pm_types:new_invoice_entity_identity(#{
        opportunity_id => <<"sample_opportunity_id">>,
        invoice_entity_id => <<"sample_invoice_entity_id">>,
        identity_evidence_hash => <<"sample_identity_evidence_hash">>
    });
sample(invoice_line_item, full) ->
    beam4pm_types:new_invoice_line_item(#{
        invoice_line_item_id => <<"sample_invoice_line_item_id">>,
        account_id => <<"sample_account_id">>,
        line_amount => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(invoice_line_item, minimal) ->
    beam4pm_types:new_invoice_line_item(#{
        invoice_line_item_id => <<"sample_invoice_line_item_id">>,
        account_id => <<"sample_account_id">>,
        line_amount => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(invoice_schedule, full) ->
    beam4pm_types:new_invoice_schedule(#{
        schedule_id => <<"sample_schedule_id">>,
        billing_account_id => <<"sample_billing_account_id">>,
        cadence => sample_atom,
        next_invoice_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(invoice_schedule, minimal) ->
    beam4pm_types:new_invoice_schedule(#{
        schedule_id => <<"sample_schedule_id">>,
        billing_account_id => <<"sample_billing_account_id">>,
        cadence => sample_atom,
        next_invoice_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(irreversibility_budget, full) ->
    beam4pm_types:new_irreversibility_budget(#{
        episode_id => <<"sample_episode_id">>,
        budget => <<"sample_budget">>,
        consumed => <<"sample_consumed">>
    });
sample(irreversibility_budget, minimal) ->
    beam4pm_types:new_irreversibility_budget(#{
        episode_id => <<"sample_episode_id">>,
        budget => <<"sample_budget">>,
        consumed => <<"sample_consumed">>
    });
sample(k8s_object_ref, full) ->
    beam4pm_types:new_k8s_object_ref(#{
        kind => <<"sample_kind">>,
        name => <<"sample_name">>,
        namespace => <<"sample_namespace">>
    });
sample(k8s_object_ref, minimal) ->
    beam4pm_types:new_k8s_object_ref(#{
        kind => <<"sample_kind">>,
        name => <<"sample_name">>
    });
sample(late_arriving_usage, full) ->
    beam4pm_types:new_late_arriving_usage(#{
        late_arriving_usage_id => <<"sample_late_arriving_usage_id">>,
        account_id => <<"sample_account_id">>,
        occurred_at => <<"2026-08-29T12:00:00.123456Z">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(late_arriving_usage, minimal) ->
    beam4pm_types:new_late_arriving_usage(#{
        late_arriving_usage_id => <<"sample_late_arriving_usage_id">>,
        account_id => <<"sample_account_id">>,
        occurred_at => <<"2026-08-29T12:00:00.123456Z">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(latency_budget_observation, full) ->
    beam4pm_types:new_latency_budget_observation(#{
        tenant_id => <<"sample_tenant_id">>,
        workload_id => <<"sample_workload_id">>,
        observation_hash => <<"sample_observation_hash">>
    });
sample(latency_budget_observation, minimal) ->
    beam4pm_types:new_latency_budget_observation(#{
        tenant_id => <<"sample_tenant_id">>,
        workload_id => <<"sample_workload_id">>,
        observation_hash => <<"sample_observation_hash">>
    });
sample(leakage_finding, full) ->
    beam4pm_types:new_leakage_finding(#{
        case_id => <<"sample_case_id">>,
        fitness => 3.5,
        precision => 3.5,
        amount_at_risk => 3.5
    });
sample(leakage_finding, minimal) ->
    beam4pm_types:new_leakage_finding(#{
        case_id => <<"sample_case_id">>,
        fitness => 3.5,
        precision => 3.5
    });
sample(learning_episode, full) ->
    beam4pm_types:new_learning_episode(#{
        episode_id => <<"sample_episode_id">>,
        exact_subject_sha => <<"sample_exact_subject_sha">>,
        evidence_digest => <<"sample_evidence_digest">>,
        outcome => <<"sample_outcome">>,
        authority_ceiling => <<"sample_authority_ceiling">>
    });
sample(learning_episode, minimal) ->
    beam4pm_types:new_learning_episode(#{
        episode_id => <<"sample_episode_id">>,
        exact_subject_sha => <<"sample_exact_subject_sha">>,
        evidence_digest => <<"sample_evidence_digest">>,
        outcome => <<"sample_outcome">>,
        authority_ceiling => <<"sample_authority_ceiling">>
    });
sample(lease_expiry_replan, full) ->
    beam4pm_types:new_lease_expiry_replan(#{
        replan_id => <<"sample_replan_id">>,
        expired_lease_id => <<"sample_expired_lease_id">>,
        returned_frontier_digest => <<"sample_returned_frontier_digest">>
    });
sample(lease_expiry_replan, minimal) ->
    beam4pm_types:new_lease_expiry_replan(#{
        replan_id => <<"sample_replan_id">>,
        expired_lease_id => <<"sample_expired_lease_id">>,
        returned_frontier_digest => <<"sample_returned_frontier_digest">>
    });
sample(least_authority_evidence, full) ->
    beam4pm_types:new_least_authority_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        granted_permissions => [<<"alpha">>, <<"beta">>],
        observed_result => sample_atom
    });
sample(least_authority_evidence, minimal) ->
    beam4pm_types:new_least_authority_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        granted_permissions => [<<"alpha">>, <<"beta">>],
        observed_result => sample_atom
    });
sample(legal_blocker, full) ->
    beam4pm_types:new_legal_blocker(#{
        opportunity_id => <<"sample_opportunity_id">>,
        blocker_id => <<"sample_blocker_id">>,
        refusal_code => <<"sample_refusal_code">>
    });
sample(legal_blocker, minimal) ->
    beam4pm_types:new_legal_blocker(#{
        opportunity_id => <<"sample_opportunity_id">>,
        blocker_id => <<"sample_blocker_id">>,
        refusal_code => <<"sample_refusal_code">>
    });
sample(liability_cap_admission, full) ->
    beam4pm_types:new_liability_cap_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        liability_cap_id => <<"sample_liability_cap_id">>,
        decision => <<"sample_decision">>
    });
sample(liability_cap_admission, minimal) ->
    beam4pm_types:new_liability_cap_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        liability_cap_id => <<"sample_liability_cap_id">>,
        decision => <<"sample_decision">>
    });
sample(license_evidence, full) ->
    beam4pm_types:new_license_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        license_expression => <<"sample_license_expression">>,
        observed_result => sample_atom
    });
sample(license_evidence, minimal) ->
    beam4pm_types:new_license_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        license_expression => <<"sample_license_expression">>,
        observed_result => sample_atom
    });
sample(log_trace, full) ->
    beam4pm_types:new_log_trace(#{
        case_id => <<"sample_case_id">>,
        activity_sequence => [<<"alpha">>, <<"beta">>]
    });
sample(log_trace, minimal) ->
    beam4pm_types:new_log_trace(#{
        case_id => <<"sample_case_id">>,
        activity_sequence => [<<"alpha">>, <<"beta">>]
    });
sample(machine_actionable_delta, full) ->
    beam4pm_types:new_machine_actionable_delta(#{
        subject_id => <<"sample_subject_id">>,
        prior_state_digest => <<"sample_prior_state_digest">>,
        delta_digest => <<"sample_delta_digest">>,
        recommended_action => <<"sample_recommended_action">>
    });
sample(machine_actionable_delta, minimal) ->
    beam4pm_types:new_machine_actionable_delta(#{
        subject_id => <<"sample_subject_id">>,
        prior_state_digest => <<"sample_prior_state_digest">>,
        delta_digest => <<"sample_delta_digest">>,
        recommended_action => <<"sample_recommended_action">>
    });
sample(manufacture_receipt_presence_observation, full) ->
    beam4pm_types:new_manufacture_receipt_presence_observation(#{
        subject_sha => <<"sample_subject_sha">>,
        receipt_id => <<"sample_receipt_id">>,
        receipt_digest => <<"sample_receipt_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(manufacture_receipt_presence_observation, minimal) ->
    beam4pm_types:new_manufacture_receipt_presence_observation(#{
        subject_sha => <<"sample_subject_sha">>,
        receipt_id => <<"sample_receipt_id">>,
        receipt_digest => <<"sample_receipt_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(manufacture_receipt_validity_observation, full) ->
    beam4pm_types:new_manufacture_receipt_validity_observation(#{
        receipt_digest => <<"sample_receipt_digest">>,
        subject_sha => <<"sample_subject_sha">>,
        verification_status => <<"sample_verification_status">>,
        verifier_identity => <<"sample_verifier_identity">>
    });
sample(manufacture_receipt_validity_observation, minimal) ->
    beam4pm_types:new_manufacture_receipt_validity_observation(#{
        receipt_digest => <<"sample_receipt_digest">>,
        subject_sha => <<"sample_subject_sha">>,
        verification_status => <<"sample_verification_status">>,
        verifier_identity => <<"sample_verifier_identity">>
    });
sample(master_service_agreement_binding, full) ->
    beam4pm_types:new_master_service_agreement_binding(#{
        master_service_agreement_binding_id => <<"sample_master_service_agreement_binding_id">>,
        account_id => <<"sample_account_id">>,
        msa_digest => <<"sample_msa_digest">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(master_service_agreement_binding, minimal) ->
    beam4pm_types:new_master_service_agreement_binding(#{
        master_service_agreement_binding_id => <<"sample_master_service_agreement_binding_id">>,
        account_id => <<"sample_account_id">>,
        msa_digest => <<"sample_msa_digest">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(master_service_agreement_state, full) ->
    beam4pm_types:new_master_service_agreement_state(#{
        opportunity_id => <<"sample_opportunity_id">>,
        agreement_id => <<"sample_agreement_id">>,
        agreement_state => <<"sample_agreement_state">>
    });
sample(master_service_agreement_state, minimal) ->
    beam4pm_types:new_master_service_agreement_state(#{
        opportunity_id => <<"sample_opportunity_id">>,
        agreement_id => <<"sample_agreement_id">>,
        agreement_state => <<"sample_agreement_state">>
    });
sample(mcts_plan_candidate, full) ->
    beam4pm_types:new_mcts_plan_candidate(#{
        plan_id => <<"sample_plan_id">>,
        seed => <<"sample_seed">>,
        rollout_hash => <<"sample_rollout_hash">>
    });
sample(mcts_plan_candidate, minimal) ->
    beam4pm_types:new_mcts_plan_candidate(#{
        plan_id => <<"sample_plan_id">>,
        seed => <<"sample_seed">>,
        rollout_hash => <<"sample_rollout_hash">>
    });
sample(meta_router, full) ->
    beam4pm_types:new_meta_router(#{
        portfolio_id => <<"sample_portfolio_id">>,
        observation_hash => <<"sample_observation_hash">>,
        selected_planner_id => <<"sample_selected_planner_id">>
    });
sample(meta_router, minimal) ->
    beam4pm_types:new_meta_router(#{
        portfolio_id => <<"sample_portfolio_id">>,
        observation_hash => <<"sample_observation_hash">>,
        selected_planner_id => <<"sample_selected_planner_id">>
    });
sample(meter_definition, full) ->
    beam4pm_types:new_meter_definition(#{
        meter_definition_id => <<"sample_meter_definition_id">>,
        account_id => <<"sample_account_id">>,
        meter_name => <<"sample_meter_name">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(meter_definition, minimal) ->
    beam4pm_types:new_meter_definition(#{
        meter_definition_id => <<"sample_meter_definition_id">>,
        account_id => <<"sample_account_id">>,
        meter_name => <<"sample_meter_name">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(meter_dimension, full) ->
    beam4pm_types:new_meter_dimension(#{
        meter_dimension_id => <<"sample_meter_dimension_id">>,
        account_id => <<"sample_account_id">>,
        dimension_name => <<"sample_dimension_name">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(meter_dimension, minimal) ->
    beam4pm_types:new_meter_dimension(#{
        meter_dimension_id => <<"sample_meter_dimension_id">>,
        account_id => <<"sample_account_id">>,
        dimension_name => <<"sample_dimension_name">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(meter_rollup, full) ->
    beam4pm_types:new_meter_rollup(#{
        meter_rollup_id => <<"sample_meter_rollup_id">>,
        account_id => <<"sample_account_id">>,
        rollup_function => <<"sample_rollup_function">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(meter_rollup, minimal) ->
    beam4pm_types:new_meter_rollup(#{
        meter_rollup_id => <<"sample_meter_rollup_id">>,
        account_id => <<"sample_account_id">>,
        rollup_function => <<"sample_rollup_function">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(metered_usage_sample, full) ->
    beam4pm_types:new_metered_usage_sample(#{
        tenant_id => <<"sample_tenant_id">>,
        usage_sample_id => <<"sample_usage_sample_id">>,
        sample_hash => <<"sample_sample_hash">>
    });
sample(metered_usage_sample, minimal) ->
    beam4pm_types:new_metered_usage_sample(#{
        tenant_id => <<"sample_tenant_id">>,
        usage_sample_id => <<"sample_usage_sample_id">>,
        sample_hash => <<"sample_sample_hash">>
    });
sample(method_precondition_gate, full) ->
    beam4pm_types:new_method_precondition_gate(#{
        method_id => <<"sample_method_id">>,
        state_digest => <<"sample_state_digest">>,
        decision => <<"sample_decision">>
    });
sample(method_precondition_gate, minimal) ->
    beam4pm_types:new_method_precondition_gate(#{
        method_id => <<"sample_method_id">>,
        state_digest => <<"sample_state_digest">>,
        decision => <<"sample_decision">>
    });
sample(migration_contract, full) ->
    beam4pm_types:new_migration_contract(#{
        migration_id => <<"sample_migration_id">>,
        from_version => <<"sample_from_version">>,
        to_version => <<"sample_to_version">>,
        rollback_plan => <<"sample_rollback_plan">>
    });
sample(migration_contract, minimal) ->
    beam4pm_types:new_migration_contract(#{
        migration_id => <<"sample_migration_id">>,
        from_version => <<"sample_from_version">>,
        to_version => <<"sample_to_version">>,
        rollback_plan => <<"sample_rollback_plan">>
    });
sample(migration_readiness, full) ->
    beam4pm_types:new_migration_readiness(#{
        migration_readiness_id => <<"sample_migration_readiness_id">>,
        account_id => <<"sample_account_id">>,
        migration_effort_days => 42,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(migration_readiness, minimal) ->
    beam4pm_types:new_migration_readiness(#{
        migration_readiness_id => <<"sample_migration_readiness_id">>,
        account_id => <<"sample_account_id">>,
        migration_effort_days => 42,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(minimum_commitment_schedule, full) ->
    beam4pm_types:new_minimum_commitment_schedule(#{
        minimum_commitment_schedule_id => <<"sample_minimum_commitment_schedule_id">>,
        account_id => <<"sample_account_id">>,
        committed_amount => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(minimum_commitment_schedule, minimal) ->
    beam4pm_types:new_minimum_commitment_schedule(#{
        minimum_commitment_schedule_id => <<"sample_minimum_commitment_schedule_id">>,
        account_id => <<"sample_account_id">>,
        committed_amount => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(minimum_term_admission, full) ->
    beam4pm_types:new_minimum_term_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        minimum_term => <<"sample_minimum_term">>,
        decision => <<"sample_decision">>
    });
sample(minimum_term_admission, minimal) ->
    beam4pm_types:new_minimum_term_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        minimum_term => <<"sample_minimum_term">>,
        decision => <<"sample_decision">>
    });
sample(multiarch_evidence, full) ->
    beam4pm_types:new_multiarch_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        platforms => [<<"alpha">>, <<"beta">>],
        observed_result => sample_atom
    });
sample(multiarch_evidence, minimal) ->
    beam4pm_types:new_multiarch_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        platforms => [<<"alpha">>, <<"beta">>],
        observed_result => sample_atom
    });
sample(mutable_identity_refusal_evidence, full) ->
    beam4pm_types:new_mutable_identity_refusal_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        mutable_reference => <<"sample_mutable_reference">>,
        observed_result => sample_atom
    });
sample(mutable_identity_refusal_evidence, minimal) ->
    beam4pm_types:new_mutable_identity_refusal_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        mutable_reference => <<"sample_mutable_reference">>,
        observed_result => sample_atom
    });
sample(mutual_information_score, full) ->
    beam4pm_types:new_mutual_information_score(#{
        observation_id => <<"sample_observation_id">>,
        target_state_id => <<"sample_target_state_id">>,
        score => <<"sample_score">>
    });
sample(mutual_information_score, minimal) ->
    beam4pm_types:new_mutual_information_score(#{
        observation_id => <<"sample_observation_id">>,
        target_state_id => <<"sample_target_state_id">>,
        score => <<"sample_score">>
    });
sample(negative_fixture_generation, full) ->
    beam4pm_types:new_negative_fixture_generation(#{
        fixture_id => <<"sample_fixture_id">>,
        source_failure_id => <<"sample_source_failure_id">>,
        invariant_id => <<"sample_invariant_id">>,
        generated_input_digest => <<"sample_generated_input_digest">>,
        expected_refusal_code => <<"sample_expected_refusal_code">>
    });
sample(negative_fixture_generation, minimal) ->
    beam4pm_types:new_negative_fixture_generation(#{
        fixture_id => <<"sample_fixture_id">>,
        source_failure_id => <<"sample_source_failure_id">>,
        invariant_id => <<"sample_invariant_id">>,
        generated_input_digest => <<"sample_generated_input_digest">>,
        expected_refusal_code => <<"sample_expected_refusal_code">>
    });
sample(next_event_prediction_contract, full) ->
    beam4pm_types:new_next_event_prediction_contract(#{
        prediction_id => <<"sample_prediction_id">>,
        prefix_digest => <<"sample_prefix_digest">>,
        distribution_digest => <<"sample_distribution_digest">>
    });
sample(next_event_prediction_contract, minimal) ->
    beam4pm_types:new_next_event_prediction_contract(#{
        prediction_id => <<"sample_prediction_id">>,
        prefix_digest => <<"sample_prefix_digest">>,
        distribution_digest => <<"sample_distribution_digest">>
    });
sample(next_lawful_actuation, full) ->
    beam4pm_types:new_next_lawful_actuation(#{
        episode_id => <<"sample_episode_id">>,
        selected_action_id => <<"sample_selected_action_id">>,
        selection_receipt_hash => <<"sample_selection_receipt_hash">>
    });
sample(next_lawful_actuation, minimal) ->
    beam4pm_types:new_next_lawful_actuation(#{
        episode_id => <<"sample_episode_id">>,
        selected_action_id => <<"sample_selected_action_id">>,
        selection_receipt_hash => <<"sample_selection_receipt_hash">>
    });
sample(no_authority_learning_guard, full) ->
    beam4pm_types:new_no_authority_learning_guard(#{
        guard_id => <<"sample_guard_id">>,
        candidate_update_id => <<"sample_candidate_update_id">>,
        current_authority => <<"sample_current_authority">>,
        requested_authority => <<"sample_requested_authority">>,
        decision => <<"sample_decision">>,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(no_authority_learning_guard, minimal) ->
    beam4pm_types:new_no_authority_learning_guard(#{
        guard_id => <<"sample_guard_id">>,
        candidate_update_id => <<"sample_candidate_update_id">>,
        current_authority => <<"sample_current_authority">>,
        requested_authority => <<"sample_requested_authority">>,
        decision => <<"sample_decision">>,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(node_failover_event, full) ->
    beam4pm_types:new_node_failover_event(#{
        tenant_id => <<"sample_tenant_id">>,
        node_id => <<"sample_node_id">>,
        failover_hash => <<"sample_failover_hash">>
    });
sample(node_failover_event, minimal) ->
    beam4pm_types:new_node_failover_event(#{
        tenant_id => <<"sample_tenant_id">>,
        node_id => <<"sample_node_id">>,
        failover_hash => <<"sample_failover_hash">>
    });
sample(nondeterministic_effect_contract, full) ->
    beam4pm_types:new_nondeterministic_effect_contract(#{
        action_id => <<"sample_action_id">>,
        effect_set_digest => <<"sample_effect_set_digest">>,
        selection_semantics => <<"sample_selection_semantics">>
    });
sample(nondeterministic_effect_contract, minimal) ->
    beam4pm_types:new_nondeterministic_effect_contract(#{
        action_id => <<"sample_action_id">>,
        effect_set_digest => <<"sample_effect_set_digest">>,
        selection_semantics => <<"sample_selection_semantics">>
    });
sample(nonproduction_discount_policy, full) ->
    beam4pm_types:new_nonproduction_discount_policy(#{
        nonproduction_discount_policy_id => <<"sample_nonproduction_discount_policy_id">>,
        account_id => <<"sample_account_id">>,
        discount_percent => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(nonproduction_discount_policy, minimal) ->
    beam4pm_types:new_nonproduction_discount_policy(#{
        nonproduction_discount_policy_id => <<"sample_nonproduction_discount_policy_id">>,
        account_id => <<"sample_account_id">>,
        discount_percent => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(normalized_event_observation, full) ->
    beam4pm_types:new_normalized_event_observation(#{
        source_system => <<"sample_source_system">>,
        event_id => <<"sample_event_id">>,
        event_type => <<"sample_event_type">>,
        event_time => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(normalized_event_observation, minimal) ->
    beam4pm_types:new_normalized_event_observation(#{
        source_system => <<"sample_source_system">>,
        event_id => <<"sample_event_id">>,
        event_type => <<"sample_event_type">>,
        event_time => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(novelty_reward, full) ->
    beam4pm_types:new_novelty_reward(#{
        reward_id => <<"sample_reward_id">>,
        hypothesis_id => <<"sample_hypothesis_id">>,
        feature_vector_digest => <<"sample_feature_vector_digest">>,
        nearest_prior_distance => 3.5,
        reward => 3.5,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(novelty_reward, minimal) ->
    beam4pm_types:new_novelty_reward(#{
        reward_id => <<"sample_reward_id">>,
        hypothesis_id => <<"sample_hypothesis_id">>,
        feature_vector_digest => <<"sample_feature_vector_digest">>,
        nearest_prior_distance => 3.5,
        reward => 3.5,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(novelty_score, full) ->
    beam4pm_types:new_novelty_score(#{
        option_id => <<"sample_option_id">>,
        reference_set_hash => <<"sample_reference_set_hash">>,
        score => <<"sample_score">>
    });
sample(novelty_score, minimal) ->
    beam4pm_types:new_novelty_score(#{
        option_id => <<"sample_option_id">>,
        reference_set_hash => <<"sample_reference_set_hash">>,
        score => <<"sample_score">>
    });
sample(object_attribute_change, full) ->
    beam4pm_types:new_object_attribute_change(#{
        object_id => <<"sample_object_id">>,
        attribute_name => <<"sample_attribute_name">>,
        old_value => <<"sample_old_value">>,
        new_value => <<"sample_new_value">>,
        changed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(object_attribute_change, minimal) ->
    beam4pm_types:new_object_attribute_change(#{
        object_id => <<"sample_object_id">>,
        attribute_name => <<"sample_attribute_name">>,
        new_value => <<"sample_new_value">>,
        changed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(object_type, full) ->
    beam4pm_types:new_object_type(#{
        type_name => <<"sample_type_name">>,
        attribute_names => [<<"alpha">>, <<"beta">>]
    });
sample(object_type, minimal) ->
    beam4pm_types:new_object_type(#{
        type_name => <<"sample_type_name">>
    });
sample(object_volume_pricing_policy, full) ->
    beam4pm_types:new_object_volume_pricing_policy(#{
        object_volume_pricing_policy_id => <<"sample_object_volume_pricing_policy_id">>,
        account_id => <<"sample_account_id">>,
        unit_object_price => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(object_volume_pricing_policy, minimal) ->
    beam4pm_types:new_object_volume_pricing_policy(#{
        object_volume_pricing_policy_id => <<"sample_object_volume_pricing_policy_id">>,
        account_id => <<"sample_account_id">>,
        unit_object_price => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(objection, full) ->
    beam4pm_types:new_objection(#{
        objection_id => <<"sample_objection_id">>,
        account_id => <<"sample_account_id">>,
        objection_type => <<"sample_objection_type">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(objection, minimal) ->
    beam4pm_types:new_objection(#{
        objection_id => <<"sample_objection_id">>,
        account_id => <<"sample_account_id">>,
        objection_type => <<"sample_objection_type">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(objection_resolution, full) ->
    beam4pm_types:new_objection_resolution(#{
        objection_resolution_id => <<"sample_objection_resolution_id">>,
        account_id => <<"sample_account_id">>,
        resolution_status => <<"sample_resolution_status">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(objection_resolution, minimal) ->
    beam4pm_types:new_objection_resolution(#{
        objection_resolution_id => <<"sample_objection_resolution_id">>,
        account_id => <<"sample_account_id">>,
        resolution_status => <<"sample_resolution_status">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(observation_deduplication_decision, full) ->
    beam4pm_types:new_observation_deduplication_decision(#{
        event_id => <<"sample_event_id">>,
        event_digest => <<"sample_event_digest">>,
        dedup_key => <<"sample_dedup_key">>,
        decision => <<"sample_decision">>
    });
sample(observation_deduplication_decision, minimal) ->
    beam4pm_types:new_observation_deduplication_decision(#{
        event_id => <<"sample_event_id">>,
        event_digest => <<"sample_event_digest">>,
        dedup_key => <<"sample_dedup_key">>,
        decision => <<"sample_decision">>
    });
sample(observation_entropy_estimate, full) ->
    beam4pm_types:new_observation_entropy_estimate(#{
        state_vector_id => <<"sample_state_vector_id">>,
        entropy_method => <<"sample_entropy_method">>,
        entropy_value => 3.5,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(observation_entropy_estimate, minimal) ->
    beam4pm_types:new_observation_entropy_estimate(#{
        state_vector_id => <<"sample_state_vector_id">>,
        entropy_method => <<"sample_entropy_method">>,
        entropy_value => 3.5,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(observation_freshness_assessment, full) ->
    beam4pm_types:new_observation_freshness_assessment(#{
        observation_id => <<"sample_observation_id">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>,
        freshness_deadline => <<"2026-08-29T12:00:00.123456Z">>,
        freshness_status => <<"sample_freshness_status">>
    });
sample(observation_freshness_assessment, minimal) ->
    beam4pm_types:new_observation_freshness_assessment(#{
        observation_id => <<"sample_observation_id">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>,
        freshness_deadline => <<"2026-08-29T12:00:00.123456Z">>,
        freshness_status => <<"sample_freshness_status">>
    });
sample(observation_partition, full) ->
    beam4pm_types:new_observation_partition(#{
        partition_id => <<"sample_partition_id">>,
        state_set_digest => <<"sample_state_set_digest">>,
        observation_key => <<"sample_observation_key">>
    });
sample(observation_partition, minimal) ->
    beam4pm_types:new_observation_partition(#{
        partition_id => <<"sample_partition_id">>,
        state_set_digest => <<"sample_state_set_digest">>,
        observation_key => <<"sample_observation_key">>
    });
sample(observation_projection_update, full) ->
    beam4pm_types:new_observation_projection_update(#{
        update_id => <<"sample_update_id">>,
        projection_id => <<"sample_projection_id">>,
        prior_digest => <<"sample_prior_digest">>,
        new_digest => <<"sample_new_digest">>,
        triggering_gap_id => <<"sample_triggering_gap_id">>,
        authority_ceiling => <<"sample_authority_ceiling">>
    });
sample(observation_projection_update, minimal) ->
    beam4pm_types:new_observation_projection_update(#{
        update_id => <<"sample_update_id">>,
        projection_id => <<"sample_projection_id">>,
        prior_digest => <<"sample_prior_digest">>,
        new_digest => <<"sample_new_digest">>,
        triggering_gap_id => <<"sample_triggering_gap_id">>,
        authority_ceiling => <<"sample_authority_ceiling">>
    });
sample(observation_staleness_invalidation, full) ->
    beam4pm_types:new_observation_staleness_invalidation(#{
        observation_id => <<"sample_observation_id">>,
        invalidated_at => <<"2026-08-29T12:00:00.123456Z">>,
        staleness_reason => <<"sample_staleness_reason">>,
        replacement_required => true
    });
sample(observation_staleness_invalidation, minimal) ->
    beam4pm_types:new_observation_staleness_invalidation(#{
        observation_id => <<"sample_observation_id">>,
        invalidated_at => <<"2026-08-29T12:00:00.123456Z">>,
        staleness_reason => <<"sample_staleness_reason">>,
        replacement_required => true
    });
sample(oc_declare_constraint, full) ->
    beam4pm_types:new_oc_declare_constraint(#{
        constraint_id => <<"sample_constraint_id">>,
        source_activity => <<"sample_source_activity">>,
        target_activity => <<"sample_target_activity">>,
        constraint_type => sample_atom
    });
sample(oc_declare_constraint, minimal) ->
    beam4pm_types:new_oc_declare_constraint(#{
        constraint_id => <<"sample_constraint_id">>,
        source_activity => <<"sample_source_activity">>,
        target_activity => <<"sample_target_activity">>,
        constraint_type => sample_atom
    });
sample(ocel_attribute, full) ->
    beam4pm_types:new_ocel_attribute(#{
        attribute_name => <<"sample_attribute_name">>,
        attribute_value => <<"sample_attribute_value">>,
        recorded_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(ocel_attribute, minimal) ->
    beam4pm_types:new_ocel_attribute(#{
        attribute_name => <<"sample_attribute_name">>,
        attribute_value => <<"sample_attribute_value">>
    });
sample(ocel_event, full) ->
    beam4pm_types:new_ocel_event(#{
        event_id => <<"sample_event_id">>,
        event_type => <<"sample_event_type">>,
        event_time => <<"2026-08-29T12:00:00.123456Z">>,
        attributes => #{<<"k">> => <<"v">>}
    });
sample(ocel_event, minimal) ->
    beam4pm_types:new_ocel_event(#{
        event_id => <<"sample_event_id">>,
        event_type => <<"sample_event_type">>,
        event_time => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(ocel_object, full) ->
    beam4pm_types:new_ocel_object(#{
        object_id => <<"sample_object_id">>,
        object_type => <<"sample_object_type">>,
        attributes => #{<<"k">> => <<"v">>}
    });
sample(ocel_object, minimal) ->
    beam4pm_types:new_ocel_object(#{
        object_id => <<"sample_object_id">>,
        object_type => <<"sample_object_type">>
    });
sample(ocel_planning_event, full) ->
    beam4pm_types:new_ocel_planning_event(#{
        event_id => <<"sample_event_id">>,
        activity => <<"sample_activity">>,
        object_binding_digest => <<"sample_object_binding_digest">>
    });
sample(ocel_planning_event, minimal) ->
    beam4pm_types:new_ocel_planning_event(#{
        event_id => <<"sample_event_id">>,
        activity => <<"sample_activity">>,
        object_binding_digest => <<"sample_object_binding_digest">>
    });
sample(ocel_relationship, full) ->
    beam4pm_types:new_ocel_relationship(#{
        qualifier => <<"sample_qualifier">>,
        object_id => <<"sample_object_id">>
    });
sample(ocel_relationship, minimal) ->
    beam4pm_types:new_ocel_relationship(#{
        qualifier => <<"sample_qualifier">>,
        object_id => <<"sample_object_id">>
    });
sample(oci_manifest_evidence, full) ->
    beam4pm_types:new_oci_manifest_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        manifest_digest => <<"sample_manifest_digest">>,
        observed_result => sample_atom
    });
sample(oci_manifest_evidence, minimal) ->
    beam4pm_types:new_oci_manifest_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        manifest_digest => <<"sample_manifest_digest">>,
        observed_result => sample_atom
    });
sample(offline_bundle_evidence, full) ->
    beam4pm_types:new_offline_bundle_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        bundle_digest => <<"sample_bundle_digest">>,
        observed_result => sample_atom
    });
sample(offline_bundle_evidence, minimal) ->
    beam4pm_types:new_offline_bundle_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        bundle_digest => <<"sample_bundle_digest">>,
        observed_result => sample_atom
    });
sample(operator_readiness, full) ->
    beam4pm_types:new_operator_readiness(#{
        operator_readiness_id => <<"sample_operator_readiness_id">>,
        account_id => <<"sample_account_id">>,
        operator_readiness_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(operator_readiness, minimal) ->
    beam4pm_types:new_operator_readiness(#{
        operator_readiness_id => <<"sample_operator_readiness_id">>,
        account_id => <<"sample_account_id">>,
        operator_readiness_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(opportunity_currency_contract, full) ->
    beam4pm_types:new_opportunity_currency_contract(#{
        opportunity_id => <<"sample_opportunity_id">>,
        currency_code => <<"sample_currency_code">>,
        fx_basis_id => <<"sample_fx_basis_id">>
    });
sample(opportunity_currency_contract, minimal) ->
    beam4pm_types:new_opportunity_currency_contract(#{
        opportunity_id => <<"sample_opportunity_id">>,
        currency_code => <<"sample_currency_code">>,
        fx_basis_id => <<"sample_fx_basis_id">>
    });
sample(opportunity_value_range, full) ->
    beam4pm_types:new_opportunity_value_range(#{
        opportunity_id => <<"sample_opportunity_id">>,
        minimum_value => <<"sample_minimum_value">>,
        maximum_value => <<"sample_maximum_value">>
    });
sample(opportunity_value_range, minimal) ->
    beam4pm_types:new_opportunity_value_range(#{
        opportunity_id => <<"sample_opportunity_id">>,
        minimum_value => <<"sample_minimum_value">>,
        maximum_value => <<"sample_maximum_value">>
    });
sample(optimization_plan_candidate, full) ->
    beam4pm_types:new_optimization_plan_candidate(#{
        plan_id => <<"sample_plan_id">>,
        objective_id => <<"sample_objective_id">>,
        solver_receipt_hash => <<"sample_solver_receipt_hash">>
    });
sample(optimization_plan_candidate, minimal) ->
    beam4pm_types:new_optimization_plan_candidate(#{
        plan_id => <<"sample_plan_id">>,
        objective_id => <<"sample_objective_id">>,
        solver_receipt_hash => <<"sample_solver_receipt_hash">>
    });
sample(option_generation, full) ->
    beam4pm_types:new_option_generation(#{
        state_id => <<"sample_state_id">>,
        generator_id => <<"sample_generator_id">>,
        option_set_hash => <<"sample_option_set_hash">>
    });
sample(option_generation, minimal) ->
    beam4pm_types:new_option_generation(#{
        state_id => <<"sample_state_id">>,
        generator_id => <<"sample_generator_id">>,
        option_set_hash => <<"sample_option_set_hash">>
    });
sample(order_form_admission, full) ->
    beam4pm_types:new_order_form_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        order_form_id => <<"sample_order_form_id">>,
        decision => <<"sample_decision">>
    });
sample(order_form_admission, minimal) ->
    beam4pm_types:new_order_form_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        order_form_id => <<"sample_order_form_id">>,
        decision => <<"sample_decision">>
    });
sample(order_form_version, full) ->
    beam4pm_types:new_order_form_version(#{
        order_form_version_id => <<"sample_order_form_version_id">>,
        account_id => <<"sample_account_id">>,
        order_form_digest => <<"sample_order_form_digest">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(order_form_version, minimal) ->
    beam4pm_types:new_order_form_version(#{
        order_form_version_id => <<"sample_order_form_version_id">>,
        account_id => <<"sample_account_id">>,
        order_form_digest => <<"sample_order_form_digest">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(orthogonality_reward, full) ->
    beam4pm_types:new_orthogonality_reward(#{
        reward_id => <<"sample_reward_id">>,
        candidate_id => <<"sample_candidate_id">>,
        comparison_set_digest => <<"sample_comparison_set_digest">>,
        minimum_semantic_distance => 3.5,
        reward => 3.5,
        independence_evidence_digest => <<"sample_independence_evidence_digest">>
    });
sample(orthogonality_reward, minimal) ->
    beam4pm_types:new_orthogonality_reward(#{
        reward_id => <<"sample_reward_id">>,
        candidate_id => <<"sample_candidate_id">>,
        comparison_set_digest => <<"sample_comparison_set_digest">>,
        minimum_semantic_distance => 3.5,
        reward => 3.5,
        independence_evidence_digest => <<"sample_independence_evidence_digest">>
    });
sample(orthogonality_score, full) ->
    beam4pm_types:new_orthogonality_score(#{
        left_option_id => <<"sample_left_option_id">>,
        right_option_id => <<"sample_right_option_id">>,
        score => <<"sample_score">>
    });
sample(orthogonality_score, minimal) ->
    beam4pm_types:new_orthogonality_score(#{
        left_option_id => <<"sample_left_option_id">>,
        right_option_id => <<"sample_right_option_id">>,
        score => <<"sample_score">>
    });
sample(outcome_branch_set, full) ->
    beam4pm_types:new_outcome_branch_set(#{
        action_id => <<"sample_action_id">>,
        branch_digest => <<"sample_branch_digest">>,
        coverage_mode => <<"sample_coverage_mode">>
    });
sample(outcome_branch_set, minimal) ->
    beam4pm_types:new_outcome_branch_set(#{
        action_id => <<"sample_action_id">>,
        branch_digest => <<"sample_branch_digest">>,
        coverage_mode => <<"sample_coverage_mode">>
    });
sample(outcome_label, full) ->
    beam4pm_types:new_outcome_label(#{
        label_id => <<"sample_label_id">>,
        episode_id => <<"sample_episode_id">>,
        outcome_class => <<"sample_outcome_class">>,
        confidence_basis => <<"sample_confidence_basis">>,
        independent_evidence_digest => <<"sample_independent_evidence_digest">>
    });
sample(outcome_label, minimal) ->
    beam4pm_types:new_outcome_label(#{
        label_id => <<"sample_label_id">>,
        episode_id => <<"sample_episode_id">>,
        outcome_class => <<"sample_outcome_class">>,
        confidence_basis => <<"sample_confidence_basis">>,
        independent_evidence_digest => <<"sample_independent_evidence_digest">>
    });
sample(output_ownership_gate, full) ->
    beam4pm_types:new_output_ownership_gate(#{
        subject_id => <<"sample_subject_id">>,
        ownership_manifest_hash => <<"sample_ownership_manifest_hash">>,
        standing => <<"sample_standing">>
    });
sample(output_ownership_gate, minimal) ->
    beam4pm_types:new_output_ownership_gate(#{
        subject_id => <<"sample_subject_id">>,
        ownership_manifest_hash => <<"sample_ownership_manifest_hash">>,
        standing => <<"sample_standing">>
    });
sample(overage_invoice, full) ->
    beam4pm_types:new_overage_invoice(#{
        overage_invoice_id => <<"sample_overage_invoice_id">>,
        account_id => <<"sample_account_id">>,
        overage_amount => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(overage_invoice, minimal) ->
    beam4pm_types:new_overage_invoice(#{
        overage_invoice_id => <<"sample_overage_invoice_id">>,
        account_id => <<"sample_account_id">>,
        overage_amount => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(overage_policy, full) ->
    beam4pm_types:new_overage_policy(#{
        policy_id => <<"sample_policy_id">>,
        quota_id => <<"sample_quota_id">>,
        unit_price => 3.5,
        behavior => sample_atom
    });
sample(overage_policy, minimal) ->
    beam4pm_types:new_overage_policy(#{
        policy_id => <<"sample_policy_id">>,
        quota_id => <<"sample_quota_id">>,
        unit_price => 3.5,
        behavior => sample_atom
    });
sample(package_release_observation, full) ->
    beam4pm_types:new_package_release_observation(#{
        package_id => <<"sample_package_id">>,
        version => <<"sample_version">>,
        immutable_digest => <<"sample_immutable_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(package_release_observation, minimal) ->
    beam4pm_types:new_package_release_observation(#{
        package_id => <<"sample_package_id">>,
        version => <<"sample_version">>,
        immutable_digest => <<"sample_immutable_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(paid_workload_outcome_receipt, full) ->
    beam4pm_types:new_paid_workload_outcome_receipt(#{
        tenant_id => <<"sample_tenant_id">>,
        workload_id => <<"sample_workload_id">>,
        outcome_receipt_hash => <<"sample_outcome_receipt_hash">>
    });
sample(paid_workload_outcome_receipt, minimal) ->
    beam4pm_types:new_paid_workload_outcome_receipt(#{
        tenant_id => <<"sample_tenant_id">>,
        workload_id => <<"sample_workload_id">>,
        outcome_receipt_hash => <<"sample_outcome_receipt_hash">>
    });
sample(pareto_filter, full) ->
    beam4pm_types:new_pareto_filter(#{
        option_set_id => <<"sample_option_set_id">>,
        objective_set_hash => <<"sample_objective_set_hash">>,
        pareto_set_hash => <<"sample_pareto_set_hash">>
    });
sample(pareto_filter, minimal) ->
    beam4pm_types:new_pareto_filter(#{
        option_set_id => <<"sample_option_set_id">>,
        objective_set_hash => <<"sample_objective_set_hash">>,
        pareto_set_hash => <<"sample_pareto_set_hash">>
    });
sample(path_schema, full) ->
    beam4pm_types:new_path_schema(#{
        schema_id => <<"sample_schema_id">>,
        source_type => <<"sample_source_type">>,
        target_type => <<"sample_target_type">>,
        support => 42
    });
sample(path_schema, minimal) ->
    beam4pm_types:new_path_schema(#{
        schema_id => <<"sample_schema_id">>,
        source_type => <<"sample_source_type">>,
        target_type => <<"sample_target_type">>,
        support => 42
    });
sample(path_schema_query, full) ->
    beam4pm_types:new_path_schema_query(#{
        source_type => <<"sample_source_type">>,
        target_type => <<"sample_target_type">>,
        max_length => 42
    });
sample(path_schema_query, minimal) ->
    beam4pm_types:new_path_schema_query(#{
        source_type => <<"sample_source_type">>,
        target_type => <<"sample_target_type">>,
        max_length => 42
    });
sample(payment_terms, full) ->
    beam4pm_types:new_payment_terms(#{
        terms_id => <<"sample_terms_id">>,
        net_days => 42,
        late_policy => <<"sample_late_policy">>,
        status => sample_atom
    });
sample(payment_terms, minimal) ->
    beam4pm_types:new_payment_terms(#{
        terms_id => <<"sample_terms_id">>,
        net_days => 42,
        late_policy => <<"sample_late_policy">>,
        status => sample_atom
    });
sample(payment_terms_admission, full) ->
    beam4pm_types:new_payment_terms_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        payment_terms => <<"sample_payment_terms">>,
        authority_evidence_hash => <<"sample_authority_evidence_hash">>
    });
sample(payment_terms_admission, minimal) ->
    beam4pm_types:new_payment_terms_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        payment_terms => <<"sample_payment_terms">>,
        authority_evidence_hash => <<"sample_authority_evidence_hash">>
    });
sample(performance_slo_evidence, full) ->
    beam4pm_types:new_performance_slo_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        latency_p95_ms => 42,
        observed_result => sample_atom
    });
sample(performance_slo_evidence, minimal) ->
    beam4pm_types:new_performance_slo_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        latency_p95_ms => 42,
        observed_result => sample_atom
    });
sample(petri_arc, full) ->
    beam4pm_types:new_petri_arc(#{
        source_id => <<"sample_source_id">>,
        target_id => <<"sample_target_id">>,
        weight => 42
    });
sample(petri_arc, minimal) ->
    beam4pm_types:new_petri_arc(#{
        source_id => <<"sample_source_id">>,
        target_id => <<"sample_target_id">>,
        weight => 42
    });
sample(petri_place, full) ->
    beam4pm_types:new_petri_place(#{
        place_id => <<"sample_place_id">>,
        tokens => 42
    });
sample(petri_place, minimal) ->
    beam4pm_types:new_petri_place(#{
        place_id => <<"sample_place_id">>,
        tokens => 42
    });
sample(petri_transition, full) ->
    beam4pm_types:new_petri_transition(#{
        transition_id => <<"sample_transition_id">>,
        label => <<"sample_label">>
    });
sample(petri_transition, minimal) ->
    beam4pm_types:new_petri_transition(#{
        transition_id => <<"sample_transition_id">>
    });
sample(plan_handoff_receipt, full) ->
    beam4pm_types:new_plan_handoff_receipt(#{
        receipt_id => <<"sample_receipt_id">>,
        plan_digest => <<"sample_plan_digest">>,
        receiver_subject_sha => <<"sample_receiver_subject_sha">>
    });
sample(plan_handoff_receipt, minimal) ->
    beam4pm_types:new_plan_handoff_receipt(#{
        receipt_id => <<"sample_receipt_id">>,
        plan_digest => <<"sample_plan_digest">>,
        receiver_subject_sha => <<"sample_receiver_subject_sha">>
    });
sample(plan_lineage, full) ->
    beam4pm_types:new_plan_lineage(#{
        plan_id => <<"sample_plan_id">>,
        parent_plan_id => <<"sample_parent_plan_id">>,
        lineage_hash => <<"sample_lineage_hash">>
    });
sample(plan_lineage, minimal) ->
    beam4pm_types:new_plan_lineage(#{
        plan_id => <<"sample_plan_id">>,
        parent_plan_id => <<"sample_parent_plan_id">>,
        lineage_hash => <<"sample_lineage_hash">>
    });
sample(plan_memory, full) ->
    beam4pm_types:new_plan_memory(#{
        plan_id => <<"sample_plan_id">>,
        evidence_hash => <<"sample_evidence_hash">>,
        memory_hash => <<"sample_memory_hash">>
    });
sample(plan_memory, minimal) ->
    beam4pm_types:new_plan_memory(#{
        plan_id => <<"sample_plan_id">>,
        evidence_hash => <<"sample_evidence_hash">>,
        memory_hash => <<"sample_memory_hash">>
    });
sample(planner_bid, full) ->
    beam4pm_types:new_planner_bid(#{
        bid_id => <<"sample_bid_id">>,
        work_item_id => <<"sample_work_item_id">>,
        score_digest => <<"sample_score_digest">>
    });
sample(planner_bid, minimal) ->
    beam4pm_types:new_planner_bid(#{
        bid_id => <<"sample_bid_id">>,
        work_item_id => <<"sample_work_item_id">>,
        score_digest => <<"sample_score_digest">>
    });
sample(planner_capability_profile, full) ->
    beam4pm_types:new_planner_capability_profile(#{
        planner_id => <<"sample_planner_id">>,
        capability_set => <<"sample_capability_set">>,
        profile_hash => <<"sample_profile_hash">>
    });
sample(planner_capability_profile, minimal) ->
    beam4pm_types:new_planner_capability_profile(#{
        planner_id => <<"sample_planner_id">>,
        capability_set => <<"sample_capability_set">>,
        profile_hash => <<"sample_profile_hash">>
    });
sample(planner_identity, full) ->
    beam4pm_types:new_planner_identity(#{
        planner_id => <<"sample_planner_id">>,
        planner_kind => <<"sample_planner_kind">>,
        identity_hash => <<"sample_identity_hash">>
    });
sample(planner_identity, minimal) ->
    beam4pm_types:new_planner_identity(#{
        planner_id => <<"sample_planner_id">>,
        planner_kind => <<"sample_planner_kind">>,
        identity_hash => <<"sample_identity_hash">>
    });
sample(planner_lease, full) ->
    beam4pm_types:new_planner_lease(#{
        lease_id => <<"sample_lease_id">>,
        work_item_id => <<"sample_work_item_id">>,
        expires_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(planner_lease, minimal) ->
    beam4pm_types:new_planner_lease(#{
        lease_id => <<"sample_lease_id">>,
        work_item_id => <<"sample_work_item_id">>,
        expires_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(planner_payoff_observation, full) ->
    beam4pm_types:new_planner_payoff_observation(#{
        observation_id => <<"sample_observation_id">>,
        planner_id => <<"sample_planner_id">>,
        objective_id => <<"sample_objective_id">>,
        payoff => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        episode_id => <<"sample_episode_id">>
    });
sample(planner_payoff_observation, minimal) ->
    beam4pm_types:new_planner_payoff_observation(#{
        observation_id => <<"sample_observation_id">>,
        planner_id => <<"sample_planner_id">>,
        objective_id => <<"sample_objective_id">>,
        payoff => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        episode_id => <<"sample_episode_id">>
    });
sample(planner_policy_comparison, full) ->
    beam4pm_types:new_planner_policy_comparison(#{
        comparison_id => <<"sample_comparison_id">>,
        planner_id => <<"sample_planner_id">>,
        incumbent_policy_id => <<"sample_incumbent_policy_id">>,
        challenger_policy_id => <<"sample_challenger_policy_id">>,
        payoff_delta => 3.5,
        winner_evidence_digest => <<"sample_winner_evidence_digest">>
    });
sample(planner_policy_comparison, minimal) ->
    beam4pm_types:new_planner_policy_comparison(#{
        comparison_id => <<"sample_comparison_id">>,
        planner_id => <<"sample_planner_id">>,
        incumbent_policy_id => <<"sample_incumbent_policy_id">>,
        challenger_policy_id => <<"sample_challenger_policy_id">>,
        payoff_delta => 3.5,
        winner_evidence_digest => <<"sample_winner_evidence_digest">>
    });
sample(planner_portfolio, full) ->
    beam4pm_types:new_planner_portfolio(#{
        portfolio_id => <<"sample_portfolio_id">>,
        planner_ids => <<"sample_planner_ids">>,
        diversity_hash => <<"sample_diversity_hash">>
    });
sample(planner_portfolio, minimal) ->
    beam4pm_types:new_planner_portfolio(#{
        portfolio_id => <<"sample_portfolio_id">>,
        planner_ids => <<"sample_planner_ids">>,
        diversity_hash => <<"sample_diversity_hash">>
    });
sample(planner_routing_update, full) ->
    beam4pm_types:new_planner_routing_update(#{
        update_id => <<"sample_update_id">>,
        route_id => <<"sample_route_id">>,
        prior_planner_id => <<"sample_prior_planner_id">>,
        selected_planner_id => <<"sample_selected_planner_id">>,
        payoff_basis_digest => <<"sample_payoff_basis_digest">>,
        rollback_policy_id => <<"sample_rollback_policy_id">>
    });
sample(planner_routing_update, minimal) ->
    beam4pm_types:new_planner_routing_update(#{
        update_id => <<"sample_update_id">>,
        route_id => <<"sample_route_id">>,
        prior_planner_id => <<"sample_prior_planner_id">>,
        selected_planner_id => <<"sample_selected_planner_id">>,
        payoff_basis_digest => <<"sample_payoff_basis_digest">>,
        rollback_policy_id => <<"sample_rollback_policy_id">>
    });
sample(planning_action, full) ->
    beam4pm_types:new_planning_action(#{
        action_name => <<"sample_action_name">>,
        preconditions => [<<"alpha">>, <<"beta">>],
        effects => [<<"alpha">>, <<"beta">>]
    });
sample(planning_action, minimal) ->
    beam4pm_types:new_planning_action(#{
        action_name => <<"sample_action_name">>
    });
sample(planning_blackboard_claim, full) ->
    beam4pm_types:new_planning_blackboard_claim(#{
        claim_id => <<"sample_claim_id">>,
        work_item_id => <<"sample_work_item_id">>,
        owner_subject_sha => <<"sample_owner_subject_sha">>
    });
sample(planning_blackboard_claim, minimal) ->
    beam4pm_types:new_planning_blackboard_claim(#{
        claim_id => <<"sample_claim_id">>,
        work_item_id => <<"sample_work_item_id">>,
        owner_subject_sha => <<"sample_owner_subject_sha">>
    });
sample(planning_blackboard_conflict, full) ->
    beam4pm_types:new_planning_blackboard_conflict(#{
        conflict_id => <<"sample_conflict_id">>,
        claim_set_digest => <<"sample_claim_set_digest">>,
        conflict_type => <<"sample_conflict_type">>
    });
sample(planning_blackboard_conflict, minimal) ->
    beam4pm_types:new_planning_blackboard_conflict(#{
        conflict_id => <<"sample_conflict_id">>,
        claim_set_digest => <<"sample_claim_set_digest">>,
        conflict_type => <<"sample_conflict_type">>
    });
sample(planning_blackboard_fact, full) ->
    beam4pm_types:new_planning_blackboard_fact(#{
        fact_id => <<"sample_fact_id">>,
        subject_sha => <<"sample_subject_sha">>,
        provenance_digest => <<"sample_provenance_digest">>
    });
sample(planning_blackboard_fact, minimal) ->
    beam4pm_types:new_planning_blackboard_fact(#{
        fact_id => <<"sample_fact_id">>,
        subject_sha => <<"sample_subject_sha">>,
        provenance_digest => <<"sample_provenance_digest">>
    });
sample(planning_blackboard_resolution, full) ->
    beam4pm_types:new_planning_blackboard_resolution(#{
        resolution_id => <<"sample_resolution_id">>,
        conflict_id => <<"sample_conflict_id">>,
        preserved_option_digest => <<"sample_preserved_option_digest">>
    });
sample(planning_blackboard_resolution, minimal) ->
    beam4pm_types:new_planning_blackboard_resolution(#{
        resolution_id => <<"sample_resolution_id">>,
        conflict_id => <<"sample_conflict_id">>,
        preserved_option_digest => <<"sample_preserved_option_digest">>
    });
sample(planning_case_identity, full) ->
    beam4pm_types:new_planning_case_identity(#{
        case_id => <<"sample_case_id">>,
        subject_sha => <<"sample_subject_sha">>,
        lineage_root => <<"sample_lineage_root">>
    });
sample(planning_case_identity, minimal) ->
    beam4pm_types:new_planning_case_identity(#{
        case_id => <<"sample_case_id">>,
        subject_sha => <<"sample_subject_sha">>,
        lineage_root => <<"sample_lineage_root">>
    });
sample(planning_conformance_alignment, full) ->
    beam4pm_types:new_planning_conformance_alignment(#{
        alignment_id => <<"sample_alignment_id">>,
        trace_digest => <<"sample_trace_digest">>,
        model_digest => <<"sample_model_digest">>
    });
sample(planning_conformance_alignment, minimal) ->
    beam4pm_types:new_planning_conformance_alignment(#{
        alignment_id => <<"sample_alignment_id">>,
        trace_digest => <<"sample_trace_digest">>,
        model_digest => <<"sample_model_digest">>
    });
sample(planning_problem_admission, full) ->
    beam4pm_types:new_planning_problem_admission(#{
        problem_id => <<"sample_problem_id">>,
        state_digest => <<"sample_state_digest">>,
        authority_scope => <<"sample_authority_scope">>
    });
sample(planning_problem_admission, minimal) ->
    beam4pm_types:new_planning_problem_admission(#{
        problem_id => <<"sample_problem_id">>,
        state_digest => <<"sample_state_digest">>,
        authority_scope => <<"sample_authority_scope">>
    });
sample(planning_state, full) ->
    beam4pm_types:new_planning_state(#{
        state_id => <<"sample_state_id">>,
        facts => [<<"alpha">>, <<"beta">>]
    });
sample(planning_state, minimal) ->
    beam4pm_types:new_planning_state(#{
        state_id => <<"sample_state_id">>,
        facts => [<<"alpha">>, <<"beta">>]
    });
sample(poc_exit_criteria, full) ->
    beam4pm_types:new_poc_exit_criteria(#{
        poc_exit_criteria_id => <<"sample_poc_exit_criteria_id">>,
        account_id => <<"sample_account_id">>,
        criteria_pass_rate => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(poc_exit_criteria, minimal) ->
    beam4pm_types:new_poc_exit_criteria(#{
        poc_exit_criteria_id => <<"sample_poc_exit_criteria_id">>,
        account_id => <<"sample_account_id">>,
        criteria_pass_rate => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(poc_risk, full) ->
    beam4pm_types:new_poc_risk(#{
        poc_risk_id => <<"sample_poc_risk_id">>,
        account_id => <<"sample_account_id">>,
        risk_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(poc_risk, minimal) ->
    beam4pm_types:new_poc_risk(#{
        poc_risk_id => <<"sample_poc_risk_id">>,
        account_id => <<"sample_account_id">>,
        risk_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(poc_scope, full) ->
    beam4pm_types:new_poc_scope(#{
        poc_scope_id => <<"sample_poc_scope_id">>,
        account_id => <<"sample_account_id">>,
        use_case_count => 42,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(poc_scope, minimal) ->
    beam4pm_types:new_poc_scope(#{
        poc_scope_id => <<"sample_poc_scope_id">>,
        account_id => <<"sample_account_id">>,
        use_case_count => 42,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(poc_timeline, full) ->
    beam4pm_types:new_poc_timeline(#{
        poc_timeline_id => <<"sample_poc_timeline_id">>,
        account_id => <<"sample_account_id">>,
        days_to_value => 42,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(poc_timeline, minimal) ->
    beam4pm_types:new_poc_timeline(#{
        poc_timeline_id => <<"sample_poc_timeline_id">>,
        account_id => <<"sample_account_id">>,
        days_to_value => 42,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(policy_binding, full) ->
    beam4pm_types:new_policy_binding(#{
        policy_id => <<"sample_policy_id">>,
        planner_id => <<"sample_planner_id">>,
        policy_hash => <<"sample_policy_hash">>
    });
sample(policy_binding, minimal) ->
    beam4pm_types:new_policy_binding(#{
        policy_id => <<"sample_policy_id">>,
        planner_id => <<"sample_planner_id">>,
        policy_hash => <<"sample_policy_hash">>
    });
sample(policy_decision, full) ->
    beam4pm_types:new_policy_decision(#{
        decision_id => <<"sample_decision_id">>,
        verdict => sample_atom,
        reason => <<"sample_reason">>
    });
sample(policy_decision, minimal) ->
    beam4pm_types:new_policy_decision(#{
        decision_id => <<"sample_decision_id">>,
        verdict => sample_atom
    });
sample(policy_graph_edge, full) ->
    beam4pm_types:new_policy_graph_edge(#{
        source_node_id => <<"sample_source_node_id">>,
        outcome_label => <<"sample_outcome_label">>,
        target_node_id => <<"sample_target_node_id">>
    });
sample(policy_graph_edge, minimal) ->
    beam4pm_types:new_policy_graph_edge(#{
        source_node_id => <<"sample_source_node_id">>,
        outcome_label => <<"sample_outcome_label">>,
        target_node_id => <<"sample_target_node_id">>
    });
sample(policy_graph_node, full) ->
    beam4pm_types:new_policy_graph_node(#{
        node_id => <<"sample_node_id">>,
        state_digest => <<"sample_state_digest">>,
        selected_action_id => <<"sample_selected_action_id">>
    });
sample(policy_graph_node, minimal) ->
    beam4pm_types:new_policy_graph_node(#{
        node_id => <<"sample_node_id">>,
        state_digest => <<"sample_state_digest">>,
        selected_action_id => <<"sample_selected_action_id">>
    });
sample(policy_payoff_observation, full) ->
    beam4pm_types:new_policy_payoff_observation(#{
        observation_id => <<"sample_observation_id">>,
        policy_id => <<"sample_policy_id">>,
        parameter_digest => <<"sample_parameter_digest">>,
        objective_id => <<"sample_objective_id">>,
        payoff => 3.5,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(policy_payoff_observation, minimal) ->
    beam4pm_types:new_policy_payoff_observation(#{
        observation_id => <<"sample_observation_id">>,
        policy_id => <<"sample_policy_id">>,
        parameter_digest => <<"sample_parameter_digest">>,
        objective_id => <<"sample_objective_id">>,
        payoff => 3.5,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(powl_choice_graph_edge, full) ->
    beam4pm_types:new_powl_choice_graph_edge(#{
        from_kind => sample_atom,
        from_child_index => 42,
        to_kind => sample_atom,
        to_child_index => 42
    });
sample(powl_choice_graph_edge, minimal) ->
    beam4pm_types:new_powl_choice_graph_edge(#{
        from_kind => sample_atom,
        to_kind => sample_atom
    });
sample(powl_choice_operator, full) ->
    beam4pm_types:new_powl_choice_operator(#{
        operator_id => <<"sample_operator_id">>,
        branch_digest => <<"sample_branch_digest">>,
        selection_rule => <<"sample_selection_rule">>
    });
sample(powl_choice_operator, minimal) ->
    beam4pm_types:new_powl_choice_operator(#{
        operator_id => <<"sample_operator_id">>,
        branch_digest => <<"sample_branch_digest">>,
        selection_rule => <<"sample_selection_rule">>
    });
sample(powl_freq, full) ->
    beam4pm_types:new_powl_freq(#{
        min_freq => 42,
        max_freq => 42
    });
sample(powl_freq, minimal) ->
    beam4pm_types:new_powl_freq(#{
        min_freq => 42
    });
sample(powl_leaf, full) ->
    beam4pm_types:new_powl_leaf(#{
        activity_label => <<"sample_activity_label">>,
        is_tau => true,
        min_freq => 42,
        max_freq => 42
    });
sample(powl_leaf, minimal) ->
    beam4pm_types:new_powl_leaf(#{
        is_tau => true,
        min_freq => 42
    });
sample(powl_loop_operator, full) ->
    beam4pm_types:new_powl_loop_operator(#{
        operator_id => <<"sample_operator_id">>,
        body_digest => <<"sample_body_digest">>,
        exit_predicate => <<"sample_exit_predicate">>
    });
sample(powl_loop_operator, minimal) ->
    beam4pm_types:new_powl_loop_operator(#{
        operator_id => <<"sample_operator_id">>,
        body_digest => <<"sample_body_digest">>,
        exit_predicate => <<"sample_exit_predicate">>
    });
sample(powl_parallel_operator, full) ->
    beam4pm_types:new_powl_parallel_operator(#{
        operator_id => <<"sample_operator_id">>,
        branch_digest => <<"sample_branch_digest">>,
        join_rule => <<"sample_join_rule">>
    });
sample(powl_parallel_operator, minimal) ->
    beam4pm_types:new_powl_parallel_operator(#{
        operator_id => <<"sample_operator_id">>,
        branch_digest => <<"sample_branch_digest">>,
        join_rule => <<"sample_join_rule">>
    });
sample(powl_partial_order_edge, full) ->
    beam4pm_types:new_powl_partial_order_edge(#{
        from_index => 42,
        to_index => 42
    });
sample(powl_partial_order_edge, minimal) ->
    beam4pm_types:new_powl_partial_order_edge(#{
        from_index => 42,
        to_index => 42
    });
sample(powl_partial_order_plan, full) ->
    beam4pm_types:new_powl_partial_order_plan(#{
        plan_id => <<"sample_plan_id">>,
        operator_digest => <<"sample_operator_digest">>,
        order_digest => <<"sample_order_digest">>
    });
sample(powl_partial_order_plan, minimal) ->
    beam4pm_types:new_powl_partial_order_plan(#{
        plan_id => <<"sample_plan_id">>,
        operator_digest => <<"sample_operator_digest">>,
        order_digest => <<"sample_order_digest">>
    });
sample(powl_projection, full) ->
    beam4pm_types:new_powl_projection(#{
        plan_id => <<"sample_plan_id">>,
        powl_hash => <<"sample_powl_hash">>,
        projection_receipt_hash => <<"sample_projection_receipt_hash">>
    });
sample(powl_projection, minimal) ->
    beam4pm_types:new_powl_projection(#{
        plan_id => <<"sample_plan_id">>,
        powl_hash => <<"sample_powl_hash">>,
        projection_receipt_hash => <<"sample_projection_receipt_hash">>
    });
sample(powl_sequence_operator, full) ->
    beam4pm_types:new_powl_sequence_operator(#{
        operator_id => <<"sample_operator_id">>,
        step_digest => <<"sample_step_digest">>,
        predecessor_digest => <<"sample_predecessor_digest">>
    });
sample(powl_sequence_operator, minimal) ->
    beam4pm_types:new_powl_sequence_operator(#{
        operator_id => <<"sample_operator_id">>,
        step_digest => <<"sample_step_digest">>,
        predecessor_digest => <<"sample_predecessor_digest">>
    });
sample(ppddl_projection, full) ->
    beam4pm_types:new_ppddl_projection(#{
        plan_id => <<"sample_plan_id">>,
        domain_hash => <<"sample_domain_hash">>,
        problem_hash => <<"sample_problem_hash">>
    });
sample(ppddl_projection, minimal) ->
    beam4pm_types:new_ppddl_projection(#{
        plan_id => <<"sample_plan_id">>,
        domain_hash => <<"sample_domain_hash">>,
        problem_hash => <<"sample_problem_hash">>
    });
sample(prefix_alignment_frontier, full) ->
    beam4pm_types:new_prefix_alignment_frontier(#{
        frontier_id => <<"sample_frontier_id">>,
        prefix_digest => <<"sample_prefix_digest">>,
        candidate_digest => <<"sample_candidate_digest">>
    });
sample(prefix_alignment_frontier, minimal) ->
    beam4pm_types:new_prefix_alignment_frontier(#{
        frontier_id => <<"sample_frontier_id">>,
        prefix_digest => <<"sample_prefix_digest">>,
        candidate_digest => <<"sample_candidate_digest">>
    });
sample(premium_connector_pricing, full) ->
    beam4pm_types:new_premium_connector_pricing(#{
        premium_connector_pricing_id => <<"sample_premium_connector_pricing_id">>,
        account_id => <<"sample_account_id">>,
        connector_id => <<"sample_connector_id">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(premium_connector_pricing, minimal) ->
    beam4pm_types:new_premium_connector_pricing(#{
        premium_connector_pricing_id => <<"sample_premium_connector_pricing_id">>,
        account_id => <<"sample_account_id">>,
        connector_id => <<"sample_connector_id">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(prepaid_credit_balance, full) ->
    beam4pm_types:new_prepaid_credit_balance(#{
        prepaid_credit_balance_id => <<"sample_prepaid_credit_balance_id">>,
        account_id => <<"sample_account_id">>,
        remaining_credit => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(prepaid_credit_balance, minimal) ->
    beam4pm_types:new_prepaid_credit_balance(#{
        prepaid_credit_balance_id => <<"sample_prepaid_credit_balance_id">>,
        account_id => <<"sample_account_id">>,
        remaining_credit => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(price_book_version, full) ->
    beam4pm_types:new_price_book_version(#{
        price_book_version_id => <<"sample_price_book_version_id">>,
        account_id => <<"sample_account_id">>,
        version => <<"sample_version">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(price_book_version, minimal) ->
    beam4pm_types:new_price_book_version(#{
        price_book_version_id => <<"sample_price_book_version_id">>,
        account_id => <<"sample_account_id">>,
        version => <<"sample_version">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(pricing_basis_contract, full) ->
    beam4pm_types:new_pricing_basis_contract(#{
        opportunity_id => <<"sample_opportunity_id">>,
        pricing_basis_id => <<"sample_pricing_basis_id">>,
        evidence_hash => <<"sample_evidence_hash">>
    });
sample(pricing_basis_contract, minimal) ->
    beam4pm_types:new_pricing_basis_contract(#{
        opportunity_id => <<"sample_opportunity_id">>,
        pricing_basis_id => <<"sample_pricing_basis_id">>,
        evidence_hash => <<"sample_evidence_hash">>
    });
sample(primitive_task_binding, full) ->
    beam4pm_types:new_primitive_task_binding(#{
        task_id => <<"sample_task_id">>,
        action_id => <<"sample_action_id">>,
        binding_digest => <<"sample_binding_digest">>
    });
sample(primitive_task_binding, minimal) ->
    beam4pm_types:new_primitive_task_binding(#{
        task_id => <<"sample_task_id">>,
        action_id => <<"sample_action_id">>,
        binding_digest => <<"sample_binding_digest">>
    });
sample(privacy_classification_evidence, full) ->
    beam4pm_types:new_privacy_classification_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        privacy_class => <<"sample_privacy_class">>,
        observed_result => sample_atom
    });
sample(privacy_classification_evidence, minimal) ->
    beam4pm_types:new_privacy_classification_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        privacy_class => <<"sample_privacy_class">>,
        observed_result => sample_atom
    });
sample(private_offer, full) ->
    beam4pm_types:new_private_offer(#{
        offer_id => <<"sample_offer_id">>,
        account_id => <<"sample_account_id">>,
        total_price => 3.5,
        expires_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(private_offer, minimal) ->
    beam4pm_types:new_private_offer(#{
        offer_id => <<"sample_offer_id">>,
        account_id => <<"sample_account_id">>,
        total_price => 3.5,
        expires_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(private_registry_evidence, full) ->
    beam4pm_types:new_private_registry_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        registry_digest => <<"sample_registry_digest">>,
        observed_result => sample_atom
    });
sample(private_registry_evidence, minimal) ->
    beam4pm_types:new_private_registry_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        registry_digest => <<"sample_registry_digest">>,
        observed_result => sample_atom
    });
sample(process_variant, full) ->
    beam4pm_types:new_process_variant(#{
        variant_id => <<"sample_variant_id">>,
        activity_sequence => [<<"alpha">>, <<"beta">>],
        frequency => 42
    });
sample(process_variant, minimal) ->
    beam4pm_types:new_process_variant(#{
        variant_id => <<"sample_variant_id">>,
        activity_sequence => [<<"alpha">>, <<"beta">>],
        frequency => 42
    });
sample(process_volume_pricing_policy, full) ->
    beam4pm_types:new_process_volume_pricing_policy(#{
        process_volume_pricing_policy_id => <<"sample_process_volume_pricing_policy_id">>,
        account_id => <<"sample_account_id">>,
        unit_process_price => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(process_volume_pricing_policy, minimal) ->
    beam4pm_types:new_process_volume_pricing_policy(#{
        process_volume_pricing_policy_id => <<"sample_process_volume_pricing_policy_id">>,
        account_id => <<"sample_account_id">>,
        unit_process_price => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(procurement_acceptance_evidence, full) ->
    beam4pm_types:new_procurement_acceptance_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        acceptance_decision => <<"sample_acceptance_decision">>,
        observed_result => sample_atom
    });
sample(procurement_acceptance_evidence, minimal) ->
    beam4pm_types:new_procurement_acceptance_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        acceptance_decision => <<"sample_acceptance_decision">>,
        observed_result => sample_atom
    });
sample(procurement_blocker, full) ->
    beam4pm_types:new_procurement_blocker(#{
        opportunity_id => <<"sample_opportunity_id">>,
        blocker_id => <<"sample_blocker_id">>,
        refusal_code => <<"sample_refusal_code">>
    });
sample(procurement_blocker, minimal) ->
    beam4pm_types:new_procurement_blocker(#{
        opportunity_id => <<"sample_opportunity_id">>,
        blocker_id => <<"sample_blocker_id">>,
        refusal_code => <<"sample_refusal_code">>
    });
sample(procurement_channel_selection, full) ->
    beam4pm_types:new_procurement_channel_selection(#{
        opportunity_id => <<"sample_opportunity_id">>,
        channel_id => <<"sample_channel_id">>,
        selection_evidence_hash => <<"sample_selection_evidence_hash">>
    });
sample(procurement_channel_selection, minimal) ->
    beam4pm_types:new_procurement_channel_selection(#{
        opportunity_id => <<"sample_opportunity_id">>,
        channel_id => <<"sample_channel_id">>,
        selection_evidence_hash => <<"sample_selection_evidence_hash">>
    });
sample(procurement_readiness, full) ->
    beam4pm_types:new_procurement_readiness(#{
        procurement_readiness_id => <<"sample_procurement_readiness_id">>,
        account_id => <<"sample_account_id">>,
        procurement_stage => <<"sample_procurement_stage">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(procurement_readiness, minimal) ->
    beam4pm_types:new_procurement_readiness(#{
        procurement_readiness_id => <<"sample_procurement_readiness_id">>,
        account_id => <<"sample_account_id">>,
        procurement_stage => <<"sample_procurement_stage">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(production_readiness, full) ->
    beam4pm_types:new_production_readiness(#{
        production_readiness_id => <<"sample_production_readiness_id">>,
        account_id => <<"sample_account_id">>,
        production_readiness_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(production_readiness, minimal) ->
    beam4pm_types:new_production_readiness(#{
        production_readiness_id => <<"sample_production_readiness_id">>,
        account_id => <<"sample_account_id">>,
        production_readiness_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(promotion_decision, full) ->
    beam4pm_types:new_promotion_decision(#{
        decision_id => <<"sample_decision_id">>,
        candidate_policy_id => <<"sample_candidate_policy_id">>,
        threshold_id => <<"sample_threshold_id">>,
        evidence_set_digest => <<"sample_evidence_set_digest">>,
        decision => <<"sample_decision">>,
        refusal_code => <<"sample_refusal_code">>
    });
sample(promotion_decision, minimal) ->
    beam4pm_types:new_promotion_decision(#{
        decision_id => <<"sample_decision_id">>,
        candidate_policy_id => <<"sample_candidate_policy_id">>,
        threshold_id => <<"sample_threshold_id">>,
        evidence_set_digest => <<"sample_evidence_set_digest">>,
        decision => <<"sample_decision">>,
        refusal_code => <<"sample_refusal_code">>
    });
sample(promotion_threshold, full) ->
    beam4pm_types:new_promotion_threshold(#{
        threshold_id => <<"sample_threshold_id">>,
        objective_id => <<"sample_objective_id">>,
        minimum_payoff => 3.5,
        maximum_regression => 3.5,
        minimum_evidence_count => 42,
        authority_binding => <<"sample_authority_binding">>
    });
sample(promotion_threshold, minimal) ->
    beam4pm_types:new_promotion_threshold(#{
        threshold_id => <<"sample_threshold_id">>,
        objective_id => <<"sample_objective_id">>,
        minimum_payoff => 3.5,
        maximum_regression => 3.5,
        minimum_evidence_count => 42,
        authority_binding => <<"sample_authority_binding">>
    });
sample(proof_of_value_budget, full) ->
    beam4pm_types:new_proof_of_value_budget(#{
        opportunity_id => <<"sample_opportunity_id">>,
        budget_id => <<"sample_budget_id">>,
        decision => <<"sample_decision">>
    });
sample(proof_of_value_budget, minimal) ->
    beam4pm_types:new_proof_of_value_budget(#{
        opportunity_id => <<"sample_opportunity_id">>,
        budget_id => <<"sample_budget_id">>,
        decision => <<"sample_decision">>
    });
sample(proof_of_value_exit_gate, full) ->
    beam4pm_types:new_proof_of_value_exit_gate(#{
        pov_id => <<"sample_pov_id">>,
        exit_gate_id => <<"sample_exit_gate_id">>,
        decision => <<"sample_decision">>
    });
sample(proof_of_value_exit_gate, minimal) ->
    beam4pm_types:new_proof_of_value_exit_gate(#{
        pov_id => <<"sample_pov_id">>,
        exit_gate_id => <<"sample_exit_gate_id">>,
        decision => <<"sample_decision">>
    });
sample(proof_of_value_package, full) ->
    beam4pm_types:new_proof_of_value_package(#{
        proof_of_value_package_id => <<"sample_proof_of_value_package_id">>,
        account_id => <<"sample_account_id">>,
        success_metric => <<"sample_success_metric">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(proof_of_value_package, minimal) ->
    beam4pm_types:new_proof_of_value_package(#{
        proof_of_value_package_id => <<"sample_proof_of_value_package_id">>,
        account_id => <<"sample_account_id">>,
        success_metric => <<"sample_success_metric">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(propagation_score, full) ->
    beam4pm_types:new_propagation_score(#{
        option_id => <<"sample_option_id">>,
        graph_hash => <<"sample_graph_hash">>,
        score => <<"sample_score">>
    });
sample(propagation_score, minimal) ->
    beam4pm_types:new_propagation_score(#{
        option_id => <<"sample_option_id">>,
        graph_hash => <<"sample_graph_hash">>,
        score => <<"sample_score">>
    });
sample(proration_policy, full) ->
    beam4pm_types:new_proration_policy(#{
        proration_policy_id => <<"sample_proration_policy_id">>,
        account_id => <<"sample_account_id">>,
        proration_method => <<"sample_proration_method">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(proration_policy, minimal) ->
    beam4pm_types:new_proration_policy(#{
        proration_policy_id => <<"sample_proration_policy_id">>,
        account_id => <<"sample_account_id">>,
        proration_method => <<"sample_proration_method">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(provenance_binding_evidence, full) ->
    beam4pm_types:new_provenance_binding_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        builder_identity => <<"sample_builder_identity">>,
        observed_result => sample_atom
    });
sample(provenance_binding_evidence, minimal) ->
    beam4pm_types:new_provenance_binding_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        builder_identity => <<"sample_builder_identity">>,
        observed_result => sample_atom
    });
sample(provenance_binding_observation, full) ->
    beam4pm_types:new_provenance_binding_observation(#{
        observation_id => <<"sample_observation_id">>,
        source_capsule_digest => <<"sample_source_capsule_digest">>,
        evidence_digest => <<"sample_evidence_digest">>,
        binding_status => <<"sample_binding_status">>
    });
sample(provenance_binding_observation, minimal) ->
    beam4pm_types:new_provenance_binding_observation(#{
        observation_id => <<"sample_observation_id">>,
        source_capsule_digest => <<"sample_source_capsule_digest">>,
        evidence_digest => <<"sample_evidence_digest">>,
        binding_status => <<"sample_binding_status">>
    });
sample(psro_population, full) ->
    beam4pm_types:new_psro_population(#{
        population_id => <<"sample_population_id">>,
        policy_ids => <<"sample_policy_ids">>,
        population_hash => <<"sample_population_hash">>
    });
sample(psro_population, minimal) ->
    beam4pm_types:new_psro_population(#{
        population_id => <<"sample_population_id">>,
        policy_ids => <<"sample_policy_ids">>,
        population_hash => <<"sample_population_hash">>
    });
sample(purchase_order_binding, full) ->
    beam4pm_types:new_purchase_order_binding(#{
        binding_id => <<"sample_binding_id">>,
        order_id => <<"sample_order_id">>,
        purchase_order_number => <<"sample_purchase_order_number">>,
        status => sample_atom
    });
sample(purchase_order_binding, minimal) ->
    beam4pm_types:new_purchase_order_binding(#{
        binding_id => <<"sample_binding_id">>,
        order_id => <<"sample_order_id">>,
        purchase_order_number => <<"sample_purchase_order_number">>,
        status => sample_atom
    });
sample(purchase_order_requirement, full) ->
    beam4pm_types:new_purchase_order_requirement(#{
        opportunity_id => <<"sample_opportunity_id">>,
        requirement_id => <<"sample_requirement_id">>,
        evidence_hash => <<"sample_evidence_hash">>
    });
sample(purchase_order_requirement, minimal) ->
    beam4pm_types:new_purchase_order_requirement(#{
        opportunity_id => <<"sample_opportunity_id">>,
        requirement_id => <<"sample_requirement_id">>,
        evidence_hash => <<"sample_evidence_hash">>
    });
sample(purchasing_entity_identity, full) ->
    beam4pm_types:new_purchasing_entity_identity(#{
        opportunity_id => <<"sample_opportunity_id">>,
        purchasing_entity_id => <<"sample_purchasing_entity_id">>,
        identity_evidence_hash => <<"sample_identity_evidence_hash">>
    });
sample(purchasing_entity_identity, minimal) ->
    beam4pm_types:new_purchasing_entity_identity(#{
        opportunity_id => <<"sample_opportunity_id">>,
        purchasing_entity_id => <<"sample_purchasing_entity_id">>,
        identity_evidence_hash => <<"sample_identity_evidence_hash">>
    });
sample(queue_snapshot, full) ->
    beam4pm_types:new_queue_snapshot(#{
        queue_name => <<"sample_queue_name">>,
        depth => 42,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(queue_snapshot, minimal) ->
    beam4pm_types:new_queue_snapshot(#{
        queue_name => <<"sample_queue_name">>,
        depth => 42,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(quota_burst_allowance, full) ->
    beam4pm_types:new_quota_burst_allowance(#{
        quota_burst_allowance_id => <<"sample_quota_burst_allowance_id">>,
        account_id => <<"sample_account_id">>,
        burst_units => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(quota_burst_allowance, minimal) ->
    beam4pm_types:new_quota_burst_allowance(#{
        quota_burst_allowance_id => <<"sample_quota_burst_allowance_id">>,
        account_id => <<"sample_account_id">>,
        burst_units => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(quota_override, full) ->
    beam4pm_types:new_quota_override(#{
        quota_override_id => <<"sample_quota_override_id">>,
        account_id => <<"sample_account_id">>,
        override_units => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(quota_override, minimal) ->
    beam4pm_types:new_quota_override(#{
        quota_override_id => <<"sample_quota_override_id">>,
        account_id => <<"sample_account_id">>,
        override_units => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(quota_policy, full) ->
    beam4pm_types:new_quota_policy(#{
        quota_id => <<"sample_quota_id">>,
        metric_name => <<"sample_metric_name">>,
        limit => 3.5,
        window => sample_atom
    });
sample(quota_policy, minimal) ->
    beam4pm_types:new_quota_policy(#{
        quota_id => <<"sample_quota_id">>,
        metric_name => <<"sample_metric_name">>,
        limit => 3.5,
        window => sample_atom
    });
sample(ramp_commitment, full) ->
    beam4pm_types:new_ramp_commitment(#{
        ramp_id => <<"sample_ramp_id">>,
        phase => 42,
        committed_amount => 3.5,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(ramp_commitment, minimal) ->
    beam4pm_types:new_ramp_commitment(#{
        ramp_id => <<"sample_ramp_id">>,
        phase => 42,
        committed_amount => 3.5,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(rate_card_entry, full) ->
    beam4pm_types:new_rate_card_entry(#{
        rate_card_entry_id => <<"sample_rate_card_entry_id">>,
        account_id => <<"sample_account_id">>,
        unit_price => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(rate_card_entry, minimal) ->
    beam4pm_types:new_rate_card_entry(#{
        rate_card_entry_id => <<"sample_rate_card_entry_id">>,
        account_id => <<"sample_account_id">>,
        unit_price => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(rate_distortion_budget, full) ->
    beam4pm_types:new_rate_distortion_budget(#{
        budget_id => <<"sample_budget_id">>,
        decision_id => <<"sample_decision_id">>,
        source_information_bits => 3.5,
        retained_information_bits => 3.5,
        maximum_distortion => 3.5
    });
sample(rate_distortion_budget, minimal) ->
    beam4pm_types:new_rate_distortion_budget(#{
        budget_id => <<"sample_budget_id">>,
        decision_id => <<"sample_decision_id">>,
        source_information_bits => 3.5,
        retained_information_bits => 3.5,
        maximum_distortion => 3.5
    });
sample(reachability_analysis, full) ->
    beam4pm_types:new_reachability_analysis(#{
        state_id => <<"sample_state_id">>,
        goal_id => <<"sample_goal_id">>,
        reachability_proof_hash => <<"sample_reachability_proof_hash">>
    });
sample(reachability_analysis, minimal) ->
    beam4pm_types:new_reachability_analysis(#{
        state_id => <<"sample_state_id">>,
        goal_id => <<"sample_goal_id">>,
        reachability_proof_hash => <<"sample_reachability_proof_hash">>
    });
sample(receipt_learning_compilation, full) ->
    beam4pm_types:new_receipt_learning_compilation(#{
        compilation_id => <<"sample_compilation_id">>,
        receipt_chain_head => <<"sample_receipt_chain_head">>,
        admitted_receipt_count => 42,
        rejected_receipt_count => 42,
        learning_digest => <<"sample_learning_digest">>
    });
sample(receipt_learning_compilation, minimal) ->
    beam4pm_types:new_receipt_learning_compilation(#{
        compilation_id => <<"sample_compilation_id">>,
        receipt_chain_head => <<"sample_receipt_chain_head">>,
        admitted_receipt_count => 42,
        rejected_receipt_count => 42,
        learning_digest => <<"sample_learning_digest">>
    });
sample(receipt_replay_evidence, full) ->
    beam4pm_types:new_receipt_replay_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        replay_result_hash => <<"sample_replay_result_hash">>,
        observed_result => sample_atom
    });
sample(receipt_replay_evidence, minimal) ->
    beam4pm_types:new_receipt_replay_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        replay_result_hash => <<"sample_replay_result_hash">>,
        observed_result => sample_atom
    });
sample(receipt_replay_request, full) ->
    beam4pm_types:new_receipt_replay_request(#{
        tenant_id => <<"sample_tenant_id">>,
        receipt_id => <<"sample_receipt_id">>,
        replay_request_hash => <<"sample_replay_request_hash">>
    });
sample(receipt_replay_request, minimal) ->
    beam4pm_types:new_receipt_replay_request(#{
        tenant_id => <<"sample_tenant_id">>,
        receipt_id => <<"sample_receipt_id">>,
        replay_request_hash => <<"sample_replay_request_hash">>
    });
sample(receipt_required_gate, full) ->
    beam4pm_types:new_receipt_required_gate(#{
        action_id => <<"sample_action_id">>,
        receipt_contract_id => <<"sample_receipt_contract_id">>,
        standing => <<"sample_standing">>
    });
sample(receipt_required_gate, minimal) ->
    beam4pm_types:new_receipt_required_gate(#{
        action_id => <<"sample_action_id">>,
        receipt_contract_id => <<"sample_receipt_contract_id">>,
        standing => <<"sample_standing">>
    });
sample(receipt_signature, full) ->
    beam4pm_types:new_receipt_signature(#{
        tenant_id => <<"sample_tenant_id">>,
        receipt_id => <<"sample_receipt_id">>,
        signature_hash => <<"sample_signature_hash">>
    });
sample(receipt_signature, minimal) ->
    beam4pm_types:new_receipt_signature(#{
        tenant_id => <<"sample_tenant_id">>,
        receipt_id => <<"sample_receipt_id">>,
        signature_hash => <<"sample_signature_hash">>
    });
sample(receipt_subject_binding, full) ->
    beam4pm_types:new_receipt_subject_binding(#{
        tenant_id => <<"sample_tenant_id">>,
        workload_id => <<"sample_workload_id">>,
        binding_hash => <<"sample_binding_hash">>
    });
sample(receipt_subject_binding, minimal) ->
    beam4pm_types:new_receipt_subject_binding(#{
        tenant_id => <<"sample_tenant_id">>,
        workload_id => <<"sample_workload_id">>,
        binding_hash => <<"sample_binding_hash">>
    });
sample(receipt_verification, full) ->
    beam4pm_types:new_receipt_verification(#{
        tenant_id => <<"sample_tenant_id">>,
        receipt_id => <<"sample_receipt_id">>,
        verification_hash => <<"sample_verification_hash">>
    });
sample(receipt_verification, minimal) ->
    beam4pm_types:new_receipt_verification(#{
        tenant_id => <<"sample_tenant_id">>,
        receipt_id => <<"sample_receipt_id">>,
        verification_hash => <<"sample_verification_hash">>
    });
sample(recovery_plan, full) ->
    beam4pm_types:new_recovery_plan(#{
        recovery_plan_id => <<"sample_recovery_plan_id">>,
        account_id => <<"sample_account_id">>,
        recovery_time_hours => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(recovery_plan, minimal) ->
    beam4pm_types:new_recovery_plan(#{
        recovery_plan_id => <<"sample_recovery_plan_id">>,
        account_id => <<"sample_account_id">>,
        recovery_time_hours => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(recovery_point_receipt, full) ->
    beam4pm_types:new_recovery_point_receipt(#{
        tenant_id => <<"sample_tenant_id">>,
        incident_id => <<"sample_incident_id">>,
        recovery_hash => <<"sample_recovery_hash">>
    });
sample(recovery_point_receipt, minimal) ->
    beam4pm_types:new_recovery_point_receipt(#{
        tenant_id => <<"sample_tenant_id">>,
        incident_id => <<"sample_incident_id">>,
        recovery_hash => <<"sample_recovery_hash">>
    });
sample(recovery_subtask, full) ->
    beam4pm_types:new_recovery_subtask(#{
        subtask_id => <<"sample_subtask_id">>,
        trigger_outcome => <<"sample_trigger_outcome">>,
        recovery_goal_digest => <<"sample_recovery_goal_digest">>
    });
sample(recovery_subtask, minimal) ->
    beam4pm_types:new_recovery_subtask(#{
        subtask_id => <<"sample_subtask_id">>,
        trigger_outcome => <<"sample_trigger_outcome">>,
        recovery_goal_digest => <<"sample_recovery_goal_digest">>
    });
sample(recovery_time_receipt, full) ->
    beam4pm_types:new_recovery_time_receipt(#{
        tenant_id => <<"sample_tenant_id">>,
        incident_id => <<"sample_incident_id">>,
        recovery_hash => <<"sample_recovery_hash">>
    });
sample(recovery_time_receipt, minimal) ->
    beam4pm_types:new_recovery_time_receipt(#{
        tenant_id => <<"sample_tenant_id">>,
        incident_id => <<"sample_incident_id">>,
        recovery_hash => <<"sample_recovery_hash">>
    });
sample(refund_policy, full) ->
    beam4pm_types:new_refund_policy(#{
        refund_policy_id => <<"sample_refund_policy_id">>,
        account_id => <<"sample_account_id">>,
        refund_method => <<"sample_refund_method">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(refund_policy, minimal) ->
    beam4pm_types:new_refund_policy(#{
        refund_policy_id => <<"sample_refund_policy_id">>,
        account_id => <<"sample_account_id">>,
        refund_method => <<"sample_refund_method">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(refusal_boundary_observation, full) ->
    beam4pm_types:new_refusal_boundary_observation(#{
        subject_id => <<"sample_subject_id">>,
        refusal_code => <<"sample_refusal_code">>,
        authority_boundary => <<"sample_authority_boundary">>,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(refusal_boundary_observation, minimal) ->
    beam4pm_types:new_refusal_boundary_observation(#{
        subject_id => <<"sample_subject_id">>,
        refusal_code => <<"sample_refusal_code">>,
        authority_boundary => <<"sample_authority_boundary">>,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(refusal_threshold, full) ->
    beam4pm_types:new_refusal_threshold(#{
        threshold_id => <<"sample_threshold_id">>,
        refusal_class => <<"sample_refusal_class">>,
        limit => 3.5,
        metric_id => <<"sample_metric_id">>,
        authority_binding => <<"sample_authority_binding">>
    });
sample(refusal_threshold, minimal) ->
    beam4pm_types:new_refusal_threshold(#{
        threshold_id => <<"sample_threshold_id">>,
        refusal_class => <<"sample_refusal_class">>,
        limit => 3.5,
        metric_id => <<"sample_metric_id">>,
        authority_binding => <<"sample_authority_binding">>
    });
sample(region_pricing_policy, full) ->
    beam4pm_types:new_region_pricing_policy(#{
        region_pricing_policy_id => <<"sample_region_pricing_policy_id">>,
        account_id => <<"sample_account_id">>,
        region_code => <<"sample_region_code">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(region_pricing_policy, minimal) ->
    beam4pm_types:new_region_pricing_policy(#{
        region_pricing_policy_id => <<"sample_region_pricing_policy_id">>,
        account_id => <<"sample_account_id">>,
        region_code => <<"sample_region_code">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(regression_detector, full) ->
    beam4pm_types:new_regression_detector(#{
        detection_id => <<"sample_detection_id">>,
        baseline_policy_id => <<"sample_baseline_policy_id">>,
        candidate_policy_id => <<"sample_candidate_policy_id">>,
        metric_id => <<"sample_metric_id">>,
        regression_delta => 3.5,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(regression_detector, minimal) ->
    beam4pm_types:new_regression_detector(#{
        detection_id => <<"sample_detection_id">>,
        baseline_policy_id => <<"sample_baseline_policy_id">>,
        candidate_policy_id => <<"sample_candidate_policy_id">>,
        metric_id => <<"sample_metric_id">>,
        regression_delta => 3.5,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(regression_refusal, full) ->
    beam4pm_types:new_regression_refusal(#{
        refusal_id => <<"sample_refusal_id">>,
        candidate_policy_id => <<"sample_candidate_policy_id">>,
        regression_detection_id => <<"sample_regression_detection_id">>,
        acceptance_threshold => 3.5,
        observed_regression => 3.5,
        authority_ceiling => <<"sample_authority_ceiling">>
    });
sample(regression_refusal, minimal) ->
    beam4pm_types:new_regression_refusal(#{
        refusal_id => <<"sample_refusal_id">>,
        candidate_policy_id => <<"sample_candidate_policy_id">>,
        regression_detection_id => <<"sample_regression_detection_id">>,
        acceptance_threshold => 3.5,
        observed_regression => 3.5,
        authority_ceiling => <<"sample_authority_ceiling">>
    });
sample(remaining_time_estimate_contract, full) ->
    beam4pm_types:new_remaining_time_estimate_contract(#{
        estimate_id => <<"sample_estimate_id">>,
        case_state_digest => <<"sample_case_state_digest">>,
        interval_digest => <<"sample_interval_digest">>
    });
sample(remaining_time_estimate_contract, minimal) ->
    beam4pm_types:new_remaining_time_estimate_contract(#{
        estimate_id => <<"sample_estimate_id">>,
        case_state_digest => <<"sample_case_state_digest">>,
        interval_digest => <<"sample_interval_digest">>
    });
sample(remediation_sla_evidence, full) ->
    beam4pm_types:new_remediation_sla_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        remediation_due_at => <<"2026-08-29T12:00:00.123456Z">>,
        observed_result => sample_atom
    });
sample(remediation_sla_evidence, minimal) ->
    beam4pm_types:new_remediation_sla_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        remediation_due_at => <<"2026-08-29T12:00:00.123456Z">>,
        observed_result => sample_atom
    });
sample(renewal_evidence, full) ->
    beam4pm_types:new_renewal_evidence(#{
        renewal_evidence_id => <<"sample_renewal_evidence_id">>,
        account_id => <<"sample_account_id">>,
        renewal_value => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(renewal_evidence, minimal) ->
    beam4pm_types:new_renewal_evidence(#{
        renewal_evidence_id => <<"sample_renewal_evidence_id">>,
        account_id => <<"sample_account_id">>,
        renewal_value => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(renewal_health, full) ->
    beam4pm_types:new_renewal_health(#{
        renewal_health_id => <<"sample_renewal_health_id">>,
        account_id => <<"sample_account_id">>,
        renewal_health_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(renewal_health, minimal) ->
    beam4pm_types:new_renewal_health(#{
        renewal_health_id => <<"sample_renewal_health_id">>,
        account_id => <<"sample_account_id">>,
        renewal_health_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(renewal_option, full) ->
    beam4pm_types:new_renewal_option(#{
        option_id => <<"sample_option_id">>,
        subscription_id => <<"sample_subscription_id">>,
        term_months => 42,
        notice_by => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(renewal_option, minimal) ->
    beam4pm_types:new_renewal_option(#{
        option_id => <<"sample_option_id">>,
        subscription_id => <<"sample_subscription_id">>,
        term_months => 42,
        notice_by => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(renewal_risk, full) ->
    beam4pm_types:new_renewal_risk(#{
        renewal_risk_id => <<"sample_renewal_risk_id">>,
        account_id => <<"sample_account_id">>,
        renewal_risk_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(renewal_risk, minimal) ->
    beam4pm_types:new_renewal_risk(#{
        renewal_risk_id => <<"sample_renewal_risk_id">>,
        account_id => <<"sample_account_id">>,
        renewal_risk_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(renewal_term_admission, full) ->
    beam4pm_types:new_renewal_term_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        renewal_term => <<"sample_renewal_term">>,
        decision => <<"sample_decision">>
    });
sample(renewal_term_admission, minimal) ->
    beam4pm_types:new_renewal_term_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        renewal_term => <<"sample_renewal_term">>,
        decision => <<"sample_decision">>
    });
sample(repair_effectiveness_measurement, full) ->
    beam4pm_types:new_repair_effectiveness_measurement(#{
        measurement_id => <<"sample_measurement_id">>,
        repair_id => <<"sample_repair_id">>,
        baseline_failure_rate => 3.5,
        post_repair_failure_rate => 3.5,
        evaluation_window_digest => <<"sample_evaluation_window_digest">>
    });
sample(repair_effectiveness_measurement, minimal) ->
    beam4pm_types:new_repair_effectiveness_measurement(#{
        measurement_id => <<"sample_measurement_id">>,
        repair_id => <<"sample_repair_id">>,
        baseline_failure_rate => 3.5,
        post_repair_failure_rate => 3.5,
        evaluation_window_digest => <<"sample_evaluation_window_digest">>
    });
sample(replay_environment_identity, full) ->
    beam4pm_types:new_replay_environment_identity(#{
        tenant_id => <<"sample_tenant_id">>,
        environment_id => <<"sample_environment_id">>,
        identity_hash => <<"sample_identity_hash">>
    });
sample(replay_environment_identity, minimal) ->
    beam4pm_types:new_replay_environment_identity(#{
        tenant_id => <<"sample_tenant_id">>,
        environment_id => <<"sample_environment_id">>,
        identity_hash => <<"sample_identity_hash">>
    });
sample(repository_ancestry_observation, full) ->
    beam4pm_types:new_repository_ancestry_observation(#{
        ancestor_sha => <<"sample_ancestor_sha">>,
        descendant_sha => <<"sample_descendant_sha">>,
        relation => <<"sample_relation">>,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(repository_ancestry_observation, minimal) ->
    beam4pm_types:new_repository_ancestry_observation(#{
        ancestor_sha => <<"sample_ancestor_sha">>,
        descendant_sha => <<"sample_descendant_sha">>,
        relation => <<"sample_relation">>,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(repository_default_branch_observation, full) ->
    beam4pm_types:new_repository_default_branch_observation(#{
        repository_id => <<"sample_repository_id">>,
        default_branch => <<"sample_default_branch">>,
        head_sha => <<"sample_head_sha">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(repository_default_branch_observation, minimal) ->
    beam4pm_types:new_repository_default_branch_observation(#{
        repository_id => <<"sample_repository_id">>,
        default_branch => <<"sample_default_branch">>,
        head_sha => <<"sample_head_sha">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(repository_exact_head_observation, full) ->
    beam4pm_types:new_repository_exact_head_observation(#{
        repository_id => <<"sample_repository_id">>,
        branch_name => <<"sample_branch_name">>,
        head_sha => <<"sample_head_sha">>,
        previous_head_sha => <<"sample_previous_head_sha">>
    });
sample(repository_exact_head_observation, minimal) ->
    beam4pm_types:new_repository_exact_head_observation(#{
        repository_id => <<"sample_repository_id">>,
        branch_name => <<"sample_branch_name">>,
        head_sha => <<"sample_head_sha">>,
        previous_head_sha => <<"sample_previous_head_sha">>
    });
sample(repository_worktree_state_observation, full) ->
    beam4pm_types:new_repository_worktree_state_observation(#{
        repository_id => <<"sample_repository_id">>,
        worktree_hash => <<"sample_worktree_hash">>,
        dirty_path_count => 42,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(repository_worktree_state_observation, minimal) ->
    beam4pm_types:new_repository_worktree_state_observation(#{
        repository_id => <<"sample_repository_id">>,
        worktree_hash => <<"sample_worktree_hash">>,
        dirty_path_count => 42,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(reproducible_build_evidence, full) ->
    beam4pm_types:new_reproducible_build_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        build_digest => <<"sample_build_digest">>,
        observed_result => sample_atom
    });
sample(reproducible_build_evidence, minimal) ->
    beam4pm_types:new_reproducible_build_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        build_digest => <<"sample_build_digest">>,
        observed_result => sample_atom
    });
sample(reseller_authorization, full) ->
    beam4pm_types:new_reseller_authorization(#{
        authorization_id => <<"sample_authorization_id">>,
        reseller_id => <<"sample_reseller_id">>,
        sku => <<"sample_sku">>,
        status => sample_atom
    });
sample(reseller_authorization, minimal) ->
    beam4pm_types:new_reseller_authorization(#{
        authorization_id => <<"sample_authorization_id">>,
        reseller_id => <<"sample_reseller_id">>,
        sku => <<"sample_sku">>,
        status => sample_atom
    });
sample(reserve_work_promotion, full) ->
    beam4pm_types:new_reserve_work_promotion(#{
        blocked_work_id => <<"sample_blocked_work_id">>,
        reserve_set_hash => <<"sample_reserve_set_hash">>,
        promoted_work_id => <<"sample_promoted_work_id">>
    });
sample(reserve_work_promotion, minimal) ->
    beam4pm_types:new_reserve_work_promotion(#{
        blocked_work_id => <<"sample_blocked_work_id">>,
        reserve_set_hash => <<"sample_reserve_set_hash">>,
        promoted_work_id => <<"sample_promoted_work_id">>
    });
sample(residency_evidence, full) ->
    beam4pm_types:new_residency_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        region => <<"sample_region">>,
        observed_result => sample_atom
    });
sample(residency_evidence, minimal) ->
    beam4pm_types:new_residency_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        region => <<"sample_region">>,
        observed_result => sample_atom
    });
sample(resource_allocation, full) ->
    beam4pm_types:new_resource_allocation(#{
        resource_id => <<"sample_resource_id">>,
        activity => <<"sample_activity">>,
        event_id => <<"sample_event_id">>
    });
sample(resource_allocation, minimal) ->
    beam4pm_types:new_resource_allocation(#{
        resource_id => <<"sample_resource_id">>,
        activity => <<"sample_activity">>,
        event_id => <<"sample_event_id">>
    });
sample(resource_capacity_plan, full) ->
    beam4pm_types:new_resource_capacity_plan(#{
        episode_id => <<"sample_episode_id">>,
        resource_pool_hash => <<"sample_resource_pool_hash">>,
        allocation_hash => <<"sample_allocation_hash">>
    });
sample(resource_capacity_plan, minimal) ->
    beam4pm_types:new_resource_capacity_plan(#{
        episode_id => <<"sample_episode_id">>,
        resource_pool_hash => <<"sample_resource_pool_hash">>,
        allocation_hash => <<"sample_allocation_hash">>
    });
sample(retention_policy_evidence, full) ->
    beam4pm_types:new_retention_policy_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        retention_days => 42,
        observed_result => sample_atom
    });
sample(retention_policy_evidence, minimal) ->
    beam4pm_types:new_retention_policy_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        retention_days => 42,
        observed_result => sample_atom
    });
sample(retention_pricing_policy, full) ->
    beam4pm_types:new_retention_pricing_policy(#{
        retention_pricing_policy_id => <<"sample_retention_pricing_policy_id">>,
        account_id => <<"sample_account_id">>,
        retention_days => 42,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(retention_pricing_policy, minimal) ->
    beam4pm_types:new_retention_pricing_policy(#{
        retention_pricing_policy_id => <<"sample_retention_pricing_policy_id">>,
        account_id => <<"sample_account_id">>,
        retention_days => 42,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(revenue_attribution, full) ->
    beam4pm_types:new_revenue_attribution(#{
        revenue_attribution_id => <<"sample_revenue_attribution_id">>,
        account_id => <<"sample_account_id">>,
        attributed_revenue => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(revenue_attribution, minimal) ->
    beam4pm_types:new_revenue_attribution(#{
        revenue_attribution_id => <<"sample_revenue_attribution_id">>,
        account_id => <<"sample_account_id">>,
        attributed_revenue => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(revenue_contract_admission, full) ->
    beam4pm_types:new_revenue_contract_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        contract_id => <<"sample_contract_id">>,
        admission_receipt_hash => <<"sample_admission_receipt_hash">>
    });
sample(revenue_contract_admission, minimal) ->
    beam4pm_types:new_revenue_contract_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        contract_id => <<"sample_contract_id">>,
        admission_receipt_hash => <<"sample_admission_receipt_hash">>
    });
sample(revenue_schedule_assumption, full) ->
    beam4pm_types:new_revenue_schedule_assumption(#{
        opportunity_id => <<"sample_opportunity_id">>,
        schedule_id => <<"sample_schedule_id">>,
        assumption_evidence_hash => <<"sample_assumption_evidence_hash">>
    });
sample(revenue_schedule_assumption, minimal) ->
    beam4pm_types:new_revenue_schedule_assumption(#{
        opportunity_id => <<"sample_opportunity_id">>,
        schedule_id => <<"sample_schedule_id">>,
        assumption_evidence_hash => <<"sample_assumption_evidence_hash">>
    });
sample(reversibility_weight, full) ->
    beam4pm_types:new_reversibility_weight(#{
        action_id => <<"sample_action_id">>,
        rollback_id => <<"sample_rollback_id">>,
        weight => <<"sample_weight">>
    });
sample(reversibility_weight, minimal) ->
    beam4pm_types:new_reversibility_weight(#{
        action_id => <<"sample_action_id">>,
        rollback_id => <<"sample_rollback_id">>,
        weight => <<"sample_weight">>
    });
sample(rework_cost, full) ->
    beam4pm_types:new_rework_cost(#{
        case_id => <<"sample_case_id">>,
        loop_count => 42,
        weighted_cost => 3.5
    });
sample(rework_cost, minimal) ->
    beam4pm_types:new_rework_cost(#{
        case_id => <<"sample_case_id">>,
        loop_count => 42,
        weighted_cost => 3.5
    });
sample(rfp_response_evidence, full) ->
    beam4pm_types:new_rfp_response_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        answer_set_hash => <<"sample_answer_set_hash">>,
        observed_result => sample_atom
    });
sample(rfp_response_evidence, minimal) ->
    beam4pm_types:new_rfp_response_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        answer_set_hash => <<"sample_answer_set_hash">>,
        observed_result => sample_atom
    });
sample(role_compatibility, full) ->
    beam4pm_types:new_role_compatibility(#{
        role_id => <<"sample_role_id">>,
        policy_id => <<"sample_policy_id">>,
        compatibility => <<"sample_compatibility">>
    });
sample(role_compatibility, minimal) ->
    beam4pm_types:new_role_compatibility(#{
        role_id => <<"sample_role_id">>,
        policy_id => <<"sample_policy_id">>,
        compatibility => <<"sample_compatibility">>
    });
sample(rollback_checkpoint, full) ->
    beam4pm_types:new_rollback_checkpoint(#{
        tenant_id => <<"sample_tenant_id">>,
        checkpoint_id => <<"sample_checkpoint_id">>,
        state_hash => <<"sample_state_hash">>
    });
sample(rollback_checkpoint, minimal) ->
    beam4pm_types:new_rollback_checkpoint(#{
        tenant_id => <<"sample_tenant_id">>,
        checkpoint_id => <<"sample_checkpoint_id">>,
        state_hash => <<"sample_state_hash">>
    });
sample(rollback_decision, full) ->
    beam4pm_types:new_rollback_decision(#{
        rollback_decision_id => <<"sample_rollback_decision_id">>,
        account_id => <<"sample_account_id">>,
        rollback_result => <<"sample_rollback_result">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(rollback_decision, minimal) ->
    beam4pm_types:new_rollback_decision(#{
        rollback_decision_id => <<"sample_rollback_decision_id">>,
        account_id => <<"sample_account_id">>,
        rollback_result => <<"sample_rollback_result">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(rollback_evidence, full) ->
    beam4pm_types:new_rollback_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        rollback_digest => <<"sample_rollback_digest">>,
        observed_result => sample_atom
    });
sample(rollback_evidence, minimal) ->
    beam4pm_types:new_rollback_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        rollback_digest => <<"sample_rollback_digest">>,
        observed_result => sample_atom
    });
sample(rollback_outcome_learning, full) ->
    beam4pm_types:new_rollback_outcome_learning(#{
        learning_id => <<"sample_learning_id">>,
        rollback_id => <<"sample_rollback_id">>,
        pre_rollback_state_digest => <<"sample_pre_rollback_state_digest">>,
        post_rollback_state_digest => <<"sample_post_rollback_state_digest">>,
        recovery_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(rollback_outcome_learning, minimal) ->
    beam4pm_types:new_rollback_outcome_learning(#{
        learning_id => <<"sample_learning_id">>,
        rollback_id => <<"sample_rollback_id">>,
        pre_rollback_state_digest => <<"sample_pre_rollback_state_digest">>,
        post_rollback_state_digest => <<"sample_post_rollback_state_digest">>,
        recovery_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(rolling_upgrade_plan, full) ->
    beam4pm_types:new_rolling_upgrade_plan(#{
        tenant_id => <<"sample_tenant_id">>,
        upgrade_id => <<"sample_upgrade_id">>,
        plan_hash => <<"sample_plan_hash">>
    });
sample(rolling_upgrade_plan, minimal) ->
    beam4pm_types:new_rolling_upgrade_plan(#{
        tenant_id => <<"sample_tenant_id">>,
        upgrade_id => <<"sample_upgrade_id">>,
        plan_hash => <<"sample_plan_hash">>
    });
sample(root_cause_pattern, full) ->
    beam4pm_types:new_root_cause_pattern(#{
        pattern_id => <<"sample_pattern_id">>,
        failure_class => <<"sample_failure_class">>,
        causal_graph_digest => <<"sample_causal_graph_digest">>,
        confirmed_reproducer_digest => <<"sample_confirmed_reproducer_digest">>,
        reusable_scope => <<"sample_reusable_scope">>
    });
sample(root_cause_pattern, minimal) ->
    beam4pm_types:new_root_cause_pattern(#{
        pattern_id => <<"sample_pattern_id">>,
        failure_class => <<"sample_failure_class">>,
        causal_graph_digest => <<"sample_causal_graph_digest">>,
        confirmed_reproducer_digest => <<"sample_confirmed_reproducer_digest">>,
        reusable_scope => <<"sample_reusable_scope">>
    });
sample(root_cause_reuse_decision, full) ->
    beam4pm_types:new_root_cause_reuse_decision(#{
        decision_id => <<"sample_decision_id">>,
        pattern_id => <<"sample_pattern_id">>,
        current_failure_id => <<"sample_current_failure_id">>,
        similarity_score => 3.5,
        applicability_evidence_digest => <<"sample_applicability_evidence_digest">>
    });
sample(root_cause_reuse_decision, minimal) ->
    beam4pm_types:new_root_cause_reuse_decision(#{
        decision_id => <<"sample_decision_id">>,
        pattern_id => <<"sample_pattern_id">>,
        current_failure_id => <<"sample_current_failure_id">>,
        similarity_score => 3.5,
        applicability_evidence_digest => <<"sample_applicability_evidence_digest">>
    });
sample(runtime_health_observation, full) ->
    beam4pm_types:new_runtime_health_observation(#{
        runtime_id => <<"sample_runtime_id">>,
        health_state => <<"sample_health_state">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(runtime_health_observation, minimal) ->
    beam4pm_types:new_runtime_health_observation(#{
        runtime_id => <<"sample_runtime_id">>,
        health_state => <<"sample_health_state">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(runtime_policy_decision, full) ->
    beam4pm_types:new_runtime_policy_decision(#{
        tenant_id => <<"sample_tenant_id">>,
        decision_id => <<"sample_decision_id">>,
        policy_hash => <<"sample_policy_hash">>
    });
sample(runtime_policy_decision, minimal) ->
    beam4pm_types:new_runtime_policy_decision(#{
        tenant_id => <<"sample_tenant_id">>,
        decision_id => <<"sample_decision_id">>,
        policy_hash => <<"sample_policy_hash">>
    });
sample(sanctions_screening_result, full) ->
    beam4pm_types:new_sanctions_screening_result(#{
        account_id => <<"sample_account_id">>,
        screening_id => <<"sample_screening_id">>,
        screening_result => <<"sample_screening_result">>
    });
sample(sanctions_screening_result, minimal) ->
    beam4pm_types:new_sanctions_screening_result(#{
        account_id => <<"sample_account_id">>,
        screening_id => <<"sample_screening_id">>,
        screening_result => <<"sample_screening_result">>
    });
sample(sandbox_entitlement, full) ->
    beam4pm_types:new_sandbox_entitlement(#{
        sandbox_entitlement_id => <<"sample_sandbox_entitlement_id">>,
        account_id => <<"sample_account_id">>,
        sandbox_limit => 42,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(sandbox_entitlement, minimal) ->
    beam4pm_types:new_sandbox_entitlement(#{
        sandbox_entitlement_id => <<"sample_sandbox_entitlement_id">>,
        account_id => <<"sample_account_id">>,
        sandbox_limit => 42,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(saturation_detection, full) ->
    beam4pm_types:new_saturation_detection(#{
        detection_id => <<"sample_detection_id">>,
        curriculum_id => <<"sample_curriculum_id">>,
        recent_gain => 3.5,
        gain_slope => 3.5,
        novelty_floor => 3.5,
        coverage_digest => <<"sample_coverage_digest">>
    });
sample(saturation_detection, minimal) ->
    beam4pm_types:new_saturation_detection(#{
        detection_id => <<"sample_detection_id">>,
        curriculum_id => <<"sample_curriculum_id">>,
        recent_gain => 3.5,
        gain_slope => 3.5,
        novelty_floor => 3.5,
        coverage_digest => <<"sample_coverage_digest">>
    });
sample(sbom_inventory_evidence, full) ->
    beam4pm_types:new_sbom_inventory_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        component_count => 42,
        observed_result => sample_atom
    });
sample(sbom_inventory_evidence, minimal) ->
    beam4pm_types:new_sbom_inventory_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        component_count => 42,
        observed_result => sample_atom
    });
sample(scheduling_priority_score, full) ->
    beam4pm_types:new_scheduling_priority_score(#{
        score_id => <<"sample_score_id">>,
        work_item_id => <<"sample_work_item_id">>,
        component_digest => <<"sample_component_digest">>
    });
sample(scheduling_priority_score, minimal) ->
    beam4pm_types:new_scheduling_priority_score(#{
        score_id => <<"sample_score_id">>,
        work_item_id => <<"sample_work_item_id">>,
        component_digest => <<"sample_component_digest">>
    });
sample(seat_pricing_policy, full) ->
    beam4pm_types:new_seat_pricing_policy(#{
        seat_pricing_policy_id => <<"sample_seat_pricing_policy_id">>,
        account_id => <<"sample_account_id">>,
        seat_price => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(seat_pricing_policy, minimal) ->
    beam4pm_types:new_seat_pricing_policy(#{
        seat_pricing_policy_id => <<"sample_seat_pricing_policy_id">>,
        account_id => <<"sample_account_id">>,
        seat_price => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(second_pass_byte_identity_observation, full) ->
    beam4pm_types:new_second_pass_byte_identity_observation(#{
        first_tree_digest => <<"sample_first_tree_digest">>,
        second_tree_digest => <<"sample_second_tree_digest">>,
        byte_identity => true,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(second_pass_byte_identity_observation, minimal) ->
    beam4pm_types:new_second_pass_byte_identity_observation(#{
        first_tree_digest => <<"sample_first_tree_digest">>,
        second_tree_digest => <<"sample_second_tree_digest">>,
        byte_identity => true,
        receipt_digest => <<"sample_receipt_digest">>
    });
sample(second_run_identity_objective, full) ->
    beam4pm_types:new_second_run_identity_objective(#{
        subject_id => <<"sample_subject_id">>,
        first_tree_hash => <<"sample_first_tree_hash">>,
        second_tree_hash => <<"sample_second_tree_hash">>
    });
sample(second_run_identity_objective, minimal) ->
    beam4pm_types:new_second_run_identity_objective(#{
        subject_id => <<"sample_subject_id">>,
        first_tree_hash => <<"sample_first_tree_hash">>,
        second_tree_hash => <<"sample_second_tree_hash">>
    });
sample(secret_boundary_evidence, full) ->
    beam4pm_types:new_secret_boundary_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        secret_source => <<"sample_secret_source">>,
        observed_result => sample_atom
    });
sample(secret_boundary_evidence, minimal) ->
    beam4pm_types:new_secret_boundary_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        secret_source => <<"sample_secret_source">>,
        observed_result => sample_atom
    });
sample(security_addendum_state, full) ->
    beam4pm_types:new_security_addendum_state(#{
        opportunity_id => <<"sample_opportunity_id">>,
        addendum_id => <<"sample_addendum_id">>,
        addendum_state => <<"sample_addendum_state">>
    });
sample(security_addendum_state, minimal) ->
    beam4pm_types:new_security_addendum_state(#{
        opportunity_id => <<"sample_opportunity_id">>,
        addendum_id => <<"sample_addendum_id">>,
        addendum_state => <<"sample_addendum_state">>
    });
sample(security_blocker, full) ->
    beam4pm_types:new_security_blocker(#{
        opportunity_id => <<"sample_opportunity_id">>,
        blocker_id => <<"sample_blocker_id">>,
        refusal_code => <<"sample_refusal_code">>
    });
sample(security_blocker, minimal) ->
    beam4pm_types:new_security_blocker(#{
        opportunity_id => <<"sample_opportunity_id">>,
        blocker_id => <<"sample_blocker_id">>,
        refusal_code => <<"sample_refusal_code">>
    });
sample(security_readiness, full) ->
    beam4pm_types:new_security_readiness(#{
        security_readiness_id => <<"sample_security_readiness_id">>,
        account_id => <<"sample_account_id">>,
        control_coverage => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(security_readiness, minimal) ->
    beam4pm_types:new_security_readiness(#{
        security_readiness_id => <<"sample_security_readiness_id">>,
        account_id => <<"sample_account_id">>,
        control_coverage => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(seeded_evaluation, full) ->
    beam4pm_types:new_seeded_evaluation(#{
        evaluation_id => <<"sample_evaluation_id">>,
        seed => <<"sample_seed">>,
        dataset_digest => <<"sample_dataset_digest">>,
        policy_id => <<"sample_policy_id">>,
        score => 3.5,
        replay_digest => <<"sample_replay_digest">>
    });
sample(seeded_evaluation, minimal) ->
    beam4pm_types:new_seeded_evaluation(#{
        evaluation_id => <<"sample_evaluation_id">>,
        seed => <<"sample_seed">>,
        dataset_digest => <<"sample_dataset_digest">>,
        policy_id => <<"sample_policy_id">>,
        score => 3.5,
        replay_digest => <<"sample_replay_digest">>
    });
sample(semantic_drift_observation, full) ->
    beam4pm_types:new_semantic_drift_observation(#{
        subject_id => <<"sample_subject_id">>,
        prior_semantic_digest => <<"sample_prior_semantic_digest">>,
        current_semantic_digest => <<"sample_current_semantic_digest">>,
        drift_class => <<"sample_drift_class">>
    });
sample(semantic_drift_observation, minimal) ->
    beam4pm_types:new_semantic_drift_observation(#{
        subject_id => <<"sample_subject_id">>,
        prior_semantic_digest => <<"sample_prior_semantic_digest">>,
        current_semantic_digest => <<"sample_current_semantic_digest">>,
        drift_class => <<"sample_drift_class">>
    });
sample(service_credit, full) ->
    beam4pm_types:new_service_credit(#{
        credit_id => <<"sample_credit_id">>,
        slo_id => <<"sample_slo_id">>,
        amount => 3.5,
        currency => <<"sample_currency">>
    });
sample(service_credit, minimal) ->
    beam4pm_types:new_service_credit(#{
        credit_id => <<"sample_credit_id">>,
        slo_id => <<"sample_slo_id">>,
        amount => 3.5,
        currency => <<"sample_currency">>
    });
sample(service_credit_admission, full) ->
    beam4pm_types:new_service_credit_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        service_credit_id => <<"sample_service_credit_id">>,
        decision => <<"sample_decision">>
    });
sample(service_credit_admission, minimal) ->
    beam4pm_types:new_service_credit_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        service_credit_id => <<"sample_service_credit_id">>,
        decision => <<"sample_decision">>
    });
sample(service_credit_ledger, full) ->
    beam4pm_types:new_service_credit_ledger(#{
        service_credit_ledger_id => <<"sample_service_credit_ledger_id">>,
        account_id => <<"sample_account_id">>,
        credit_amount => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(service_credit_ledger, minimal) ->
    beam4pm_types:new_service_credit_ledger(#{
        service_credit_ledger_id => <<"sample_service_credit_ledger_id">>,
        account_id => <<"sample_account_id">>,
        credit_amount => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(service_health_snapshot, full) ->
    beam4pm_types:new_service_health_snapshot(#{
        tenant_id => <<"sample_tenant_id">>,
        service_id => <<"sample_service_id">>,
        snapshot_hash => <<"sample_snapshot_hash">>
    });
sample(service_health_snapshot, minimal) ->
    beam4pm_types:new_service_health_snapshot(#{
        tenant_id => <<"sample_tenant_id">>,
        service_id => <<"sample_service_id">>,
        snapshot_hash => <<"sample_snapshot_hash">>
    });
sample(service_level_objective, full) ->
    beam4pm_types:new_service_level_objective(#{
        slo_id => <<"sample_slo_id">>,
        contract_id => <<"sample_contract_id">>,
        target_percent => 3.5,
        measurement_window => sample_atom
    });
sample(service_level_objective, minimal) ->
    beam4pm_types:new_service_level_objective(#{
        slo_id => <<"sample_slo_id">>,
        contract_id => <<"sample_contract_id">>,
        target_percent => 3.5,
        measurement_window => sample_atom
    });
sample(service_slo_contract, full) ->
    beam4pm_types:new_service_slo_contract(#{
        tenant_id => <<"sample_tenant_id">>,
        slo_id => <<"sample_slo_id">>,
        contract_hash => <<"sample_contract_hash">>
    });
sample(service_slo_contract, minimal) ->
    beam4pm_types:new_service_slo_contract(#{
        tenant_id => <<"sample_tenant_id">>,
        slo_id => <<"sample_slo_id">>,
        contract_hash => <<"sample_contract_hash">>
    });
sample(service_span, full) ->
    beam4pm_types:new_service_span(#{
        span_id => <<"sample_span_id">>,
        service_name => <<"sample_service_name">>,
        duration_ms => 42,
        parent_span_id => <<"sample_parent_span_id">>,
        trace_id => <<"sample_trace_id">>,
        start_time => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(service_span, minimal) ->
    beam4pm_types:new_service_span(#{
        span_id => <<"sample_span_id">>,
        service_name => <<"sample_service_name">>,
        duration_ms => 42,
        trace_id => <<"sample_trace_id">>,
        start_time => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(shadow_challenger_execution, full) ->
    beam4pm_types:new_shadow_challenger_execution(#{
        execution_id => <<"sample_execution_id">>,
        incumbent_policy_id => <<"sample_incumbent_policy_id">>,
        challenger_policy_id => <<"sample_challenger_policy_id">>,
        shared_observation_digest => <<"sample_shared_observation_digest">>,
        shadow_receipt_digest => <<"sample_shadow_receipt_digest">>,
        authority_mode => <<"sample_authority_mode">>
    });
sample(shadow_challenger_execution, minimal) ->
    beam4pm_types:new_shadow_challenger_execution(#{
        execution_id => <<"sample_execution_id">>,
        incumbent_policy_id => <<"sample_incumbent_policy_id">>,
        challenger_policy_id => <<"sample_challenger_policy_id">>,
        shared_observation_digest => <<"sample_shared_observation_digest">>,
        shadow_receipt_digest => <<"sample_shadow_receipt_digest">>,
        authority_mode => <<"sample_authority_mode">>
    });
sample(showback_allocation, full) ->
    beam4pm_types:new_showback_allocation(#{
        allocation_id => <<"sample_allocation_id">>,
        project_id => <<"sample_project_id">>,
        metric_name => <<"sample_metric_name">>,
        quantity => 3.5
    });
sample(showback_allocation, minimal) ->
    beam4pm_types:new_showback_allocation(#{
        allocation_id => <<"sample_allocation_id">>,
        project_id => <<"sample_project_id">>,
        metric_name => <<"sample_metric_name">>,
        quantity => 3.5
    });
sample(signature_evidence, full) ->
    beam4pm_types:new_signature_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        signature_identity => <<"sample_signature_identity">>,
        observed_result => sample_atom
    });
sample(signature_evidence, minimal) ->
    beam4pm_types:new_signature_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        signature_identity => <<"sample_signature_identity">>,
        observed_result => sample_atom
    });
sample(sku_definition, full) ->
    beam4pm_types:new_sku_definition(#{
        sku => <<"sample_sku">>,
        edition_id => <<"sample_edition_id">>,
        billing_model => sample_atom,
        status => sample_atom
    });
sample(sku_definition, minimal) ->
    beam4pm_types:new_sku_definition(#{
        sku => <<"sample_sku">>,
        edition_id => <<"sample_edition_id">>,
        billing_model => sample_atom,
        status => sample_atom
    });
sample(sla_offer_admission, full) ->
    beam4pm_types:new_sla_offer_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        sla_offer_id => <<"sample_sla_offer_id">>,
        decision => <<"sample_decision">>
    });
sample(sla_offer_admission, minimal) ->
    beam4pm_types:new_sla_offer_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        sla_offer_id => <<"sample_sla_offer_id">>,
        decision => <<"sample_decision">>
    });
sample(sojourn_time, full) ->
    beam4pm_types:new_sojourn_time(#{
        object_id => <<"sample_object_id">>,
        event_type => <<"sample_event_type">>,
        seconds => 3.5
    });
sample(sojourn_time, minimal) ->
    beam4pm_types:new_sojourn_time(#{
        object_id => <<"sample_object_id">>,
        event_type => <<"sample_event_type">>,
        seconds => 3.5
    });
sample(solution_fit, full) ->
    beam4pm_types:new_solution_fit(#{
        solution_fit_id => <<"sample_solution_fit_id">>,
        account_id => <<"sample_account_id">>,
        fit_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(solution_fit, minimal) ->
    beam4pm_types:new_solution_fit(#{
        solution_fit_id => <<"sample_solution_fit_id">>,
        account_id => <<"sample_account_id">>,
        fit_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(span_edge, full) ->
    beam4pm_types:new_span_edge(#{
        source_service => <<"sample_source_service">>,
        target_service => <<"sample_target_service">>,
        frequency => 42,
        evidence => sample_atom
    });
sample(span_edge, minimal) ->
    beam4pm_types:new_span_edge(#{
        source_service => <<"sample_source_service">>,
        target_service => <<"sample_target_service">>,
        frequency => 42,
        evidence => sample_atom
    });
sample(spend_drawdown, full) ->
    beam4pm_types:new_spend_drawdown(#{
        spend_drawdown_id => <<"sample_spend_drawdown_id">>,
        account_id => <<"sample_account_id">>,
        consumed_amount => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(spend_drawdown, minimal) ->
    beam4pm_types:new_spend_drawdown(#{
        spend_drawdown_id => <<"sample_spend_drawdown_id">>,
        account_id => <<"sample_account_id">>,
        consumed_amount => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(stakeholder_map, full) ->
    beam4pm_types:new_stakeholder_map(#{
        stakeholder_map_id => <<"sample_stakeholder_map_id">>,
        account_id => <<"sample_account_id">>,
        stakeholder_count => 42,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(stakeholder_map, minimal) ->
    beam4pm_types:new_stakeholder_map(#{
        stakeholder_map_id => <<"sample_stakeholder_map_id">>,
        account_id => <<"sample_account_id">>,
        stakeholder_count => 42,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(stale_plan_refusal, full) ->
    beam4pm_types:new_stale_plan_refusal(#{
        plan_id => <<"sample_plan_id">>,
        admitted_preimage_hash => <<"sample_admitted_preimage_hash">>,
        observed_preimage_hash => <<"sample_observed_preimage_hash">>
    });
sample(stale_plan_refusal, minimal) ->
    beam4pm_types:new_stale_plan_refusal(#{
        plan_id => <<"sample_plan_id">>,
        admitted_preimage_hash => <<"sample_admitted_preimage_hash">>,
        observed_preimage_hash => <<"sample_observed_preimage_hash">>
    });
sample(stale_receipt_refusal, full) ->
    beam4pm_types:new_stale_receipt_refusal(#{
        tenant_id => <<"sample_tenant_id">>,
        receipt_id => <<"sample_receipt_id">>,
        refusal_hash => <<"sample_refusal_hash">>
    });
sample(stale_receipt_refusal, minimal) ->
    beam4pm_types:new_stale_receipt_refusal(#{
        tenant_id => <<"sample_tenant_id">>,
        receipt_id => <<"sample_receipt_id">>,
        refusal_hash => <<"sample_refusal_hash">>
    });
sample(stale_subject_refusal_evidence, full) ->
    beam4pm_types:new_stale_subject_refusal_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        stale_sha => <<"sample_stale_sha">>,
        observed_result => sample_atom
    });
sample(stale_subject_refusal_evidence, minimal) ->
    beam4pm_types:new_stale_subject_refusal_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        stale_sha => <<"sample_stale_sha">>,
        observed_result => sample_atom
    });
sample(standing_state_observation, full) ->
    beam4pm_types:new_standing_state_observation(#{
        subject_id => <<"sample_subject_id">>,
        standing => <<"sample_standing">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(standing_state_observation, minimal) ->
    beam4pm_types:new_standing_state_observation(#{
        subject_id => <<"sample_subject_id">>,
        standing => <<"sample_standing">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(stopping_criterion, full) ->
    beam4pm_types:new_stopping_criterion(#{
        criterion_id => <<"sample_criterion_id">>,
        objective_id => <<"sample_objective_id">>,
        minimum_gain => 3.5,
        maximum_episodes => 42,
        evidence_window_digest => <<"sample_evidence_window_digest">>,
        authority_binding => <<"sample_authority_binding">>
    });
sample(stopping_criterion, minimal) ->
    beam4pm_types:new_stopping_criterion(#{
        criterion_id => <<"sample_criterion_id">>,
        objective_id => <<"sample_objective_id">>,
        minimum_gain => 3.5,
        maximum_episodes => 42,
        evidence_window_digest => <<"sample_evidence_window_digest">>,
        authority_binding => <<"sample_authority_binding">>
    });
sample(strong_cyclic_plan_candidate, full) ->
    beam4pm_types:new_strong_cyclic_plan_candidate(#{
        candidate_id => <<"sample_candidate_id">>,
        policy_digest => <<"sample_policy_digest">>,
        fairness_proof => <<"sample_fairness_proof">>
    });
sample(strong_cyclic_plan_candidate, minimal) ->
    beam4pm_types:new_strong_cyclic_plan_candidate(#{
        candidate_id => <<"sample_candidate_id">>,
        policy_digest => <<"sample_policy_digest">>,
        fairness_proof => <<"sample_fairness_proof">>
    });
sample(strong_plan_candidate, full) ->
    beam4pm_types:new_strong_plan_candidate(#{
        candidate_id => <<"sample_candidate_id">>,
        policy_digest => <<"sample_policy_digest">>,
        coverage_proof => <<"sample_coverage_proof">>
    });
sample(strong_plan_candidate, minimal) ->
    beam4pm_types:new_strong_plan_candidate(#{
        candidate_id => <<"sample_candidate_id">>,
        policy_digest => <<"sample_policy_digest">>,
        coverage_proof => <<"sample_coverage_proof">>
    });
sample(subject_failure_separation, full) ->
    beam4pm_types:new_subject_failure_separation(#{
        assessment_id => <<"sample_assessment_id">>,
        exact_subject_sha => <<"sample_exact_subject_sha">>,
        capsule_standing => <<"sample_capsule_standing">>,
        subject_failure_code => <<"sample_subject_failure_code">>,
        independent_reproducer_digest => <<"sample_independent_reproducer_digest">>
    });
sample(subject_failure_separation, minimal) ->
    beam4pm_types:new_subject_failure_separation(#{
        assessment_id => <<"sample_assessment_id">>,
        exact_subject_sha => <<"sample_exact_subject_sha">>,
        capsule_standing => <<"sample_capsule_standing">>,
        subject_failure_code => <<"sample_subject_failure_code">>,
        independent_reproducer_digest => <<"sample_independent_reproducer_digest">>
    });
sample(submodule_lock_observation, full) ->
    beam4pm_types:new_submodule_lock_observation(#{
        submodule_path => <<"sample_submodule_path">>,
        gitlink_sha => <<"sample_gitlink_sha">>,
        lock_sha => <<"sample_lock_sha">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(submodule_lock_observation, minimal) ->
    beam4pm_types:new_submodule_lock_observation(#{
        submodule_path => <<"sample_submodule_path">>,
        gitlink_sha => <<"sample_gitlink_sha">>,
        lock_sha => <<"sample_lock_sha">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(submodule_registration_observation, full) ->
    beam4pm_types:new_submodule_registration_observation(#{
        submodule_path => <<"sample_submodule_path">>,
        registration_state => <<"sample_registration_state">>,
        repository_url => <<"sample_repository_url">>,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(submodule_registration_observation, minimal) ->
    beam4pm_types:new_submodule_registration_observation(#{
        submodule_path => <<"sample_submodule_path">>,
        registration_state => <<"sample_registration_state">>,
        repository_url => <<"sample_repository_url">>,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(success_plan, full) ->
    beam4pm_types:new_success_plan(#{
        success_plan_id => <<"sample_success_plan_id">>,
        account_id => <<"sample_account_id">>,
        success_target => <<"sample_success_target">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(success_plan, minimal) ->
    beam4pm_types:new_success_plan(#{
        success_plan_id => <<"sample_success_plan_id">>,
        account_id => <<"sample_account_id">>,
        success_target => <<"sample_success_target">>,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(supervisor_restart_policy, full) ->
    beam4pm_types:new_supervisor_restart_policy(#{
        tenant_id => <<"sample_tenant_id">>,
        supervisor_id => <<"sample_supervisor_id">>,
        policy_hash => <<"sample_policy_hash">>
    });
sample(supervisor_restart_policy, minimal) ->
    beam4pm_types:new_supervisor_restart_policy(#{
        tenant_id => <<"sample_tenant_id">>,
        supervisor_id => <<"sample_supervisor_id">>,
        policy_hash => <<"sample_policy_hash">>
    });
sample(support_contract, full) ->
    beam4pm_types:new_support_contract(#{
        contract_id => <<"sample_contract_id">>,
        account_id => <<"sample_account_id">>,
        tier => sample_atom,
        valid_until => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(support_contract, minimal) ->
    beam4pm_types:new_support_contract(#{
        contract_id => <<"sample_contract_id">>,
        account_id => <<"sample_account_id">>,
        tier => sample_atom,
        valid_until => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(support_diagnostic_bundle, full) ->
    beam4pm_types:new_support_diagnostic_bundle(#{
        tenant_id => <<"sample_tenant_id">>,
        incident_id => <<"sample_incident_id">>,
        bundle_hash => <<"sample_bundle_hash">>
    });
sample(support_diagnostic_bundle, minimal) ->
    beam4pm_types:new_support_diagnostic_bundle(#{
        tenant_id => <<"sample_tenant_id">>,
        incident_id => <<"sample_incident_id">>,
        bundle_hash => <<"sample_bundle_hash">>
    });
sample(support_escalation_evidence, full) ->
    beam4pm_types:new_support_escalation_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        escalation_owner => <<"sample_escalation_owner">>,
        observed_result => sample_atom
    });
sample(support_escalation_evidence, minimal) ->
    beam4pm_types:new_support_escalation_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        escalation_owner => <<"sample_escalation_owner">>,
        observed_result => sample_atom
    });
sample(support_readiness, full) ->
    beam4pm_types:new_support_readiness(#{
        support_readiness_id => <<"sample_support_readiness_id">>,
        account_id => <<"sample_account_id">>,
        support_readiness_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(support_readiness, minimal) ->
    beam4pm_types:new_support_readiness(#{
        support_readiness_id => <<"sample_support_readiness_id">>,
        account_id => <<"sample_account_id">>,
        support_readiness_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(support_sla_evidence, full) ->
    beam4pm_types:new_support_sla_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        response_minutes => 42,
        observed_result => sample_atom
    });
sample(support_sla_evidence, minimal) ->
    beam4pm_types:new_support_sla_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        response_minutes => 42,
        observed_result => sample_atom
    });
sample(support_tier_admission, full) ->
    beam4pm_types:new_support_tier_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        support_tier_id => <<"sample_support_tier_id">>,
        decision => <<"sample_decision">>
    });
sample(support_tier_admission, minimal) ->
    beam4pm_types:new_support_tier_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        support_tier_id => <<"sample_support_tier_id">>,
        decision => <<"sample_decision">>
    });
sample(support_window_evidence, full) ->
    beam4pm_types:new_support_window_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        support_channel => <<"sample_support_channel">>,
        observed_result => sample_atom
    });
sample(support_window_evidence, minimal) ->
    beam4pm_types:new_support_window_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        support_channel => <<"sample_support_channel">>,
        observed_result => sample_atom
    });
sample(sync_time, full) ->
    beam4pm_types:new_sync_time(#{
        object_id => <<"sample_object_id">>,
        delaying_object_id => <<"sample_delaying_object_id">>,
        seconds => 3.5
    });
sample(sync_time, minimal) ->
    beam4pm_types:new_sync_time(#{
        object_id => <<"sample_object_id">>,
        seconds => 3.5
    });
sample(target_metric, full) ->
    beam4pm_types:new_target_metric(#{
        target_metric_id => <<"sample_target_metric_id">>,
        account_id => <<"sample_account_id">>,
        target_value => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(target_metric, minimal) ->
    beam4pm_types:new_target_metric(#{
        target_metric_id => <<"sample_target_metric_id">>,
        account_id => <<"sample_account_id">>,
        target_value => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(task_decomposition_proof, full) ->
    beam4pm_types:new_task_decomposition_proof(#{
        proof_id => <<"sample_proof_id">>,
        method_id => <<"sample_method_id">>,
        constraint_proof_digest => <<"sample_constraint_proof_digest">>
    });
sample(task_decomposition_proof, minimal) ->
    beam4pm_types:new_task_decomposition_proof(#{
        proof_id => <<"sample_proof_id">>,
        method_id => <<"sample_method_id">>,
        constraint_proof_digest => <<"sample_constraint_proof_digest">>
    });
sample(tax_jurisdiction_evidence, full) ->
    beam4pm_types:new_tax_jurisdiction_evidence(#{
        contracting_entity_id => <<"sample_contracting_entity_id">>,
        tax_jurisdiction => <<"sample_tax_jurisdiction">>,
        evidence_hash => <<"sample_evidence_hash">>
    });
sample(tax_jurisdiction_evidence, minimal) ->
    beam4pm_types:new_tax_jurisdiction_evidence(#{
        contracting_entity_id => <<"sample_contracting_entity_id">>,
        tax_jurisdiction => <<"sample_tax_jurisdiction">>,
        evidence_hash => <<"sample_evidence_hash">>
    });
sample(tax_jurisdiction_rule, full) ->
    beam4pm_types:new_tax_jurisdiction_rule(#{
        tax_jurisdiction_rule_id => <<"sample_tax_jurisdiction_rule_id">>,
        account_id => <<"sample_account_id">>,
        jurisdiction_code => <<"sample_jurisdiction_code">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(tax_jurisdiction_rule, minimal) ->
    beam4pm_types:new_tax_jurisdiction_rule(#{
        tax_jurisdiction_rule_id => <<"sample_tax_jurisdiction_rule_id">>,
        account_id => <<"sample_account_id">>,
        jurisdiction_code => <<"sample_jurisdiction_code">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(technical_blocker, full) ->
    beam4pm_types:new_technical_blocker(#{
        opportunity_id => <<"sample_opportunity_id">>,
        blocker_id => <<"sample_blocker_id">>,
        refusal_code => <<"sample_refusal_code">>
    });
sample(technical_blocker, minimal) ->
    beam4pm_types:new_technical_blocker(#{
        opportunity_id => <<"sample_opportunity_id">>,
        blocker_id => <<"sample_blocker_id">>,
        refusal_code => <<"sample_refusal_code">>
    });
sample(temporal_order_observation, full) ->
    beam4pm_types:new_temporal_order_observation(#{
        earlier_observation_id => <<"sample_earlier_observation_id">>,
        later_observation_id => <<"sample_later_observation_id">>,
        ordering_basis => <<"sample_ordering_basis">>,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(temporal_order_observation, minimal) ->
    beam4pm_types:new_temporal_order_observation(#{
        earlier_observation_id => <<"sample_earlier_observation_id">>,
        later_observation_id => <<"sample_later_observation_id">>,
        ordering_basis => <<"sample_ordering_basis">>,
        evidence_digest => <<"sample_evidence_digest">>
    });
sample(tenant_account, full) ->
    beam4pm_types:new_tenant_account(#{
        tenant_id => <<"sample_tenant_id">>,
        account_id => <<"sample_account_id">>,
        home_region => <<"sample_home_region">>,
        edition_id => <<"sample_edition_id">>
    });
sample(tenant_account, minimal) ->
    beam4pm_types:new_tenant_account(#{
        tenant_id => <<"sample_tenant_id">>,
        account_id => <<"sample_account_id">>,
        home_region => <<"sample_home_region">>,
        edition_id => <<"sample_edition_id">>
    });
sample(tenant_data_partition, full) ->
    beam4pm_types:new_tenant_data_partition(#{
        tenant_id => <<"sample_tenant_id">>,
        partition_id => <<"sample_partition_id">>,
        isolation_hash => <<"sample_isolation_hash">>
    });
sample(tenant_data_partition, minimal) ->
    beam4pm_types:new_tenant_data_partition(#{
        tenant_id => <<"sample_tenant_id">>,
        partition_id => <<"sample_partition_id">>,
        isolation_hash => <<"sample_isolation_hash">>
    });
sample(tenant_key_scope, full) ->
    beam4pm_types:new_tenant_key_scope(#{
        tenant_id => <<"sample_tenant_id">>,
        key_scope_id => <<"sample_key_scope_id">>,
        key_attestation_hash => <<"sample_key_attestation_hash">>
    });
sample(tenant_key_scope, minimal) ->
    beam4pm_types:new_tenant_key_scope(#{
        tenant_id => <<"sample_tenant_id">>,
        key_scope_id => <<"sample_key_scope_id">>,
        key_attestation_hash => <<"sample_key_attestation_hash">>
    });
sample(tenant_project, full) ->
    beam4pm_types:new_tenant_project(#{
        project_id => <<"sample_project_id">>,
        tenant_id => <<"sample_tenant_id">>,
        cost_center => <<"sample_cost_center">>,
        status => sample_atom
    });
sample(tenant_project, minimal) ->
    beam4pm_types:new_tenant_project(#{
        project_id => <<"sample_project_id">>,
        tenant_id => <<"sample_tenant_id">>,
        cost_center => <<"sample_cost_center">>,
        status => sample_atom
    });
sample(tenant_resource_quota, full) ->
    beam4pm_types:new_tenant_resource_quota(#{
        tenant_id => <<"sample_tenant_id">>,
        quota_id => <<"sample_quota_id">>,
        utilization_hash => <<"sample_utilization_hash">>
    });
sample(tenant_resource_quota, minimal) ->
    beam4pm_types:new_tenant_resource_quota(#{
        tenant_id => <<"sample_tenant_id">>,
        quota_id => <<"sample_quota_id">>,
        utilization_hash => <<"sample_utilization_hash">>
    });
sample(tenant_runtime_boundary, full) ->
    beam4pm_types:new_tenant_runtime_boundary(#{
        tenant_id => <<"sample_tenant_id">>,
        boundary_id => <<"sample_boundary_id">>,
        evidence_hash => <<"sample_evidence_hash">>
    });
sample(tenant_runtime_boundary, minimal) ->
    beam4pm_types:new_tenant_runtime_boundary(#{
        tenant_id => <<"sample_tenant_id">>,
        boundary_id => <<"sample_boundary_id">>,
        evidence_hash => <<"sample_evidence_hash">>
    });
sample(term_subscription, full) ->
    beam4pm_types:new_term_subscription(#{
        subscription_id => <<"sample_subscription_id">>,
        sku => <<"sample_sku">>,
        starts_at => <<"2026-08-29T12:00:00.123456Z">>,
        ends_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(term_subscription, minimal) ->
    beam4pm_types:new_term_subscription(#{
        subscription_id => <<"sample_subscription_id">>,
        sku => <<"sample_sku">>,
        starts_at => <<"2026-08-29T12:00:00.123456Z">>,
        ends_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(termination_right_admission, full) ->
    beam4pm_types:new_termination_right_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        termination_right_id => <<"sample_termination_right_id">>,
        decision => <<"sample_decision">>
    });
sample(termination_right_admission, minimal) ->
    beam4pm_types:new_termination_right_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        termination_right_id => <<"sample_termination_right_id">>,
        decision => <<"sample_decision">>
    });
sample(time_to_value, full) ->
    beam4pm_types:new_time_to_value(#{
        time_to_value_id => <<"sample_time_to_value_id">>,
        account_id => <<"sample_account_id">>,
        verified_days => 42,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(time_to_value, minimal) ->
    beam4pm_types:new_time_to_value(#{
        time_to_value_id => <<"sample_time_to_value_id">>,
        account_id => <<"sample_account_id">>,
        verified_days => 42,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(token_replay_state, full) ->
    beam4pm_types:new_token_replay_state(#{
        replay_id => <<"sample_replay_id">>,
        prefix_digest => <<"sample_prefix_digest">>,
        marking_digest => <<"sample_marking_digest">>
    });
sample(token_replay_state, minimal) ->
    beam4pm_types:new_token_replay_state(#{
        replay_id => <<"sample_replay_id">>,
        prefix_digest => <<"sample_prefix_digest">>,
        marking_digest => <<"sample_marking_digest">>
    });
sample(toolchain_identity, full) ->
    beam4pm_types:new_toolchain_identity(#{
        tenant_id => <<"sample_tenant_id">>,
        toolchain_id => <<"sample_toolchain_id">>,
        identity_hash => <<"sample_identity_hash">>
    });
sample(toolchain_identity, minimal) ->
    beam4pm_types:new_toolchain_identity(#{
        tenant_id => <<"sample_tenant_id">>,
        toolchain_id => <<"sample_toolchain_id">>,
        identity_hash => <<"sample_identity_hash">>
    });
sample(toolchain_identity_observation, full) ->
    beam4pm_types:new_toolchain_identity_observation(#{
        tool_name => <<"sample_tool_name">>,
        tool_version => <<"sample_tool_version">>,
        executable_digest => <<"sample_executable_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(toolchain_identity_observation, minimal) ->
    beam4pm_types:new_toolchain_identity_observation(#{
        tool_name => <<"sample_tool_name">>,
        tool_version => <<"sample_tool_version">>,
        executable_digest => <<"sample_executable_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(training_readiness, full) ->
    beam4pm_types:new_training_readiness(#{
        training_readiness_id => <<"sample_training_readiness_id">>,
        account_id => <<"sample_account_id">>,
        training_completion_rate => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(training_readiness, minimal) ->
    beam4pm_types:new_training_readiness(#{
        training_readiness_id => <<"sample_training_readiness_id">>,
        account_id => <<"sample_account_id">>,
        training_completion_rate => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(training_scope_admission, full) ->
    beam4pm_types:new_training_scope_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        training_scope_id => <<"sample_training_scope_id">>,
        decision => <<"sample_decision">>
    });
sample(training_scope_admission, minimal) ->
    beam4pm_types:new_training_scope_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        training_scope_id => <<"sample_training_scope_id">>,
        decision => <<"sample_decision">>
    });
sample(trajectory_window, full) ->
    beam4pm_types:new_trajectory_window(#{
        window_id => <<"sample_window_id">>,
        repository_id => <<"sample_repository_id">>,
        first_subject_sha => <<"sample_first_subject_sha">>,
        last_subject_sha => <<"sample_last_subject_sha">>,
        episode_count => 42
    });
sample(trajectory_window, minimal) ->
    beam4pm_types:new_trajectory_window(#{
        window_id => <<"sample_window_id">>,
        repository_id => <<"sample_repository_id">>,
        first_subject_sha => <<"sample_first_subject_sha">>,
        last_subject_sha => <<"sample_last_subject_sha">>,
        episode_count => 42
    });
sample(trial_entitlement, full) ->
    beam4pm_types:new_trial_entitlement(#{
        trial_entitlement_id => <<"sample_trial_entitlement_id">>,
        account_id => <<"sample_account_id">>,
        trial_expires_at => <<"2026-08-29T12:00:00.123456Z">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(trial_entitlement, minimal) ->
    beam4pm_types:new_trial_entitlement(#{
        trial_entitlement_id => <<"sample_trial_entitlement_id">>,
        account_id => <<"sample_account_id">>,
        trial_expires_at => <<"2026-08-29T12:00:00.123456Z">>,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(true_up_policy, full) ->
    beam4pm_types:new_true_up_policy(#{
        policy_id => <<"sample_policy_id">>,
        commitment_id => <<"sample_commitment_id">>,
        cadence => sample_atom,
        shortfall_behavior => sample_atom
    });
sample(true_up_policy, minimal) ->
    beam4pm_types:new_true_up_policy(#{
        policy_id => <<"sample_policy_id">>,
        commitment_id => <<"sample_commitment_id">>,
        cadence => sample_atom,
        shortfall_behavior => sample_atom
    });
sample(type_edge, full) ->
    beam4pm_types:new_type_edge(#{
        source_type => <<"sample_source_type">>,
        target_type => <<"sample_target_type">>,
        qualifier => <<"sample_qualifier">>,
        direction => sample_atom
    });
sample(type_edge, minimal) ->
    beam4pm_types:new_type_edge(#{
        source_type => <<"sample_source_type">>,
        target_type => <<"sample_target_type">>,
        qualifier => <<"sample_qualifier">>,
        direction => sample_atom
    });
sample(uncertainty_aware_selection, full) ->
    beam4pm_types:new_uncertainty_aware_selection(#{
        option_set_id => <<"sample_option_set_id">>,
        uncertainty_model_hash => <<"sample_uncertainty_model_hash">>,
        selected_option_id => <<"sample_selected_option_id">>
    });
sample(uncertainty_aware_selection, minimal) ->
    beam4pm_types:new_uncertainty_aware_selection(#{
        option_set_id => <<"sample_option_set_id">>,
        uncertainty_model_hash => <<"sample_uncertainty_model_hash">>,
        selected_option_id => <<"sample_selected_option_id">>
    });
sample(uncertainty_observation, full) ->
    beam4pm_types:new_uncertainty_observation(#{
        observation_id => <<"sample_observation_id">>,
        uncertainty_kind => <<"sample_uncertainty_kind">>,
        confidence_basis => <<"sample_confidence_basis">>,
        standing => <<"sample_standing">>
    });
sample(uncertainty_observation, minimal) ->
    beam4pm_types:new_uncertainty_observation(#{
        observation_id => <<"sample_observation_id">>,
        uncertainty_kind => <<"sample_uncertainty_kind">>,
        confidence_basis => <<"sample_confidence_basis">>,
        standing => <<"sample_standing">>
    });
sample(unit_economics_snapshot, full) ->
    beam4pm_types:new_unit_economics_snapshot(#{
        unit_economics_snapshot_id => <<"sample_unit_economics_snapshot_id">>,
        account_id => <<"sample_account_id">>,
        gross_margin => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(unit_economics_snapshot, minimal) ->
    beam4pm_types:new_unit_economics_snapshot(#{
        unit_economics_snapshot_id => <<"sample_unit_economics_snapshot_id">>,
        account_id => <<"sample_account_id">>,
        gross_margin => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(unsupported_capability_evidence, full) ->
    beam4pm_types:new_unsupported_capability_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        capability_name => <<"sample_capability_name">>,
        observed_result => sample_atom
    });
sample(unsupported_capability_evidence, minimal) ->
    beam4pm_types:new_unsupported_capability_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        capability_name => <<"sample_capability_name">>,
        observed_result => sample_atom
    });
sample(upgrade_evidence, full) ->
    beam4pm_types:new_upgrade_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        from_version => <<"sample_from_version">>,
        observed_result => sample_atom
    });
sample(upgrade_evidence, minimal) ->
    beam4pm_types:new_upgrade_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        from_version => <<"sample_from_version">>,
        observed_result => sample_atom
    });
sample(upsell_readiness, full) ->
    beam4pm_types:new_upsell_readiness(#{
        upsell_readiness_id => <<"sample_upsell_readiness_id">>,
        account_id => <<"sample_account_id">>,
        upsell_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(upsell_readiness, minimal) ->
    beam4pm_types:new_upsell_readiness(#{
        upsell_readiness_id => <<"sample_upsell_readiness_id">>,
        account_id => <<"sample_account_id">>,
        upsell_score => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(usage_aggregation_window, full) ->
    beam4pm_types:new_usage_aggregation_window(#{
        usage_aggregation_window_id => <<"sample_usage_aggregation_window_id">>,
        account_id => <<"sample_account_id">>,
        window_seconds => 42,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(usage_aggregation_window, minimal) ->
    beam4pm_types:new_usage_aggregation_window(#{
        usage_aggregation_window_id => <<"sample_usage_aggregation_window_id">>,
        account_id => <<"sample_account_id">>,
        window_seconds => 42,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(usage_correction, full) ->
    beam4pm_types:new_usage_correction(#{
        usage_correction_id => <<"sample_usage_correction_id">>,
        account_id => <<"sample_account_id">>,
        corrected_quantity => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(usage_correction, minimal) ->
    beam4pm_types:new_usage_correction(#{
        usage_correction_id => <<"sample_usage_correction_id">>,
        account_id => <<"sample_account_id">>,
        corrected_quantity => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        effective_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(usage_event, full) ->
    beam4pm_types:new_usage_event(#{
        event_id => <<"sample_event_id">>,
        entitlement_id => <<"sample_entitlement_id">>,
        quantity => 3.5,
        metric_name => <<"sample_metric_name">>,
        occurred_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(usage_event, minimal) ->
    beam4pm_types:new_usage_event(#{
        event_id => <<"sample_event_id">>,
        entitlement_id => <<"sample_entitlement_id">>,
        quantity => 3.5,
        metric_name => <<"sample_metric_name">>,
        occurred_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(usage_plan, full) ->
    beam4pm_types:new_usage_plan(#{
        plan_id => <<"sample_plan_id">>,
        metric_name => <<"sample_metric_name">>,
        unit => <<"sample_unit">>,
        billing_mode => sample_atom
    });
sample(usage_plan, minimal) ->
    beam4pm_types:new_usage_plan(#{
        plan_id => <<"sample_plan_id">>,
        metric_name => <<"sample_metric_name">>,
        unit => <<"sample_unit">>,
        billing_mode => sample_atom
    });
sample(usage_reconciliation_receipt, full) ->
    beam4pm_types:new_usage_reconciliation_receipt(#{
        tenant_id => <<"sample_tenant_id">>,
        billing_period_id => <<"sample_billing_period_id">>,
        reconciliation_hash => <<"sample_reconciliation_hash">>
    });
sample(usage_reconciliation_receipt, minimal) ->
    beam4pm_types:new_usage_reconciliation_receipt(#{
        tenant_id => <<"sample_tenant_id">>,
        billing_period_id => <<"sample_billing_period_id">>,
        reconciliation_hash => <<"sample_reconciliation_hash">>
    });
sample(usage_signal, full) ->
    beam4pm_types:new_usage_signal(#{
        usage_signal_id => <<"sample_usage_signal_id">>,
        account_id => <<"sample_account_id">>,
        active_user_count => 42,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(usage_signal, minimal) ->
    beam4pm_types:new_usage_signal(#{
        usage_signal_id => <<"sample_usage_signal_id">>,
        account_id => <<"sample_account_id">>,
        active_user_count => 42,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(validation_capsule_drift_observation, full) ->
    beam4pm_types:new_validation_capsule_drift_observation(#{
        expected_digest => <<"sample_expected_digest">>,
        observed_digest => <<"sample_observed_digest">>,
        drift_status => <<"sample_drift_status">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(validation_capsule_drift_observation, minimal) ->
    beam4pm_types:new_validation_capsule_drift_observation(#{
        expected_digest => <<"sample_expected_digest">>,
        observed_digest => <<"sample_observed_digest">>,
        drift_status => <<"sample_drift_status">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(validation_capsule_identity_observation, full) ->
    beam4pm_types:new_validation_capsule_identity_observation(#{
        capsule_repository => <<"sample_capsule_repository">>,
        capsule_sha => <<"sample_capsule_sha">>,
        execution_mode => <<"sample_execution_mode">>,
        image_digest => <<"sample_image_digest">>
    });
sample(validation_capsule_identity_observation, minimal) ->
    beam4pm_types:new_validation_capsule_identity_observation(#{
        capsule_repository => <<"sample_capsule_repository">>,
        capsule_sha => <<"sample_capsule_sha">>,
        execution_mode => <<"sample_execution_mode">>,
        image_digest => <<"sample_image_digest">>
    });
sample(value_baseline, full) ->
    beam4pm_types:new_value_baseline(#{
        baseline_id => <<"sample_baseline_id">>,
        account_id => <<"sample_account_id">>,
        metric_name => <<"sample_metric_name">>,
        baseline_value => 3.5
    });
sample(value_baseline, minimal) ->
    beam4pm_types:new_value_baseline(#{
        baseline_id => <<"sample_baseline_id">>,
        account_id => <<"sample_account_id">>,
        metric_name => <<"sample_metric_name">>,
        baseline_value => 3.5
    });
sample(value_driver, full) ->
    beam4pm_types:new_value_driver(#{
        value_driver_id => <<"sample_value_driver_id">>,
        account_id => <<"sample_account_id">>,
        annual_value => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(value_driver, minimal) ->
    beam4pm_types:new_value_driver(#{
        value_driver_id => <<"sample_value_driver_id">>,
        account_id => <<"sample_account_id">>,
        annual_value => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(value_of_information_estimate, full) ->
    beam4pm_types:new_value_of_information_estimate(#{
        decision_id => <<"sample_decision_id">>,
        evidence_candidate_id => <<"sample_evidence_candidate_id">>,
        expected_information_gain => 3.5,
        cost_basis => <<"sample_cost_basis">>
    });
sample(value_of_information_estimate, minimal) ->
    beam4pm_types:new_value_of_information_estimate(#{
        decision_id => <<"sample_decision_id">>,
        evidence_candidate_id => <<"sample_evidence_candidate_id">>,
        expected_information_gain => 3.5,
        cost_basis => <<"sample_cost_basis">>
    });
sample(value_of_information_score, full) ->
    beam4pm_types:new_value_of_information_score(#{
        option_id => <<"sample_option_id">>,
        observation_id => <<"sample_observation_id">>,
        score => <<"sample_score">>
    });
sample(value_of_information_score, minimal) ->
    beam4pm_types:new_value_of_information_score(#{
        option_id => <<"sample_option_id">>,
        observation_id => <<"sample_observation_id">>,
        score => <<"sample_score">>
    });
sample(value_realization, full) ->
    beam4pm_types:new_value_realization(#{
        value_realization_id => <<"sample_value_realization_id">>,
        account_id => <<"sample_account_id">>,
        realized_value => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(value_realization, minimal) ->
    beam4pm_types:new_value_realization(#{
        value_realization_id => <<"sample_value_realization_id">>,
        account_id => <<"sample_account_id">>,
        realized_value => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(value_realization_feedback, full) ->
    beam4pm_types:new_value_realization_feedback(#{
        feedback_id => <<"sample_feedback_id">>,
        objective_id => <<"sample_objective_id">>,
        baseline_value => 3.5,
        realized_value => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        policy_id => <<"sample_policy_id">>
    });
sample(value_realization_feedback, minimal) ->
    beam4pm_types:new_value_realization_feedback(#{
        feedback_id => <<"sample_feedback_id">>,
        objective_id => <<"sample_objective_id">>,
        baseline_value => 3.5,
        realized_value => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        policy_id => <<"sample_policy_id">>
    });
sample(value_receipt, full) ->
    beam4pm_types:new_value_receipt(#{
        value_receipt_id => <<"sample_value_receipt_id">>,
        account_id => <<"sample_account_id">>,
        metric_name => <<"sample_metric_name">>,
        baseline_value => 3.5,
        observed_value => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(value_receipt, minimal) ->
    beam4pm_types:new_value_receipt(#{
        value_receipt_id => <<"sample_value_receipt_id">>,
        account_id => <<"sample_account_id">>,
        metric_name => <<"sample_metric_name">>,
        baseline_value => 3.5,
        observed_value => 3.5,
        evidence_digest => <<"sample_evidence_digest">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(value_telemetry_sample, full) ->
    beam4pm_types:new_value_telemetry_sample(#{
        tenant_id => <<"sample_tenant_id">>,
        outcome_id => <<"sample_outcome_id">>,
        telemetry_hash => <<"sample_telemetry_hash">>
    });
sample(value_telemetry_sample, minimal) ->
    beam4pm_types:new_value_telemetry_sample(#{
        tenant_id => <<"sample_tenant_id">>,
        outcome_id => <<"sample_outcome_id">>,
        telemetry_hash => <<"sample_telemetry_hash">>
    });
sample(vendor_registration_state, full) ->
    beam4pm_types:new_vendor_registration_state(#{
        account_id => <<"sample_account_id">>,
        registration_id => <<"sample_registration_id">>,
        registration_state => <<"sample_registration_state">>
    });
sample(vendor_registration_state, minimal) ->
    beam4pm_types:new_vendor_registration_state(#{
        account_id => <<"sample_account_id">>,
        registration_id => <<"sample_registration_id">>,
        registration_state => <<"sample_registration_state">>
    });
sample(vendor_risk_evidence, full) ->
    beam4pm_types:new_vendor_risk_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        risk_score => 3.5,
        observed_result => sample_atom
    });
sample(vendor_risk_evidence, minimal) ->
    beam4pm_types:new_vendor_risk_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        risk_score => 3.5,
        observed_result => sample_atom
    });
sample(verification_depth_update, full) ->
    beam4pm_types:new_verification_depth_update(#{
        update_id => <<"sample_update_id">>,
        verifier_id => <<"sample_verifier_id">>,
        prior_depth => 42,
        new_depth => 42,
        risk_signal_digest => <<"sample_risk_signal_digest">>,
        minimum_depth => 42
    });
sample(verification_depth_update, minimal) ->
    beam4pm_types:new_verification_depth_update(#{
        update_id => <<"sample_update_id">>,
        verifier_id => <<"sample_verifier_id">>,
        prior_depth => 42,
        new_depth => 42,
        risk_signal_digest => <<"sample_risk_signal_digest">>,
        minimum_depth => 42
    });
sample(version_lifecycle_evidence, full) ->
    beam4pm_types:new_version_lifecycle_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        supported_until => <<"2026-08-29T12:00:00.123456Z">>,
        observed_result => sample_atom
    });
sample(version_lifecycle_evidence, minimal) ->
    beam4pm_types:new_version_lifecycle_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        supported_until => <<"2026-08-29T12:00:00.123456Z">>,
        observed_result => sample_atom
    });
sample(volume_tier_admission, full) ->
    beam4pm_types:new_volume_tier_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        volume_tier_id => <<"sample_volume_tier_id">>,
        decision => <<"sample_decision">>
    });
sample(volume_tier_admission, minimal) ->
    beam4pm_types:new_volume_tier_admission(#{
        opportunity_id => <<"sample_opportunity_id">>,
        volume_tier_id => <<"sample_volume_tier_id">>,
        decision => <<"sample_decision">>
    });
sample(vulnerability_scan_evidence, full) ->
    beam4pm_types:new_vulnerability_scan_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        vulnerability_count => 42,
        observed_result => sample_atom
    });
sample(vulnerability_scan_evidence, minimal) ->
    beam4pm_types:new_vulnerability_scan_evidence(#{
        evidence_id => <<"sample_evidence_id">>,
        subject_sha => <<"sample_subject_sha">>,
        vulnerability_count => 42,
        observed_result => sample_atom
    });
sample(weak_plan_candidate, full) ->
    beam4pm_types:new_weak_plan_candidate(#{
        candidate_id => <<"sample_candidate_id">>,
        policy_digest => <<"sample_policy_digest">>,
        uncovered_digest => <<"sample_uncovered_digest">>
    });
sample(weak_plan_candidate, minimal) ->
    beam4pm_types:new_weak_plan_candidate(#{
        candidate_id => <<"sample_candidate_id">>,
        policy_digest => <<"sample_policy_digest">>,
        uncovered_digest => <<"sample_uncovered_digest">>
    });
sample(wip_limit_gate, full) ->
    beam4pm_types:new_wip_limit_gate(#{
        episode_id => <<"sample_episode_id">>,
        wip_limit => <<"sample_wip_limit">>,
        standing => <<"sample_standing">>
    });
sample(wip_limit_gate, minimal) ->
    beam4pm_types:new_wip_limit_gate(#{
        episode_id => <<"sample_episode_id">>,
        wip_limit => <<"sample_wip_limit">>,
        standing => <<"sample_standing">>
    });
sample(workflow_definition_digest_observation, full) ->
    beam4pm_types:new_workflow_definition_digest_observation(#{
        workflow_path => <<"sample_workflow_path">>,
        definition_sha256 => <<"sample_definition_sha256">>,
        source_sha => <<"sample_source_sha">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(workflow_definition_digest_observation, minimal) ->
    beam4pm_types:new_workflow_definition_digest_observation(#{
        workflow_path => <<"sample_workflow_path">>,
        definition_sha256 => <<"sample_definition_sha256">>,
        source_sha => <<"sample_source_sha">>,
        observed_at => <<"2026-08-29T12:00:00.123456Z">>
    });
sample(workflow_job_state_observation, full) ->
    beam4pm_types:new_workflow_job_state_observation(#{
        job_id => <<"sample_job_id">>,
        run_id => <<"sample_run_id">>,
        runner_identity => <<"sample_runner_identity">>,
        conclusion => <<"sample_conclusion">>
    });
sample(workflow_job_state_observation, minimal) ->
    beam4pm_types:new_workflow_job_state_observation(#{
        job_id => <<"sample_job_id">>,
        run_id => <<"sample_run_id">>,
        runner_identity => <<"sample_runner_identity">>,
        conclusion => <<"sample_conclusion">>
    });
sample(workflow_run_state_observation, full) ->
    beam4pm_types:new_workflow_run_state_observation(#{
        run_id => <<"sample_run_id">>,
        workflow_id => <<"sample_workflow_id">>,
        head_sha => <<"sample_head_sha">>,
        conclusion => <<"sample_conclusion">>
    });
sample(workflow_run_state_observation, minimal) ->
    beam4pm_types:new_workflow_run_state_observation(#{
        run_id => <<"sample_run_id">>,
        workflow_id => <<"sample_workflow_id">>,
        head_sha => <<"sample_head_sha">>,
        conclusion => <<"sample_conclusion">>
    });
sample(workload_backpressure_signal, full) ->
    beam4pm_types:new_workload_backpressure_signal(#{
        tenant_id => <<"sample_tenant_id">>,
        signal_id => <<"sample_signal_id">>,
        measurement_hash => <<"sample_measurement_hash">>
    });
sample(workload_backpressure_signal, minimal) ->
    beam4pm_types:new_workload_backpressure_signal(#{
        tenant_id => <<"sample_tenant_id">>,
        signal_id => <<"sample_signal_id">>,
        measurement_hash => <<"sample_measurement_hash">>
    });
sample(workload_cancellation_receipt, full) ->
    beam4pm_types:new_workload_cancellation_receipt(#{
        tenant_id => <<"sample_tenant_id">>,
        execution_id => <<"sample_execution_id">>,
        receipt_hash => <<"sample_receipt_hash">>
    });
sample(workload_cancellation_receipt, minimal) ->
    beam4pm_types:new_workload_cancellation_receipt(#{
        tenant_id => <<"sample_tenant_id">>,
        execution_id => <<"sample_execution_id">>,
        receipt_hash => <<"sample_receipt_hash">>
    });
sample(workload_execution_identity, full) ->
    beam4pm_types:new_workload_execution_identity(#{
        tenant_id => <<"sample_tenant_id">>,
        execution_id => <<"sample_execution_id">>,
        subject_digest => <<"sample_subject_digest">>
    });
sample(workload_execution_identity, minimal) ->
    beam4pm_types:new_workload_execution_identity(#{
        tenant_id => <<"sample_tenant_id">>,
        execution_id => <<"sample_execution_id">>,
        subject_digest => <<"sample_subject_digest">>
    });
sample(workload_idempotency_key, full) ->
    beam4pm_types:new_workload_idempotency_key(#{
        tenant_id => <<"sample_tenant_id">>,
        execution_id => <<"sample_execution_id">>,
        idempotency_key => <<"sample_idempotency_key">>
    });
sample(workload_idempotency_key, minimal) ->
    beam4pm_types:new_workload_idempotency_key(#{
        tenant_id => <<"sample_tenant_id">>,
        execution_id => <<"sample_execution_id">>,
        idempotency_key => <<"sample_idempotency_key">>
    });
sample(workload_queue_depth, full) ->
    beam4pm_types:new_workload_queue_depth(#{
        tenant_id => <<"sample_tenant_id">>,
        queue_id => <<"sample_queue_id">>,
        measurement_hash => <<"sample_measurement_hash">>
    });
sample(workload_queue_depth, minimal) ->
    beam4pm_types:new_workload_queue_depth(#{
        tenant_id => <<"sample_tenant_id">>,
        queue_id => <<"sample_queue_id">>,
        measurement_hash => <<"sample_measurement_hash">>
    });
sample(workload_retry_policy, full) ->
    beam4pm_types:new_workload_retry_policy(#{
        tenant_id => <<"sample_tenant_id">>,
        policy_id => <<"sample_policy_id">>,
        policy_hash => <<"sample_policy_hash">>
    });
sample(workload_retry_policy, minimal) ->
    beam4pm_types:new_workload_retry_policy(#{
        tenant_id => <<"sample_tenant_id">>,
        policy_id => <<"sample_policy_id">>,
        policy_hash => <<"sample_policy_hash">>
    });
sample(workload_timeout_budget, full) ->
    beam4pm_types:new_workload_timeout_budget(#{
        tenant_id => <<"sample_tenant_id">>,
        budget_id => <<"sample_budget_id">>,
        budget_hash => <<"sample_budget_hash">>
    });
sample(workload_timeout_budget, minimal) ->
    beam4pm_types:new_workload_timeout_budget(#{
        tenant_id => <<"sample_tenant_id">>,
        budget_id => <<"sample_budget_id">>,
        budget_hash => <<"sample_budget_hash">>
    });
sample(Name, Variant) ->
    erlang:error({unknown_sample, Name, Variant}).

%% Write every record x variant sample as Dir/<record>.<variant>.erl.json.
-spec write_samples(file:filename_all()) -> ok.
write_samples(Dir) ->
    ok = filelib:ensure_path(Dir),
    lists:foreach(
        fun({Name, Variant}) ->
            {ok, Rec} = sample(Name, Variant),
            Json = beam4pm_codec:encode(Rec),
            Path = filename:join(Dir, sample_basename(Name, Variant, "erl")),
            ok = file:write_file(Path, Json)
        end,
        pairs()),
    ok.

%% Read every record x variant fixture Dir/<record>.<variant>.<Suffix>.json,
%% decode it, and compare its canonical map against this module's own sample.
-spec verify_samples(file:filename_all(), string()) ->
    {non_neg_integer(), [string()]}.
verify_samples(Dir, Suffix) ->
    lists:foldl(
        fun({Name, Variant}, {Pass, Failures}) ->
            Label = atom_to_list(Name) ++ "." ++ atom_to_list(Variant),
            Path = filename:join(Dir, sample_basename(Name, Variant, Suffix)),
            {ok, Expected} = sample(Name, Variant),
            case file:read_file(Path) of
                {ok, Bin} ->
                    case beam4pm_codec:decode(Name, Bin) of
                        {ok, Decoded} ->
                            case beam4pm_codec:to_map(Decoded) =:= beam4pm_codec:to_map(Expected) of
                                true -> {Pass + 1, Failures};
                                false -> {Pass, Failures ++ [Label ++ ": map mismatch"]}
                            end;
                        {error, Reason} ->
                            {Pass, Failures ++ [Label ++ ": decode error " ++ format_term(Reason)]}
                    end;
                {error, Reason} ->
                    {Pass, Failures ++ [Label ++ ": read error " ++ format_term(Reason)]}
            end
        end,
        {0, []},
        pairs()).

%% internal

pairs() ->
    [{Name, Variant} || Name <- record_names(), Variant <- [full, minimal]].

sample_basename(Name, Variant, Suffix) ->
    atom_to_list(Name) ++ "." ++ atom_to_list(Variant) ++ "." ++ Suffix ++ ".json".

format_term(Term) ->
    lists:flatten(io_lib:format("~0p", [Term])).
