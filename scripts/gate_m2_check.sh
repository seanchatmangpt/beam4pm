...
# Manufactured provenance is signalled per-FILE by the
# "GENERATED by ggen ... Do not edit." header comment (first non-shebang
# line), never by directory placement -- ggen output lives in the same src/,
# lib/, test/, gleam/, schema/, docs/reference/ directories hand-authored
# code uses. This script finds every manufactured file by that marker, deletes
# exactly those files (never a whole directory -- src/beam4pm.app.src and every
# other hand-authored file survives untouched), regenerates, and diffs.
set -euo pipefail
cd "$(dirname "$0")/.."
...
# Restore BOTH hand-authored tests and canonical root ontology on ANY exit.
# The composed ontology exists only in the regeneration workspace; canonical
# fragments remain separate checked-in manufacturing inputs.
trap cleanup EXIT
for f in "${HAND_AUTHORED_DEPENDENT_TESTS[@]}"; do
...
mapfile -t ONTOLOGY_FRAGMENTS < <(find ontology -maxdepth 1 -type f -name '*.ttl' -print 2>/dev/null | LC_ALL=C sort)
...
# Restore the canonical root before inspection. Both primary ggen and every
# second-engine sync above consumed the same composed O*; the composition
# itself is not a generated consequence and must never appear in the diff.
restore_ontology
