defmodule BeamPM.Evidence do
  ...
  def attach_all do
    ...
    :ok = BeamPM.Evidence.ReceiptBridge.attach(names)
  end
...
defmodule BeamPM.Evidence.ReceiptBridge do
  @moduledoc """
  Third real consumer of the `[:beam4pm, :engine, engine, op]` telemetry
  event family (alongside `BeamPM.Ingest.Bridge`'s OCEL buffer and
  `BeamPM.Evidence.OtelBridge`'s spans): writes one real, hash-chained
  `beam4pm-brce/v1` receipt per invocation via the already-real
  `BeamPM.ReceiptChain.link_fields/3` (`lib/beam4pm_receipt_chain.ex`,
  GENERATED by ggen_igniter -- read, not guessed, before this module was
  written).

  ## Chain scoping

  `chain_id` is the capability id (`"<engine>.<op>"`, e.g.
  `"petgraph.graph_new"`) -- every invocation of the SAME engine op chains
  onto the SAME receipt sequence, so `BeamPM.ReceiptChain.verify/2` on one
  `chain_id` proves the full, tamper-evident invocation history of that one
  capability. This mirrors `BeamPM.ProcessGovernor.apply_transition/3`'s
  own default (`chain_id = process_id`) at a different scope (per-capability
  rather than per-governed-process).

  ## Receipt shape

  Minimal, real `beam4pm-brce/v1` envelope: the four chain-linkage fields
  from `link_fields/3` plus the same identity/outcome fields
  `BeamPM.Evidence.OtelBridge` already puts on the span (`engine`, `op`,
  `op_iri`, `invocation_id`, `verification_class`, `args_digest`,
  `duration_ms`, `outcome`, `refusal_reason`) -- one on-disk, independently
  replayable receipt per real engine-op call, not merely an in-memory span.

  Receipts land under `receipts/engine_ops/` (top-level `*.json` files
  directly in that directory, per `link_fields/3`'s own contract -- see its
  moduledoc: a `next_receipt_path` anywhere else is never recorded).
  """

  alias BeamPM.ReceiptChain

  @receipt_schema "beam4pm-brce/v1"
  @default_receipts_dir Path.join([File.cwd!(), "receipts", "engine_ops"])
  ...
  @doc "The receipts directory this bridge writes to. Overridable via BEAM4PM_ENGINE_OP_RECEIPTS_DIR for tests/smoke checks."
  @spec receipts_dir() :: String.t()
  def receipts_dir do
    System.get_env("BEAM4PM_ENGINE_OP_RECEIPTS_DIR", @default_receipts_dir)
  end
  ...
  def handle_event([:beam4pm, :engine, engine, op], measurements, metadata, _config) do
    ...
    link = ReceiptChain.link_fields(dir, chain_id, next_receipt_path)
    ...
    receipt = %{
      ...
      "op" => to_string(op),
      "op_iri" => to_string(Map.get(metadata, :op_iri, "")),
      "invocation_id" => inspect(invocation_id),
      "verification_class" => to_string(Map.get(metadata, :verification_class, "")),
    ...
    File.write!(next_receipt_path, JSON.encode!(receipt))
    :ok
  end
end
