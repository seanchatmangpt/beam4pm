...
defmodule BeamPM.Ash.Resources.EventVolumePricingPolicyTest do
  use ExUnit.Case, async: false

  test "event_volume_pricing_policy: real Ash ETS create/read round-trips every field" do
    params =
      %{
    ...
    [read_back] = Ash.read!(BeamPM.Ash.Resources.EventVolumePricingPolicy)
    assert read_back.id == created.id
    assert read_back.event_volume_pricing_policy_id == "sample_event_volume_pricing_policy_id"
    assert read_back.account_id == "sample_account_id"
    assert read_back.unit_event_price == 3.5
