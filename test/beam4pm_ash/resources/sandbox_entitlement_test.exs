# GENERATED by ggen_igniter. Do not edit.
defmodule BeamPM.Ash.Resources.SandboxEntitlementTest do
  use ExUnit.Case, async: false

  test "sandbox_entitlement: real Ash ETS create/read round-trips every field" do
    params =
      %{
      sandbox_entitlement_id: "sample_sandbox_entitlement_id",
      account_id: "sample_account_id",
      sandbox_limit: 42,
      evidence_digest: "sample_evidence_digest",
      effective_at: "2026-08-29T12:00:00Z"
      }

    created =
      BeamPM.Ash.Resources.SandboxEntitlement
      |> Ash.Changeset.for_create(:create, params)
      |> Ash.create!()

    [read_back] = Ash.read!(BeamPM.Ash.Resources.SandboxEntitlement)
    assert read_back.id == created.id
    assert read_back.sandbox_entitlement_id == "sample_sandbox_entitlement_id"
    assert read_back.account_id == "sample_account_id"
    assert read_back.sandbox_limit == 42
    assert read_back.evidence_digest == "sample_evidence_digest"
    assert read_back.effective_at == ~U[2026-08-29 12:00:00Z]
  end
end

