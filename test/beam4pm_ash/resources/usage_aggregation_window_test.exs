# GENERATED by ggen_igniter. Do not edit.
defmodule BeamPM.Ash.Resources.UsageAggregationWindowTest do
  use ExUnit.Case, async: false

  test "usage_aggregation_window: real Ash ETS create/read round-trips every field" do
    params =
      %{
      usage_aggregation_window_id: "sample_usage_aggregation_window_id",
      account_id: "sample_account_id",
      window_seconds: 42,
      evidence_digest: "sample_evidence_digest",
      effective_at: "2026-08-29T12:00:00Z"
      }

    created =
      BeamPM.Ash.Resources.UsageAggregationWindow
      |> Ash.Changeset.for_create(:create, params)
      |> Ash.create!()

    [read_back] = Ash.read!(BeamPM.Ash.Resources.UsageAggregationWindow)
    assert read_back.id == created.id
    assert read_back.usage_aggregation_window_id == "sample_usage_aggregation_window_id"
    assert read_back.account_id == "sample_account_id"
    assert read_back.window_seconds == 42
    assert read_back.evidence_digest == "sample_evidence_digest"
    assert read_back.effective_at == ~U[2026-08-29 12:00:00Z]
  end
end

