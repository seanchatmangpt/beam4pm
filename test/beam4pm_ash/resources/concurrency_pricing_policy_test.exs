...
defmodule BeamPM.Ash.Resources.ConcurrencyPricingPolicyTest do
  use ExUnit.Case, async: false

  test "concurrency_pricing_policy: real Ash ETS create/read round-trips every field" do
    params =
      %{
    ...
    [read_back] = Ash.read!(BeamPM.Ash.Resources.ConcurrencyPricingPolicy)
    assert read_back.id == created.id
    assert read_back.concurrency_pricing_policy_id == "sample_concurrency_pricing_policy_id"
    assert read_back.account_id == "sample_account_id"
    assert read_back.included_concurrency == 42
