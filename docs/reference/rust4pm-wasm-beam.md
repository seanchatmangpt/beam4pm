...
Updated 2026-09-05. This document itself is hand-authored infrastructure prose (no ggen
provenance header). **The BEAM facade files it describes are not** — as of a same-day, later
commit than this document's main body, all four native engines' facades (`rust4pm`, `ferroplan`,
`petgraph`, `tract`) are ggen-manufactured from `bpm:Engine`/`bpm:EngineOp`/`bpm:EngineArg` facts
in `ontology.ttl` (see §9, rewritten). `lib/beam4pm_rust4pm.ex`, `src/beam4pm_rust4pm.erl`, and
`gleam/src/beam4pm/rust4pm.gleam` each now open with a `GENERATED by ggen` header; **do not
hand-edit them** — a correction goes through `ontology.ttl`'s `bap:engine_rust4pm` fact graph or
the pack's `templates/beam4pm_engine.*.tmpl` family, per this repo's source-authority doctrine.
The technical content in §1–§8 below (the wasm ABI, the memory-ownership contract, the
differential-testing story) still accurately describes the real mechanism the generated facade
implements — that protocol didn't change, only which files carry it and how they're produced.
Read §9 before assuming anything below generalizes to `petgraph` or `tract`'s own history (they
converted first, in an earlier commit).
...
example ports with their pinned semantics, a Standing section separating what is proven by a
real run from what is design-specified, and (§9) how this hand-authored engine relates to its
three sibling native engines. It is for anyone calling `BeamPM.Rust4PM`, extending the op set,
or auditing why a number the engine returns can be trusted.
...
| Sibling native engines (§9) | `native/{ferroplan,petgraph-wasm,tract-wasm}/` |
...
## 9. Sibling Native Engines: All Four Are Now Manufactured

`native/` holds **four** native-engine crates: `rust4pm-wasm` (this document), `ferroplan`,
`petgraph-wasm`, and `tract-wasm`. Earlier this session, petgraph/tract converted first, then
rust4pm/ferroplan converted the same way in a later commit. §1's "One Engine, Three Facades"
story is still true as an **architecture** statement (Elixir/Erlang/Gleam remain thin facades
over one wasm engine, no algorithm reimplemented in BEAM) but is no longer true as a
**provenance** statement — none of the four engines' BEAM facades are hand-typed anymore:

| Engine | BEAM facade files | Provenance |
| --- | --- | --- |
| `rust4pm` (this doc) | `lib/beam4pm_rust4pm.ex`, `src/beam4pm_rust4pm.erl`, `gleam/src/beam4pm/rust4pm.gleam` | **Manufactured.** Every file's first line reads `GENERATED by ggen from the admitted bpm:Engine graph (rust4pm). Do not edit.` |
| `ferroplan` | `lib/beam4pm_ferroplan.ex`, `src/beam4pm_ferroplan.erl`, `gleam/src/beam4pm/ferroplan.gleam` | **Manufactured.** Same header, `(ferroplan)`. |
| `petgraph` | `lib/beam4pm_petgraph.ex`, `src/beam4pm_petgraph.erl`, `gleam/src/beam4pm/petgraph.gleam` | **Manufactured.** Same header, `(petgraph)`. Converted first. |
| `tract` | `lib/beam4pm_tract.ex`, `src/beam4pm_tract.erl`, `gleam/src/beam4pm/tract.gleam` | **Manufactured.** Same header, `(tract)`. Converted first. |

Evidence, checked directly against this repo (not inferred from a changelog):

- `ontology.ttl` now admits **four** `bpm:Engine` individuals — `bap:engine_petgraph`,
  `bap:engine_tract`, `bap:engine_rust4pm`, `bap:engine_ferroplan` — each with a full
  `bpm:EngineOp`/`bpm:EngineArg` fact graph (op name, order, arg types, timeout class).
  rust4pm's graph covers 29 ops (matching its crate's 29 dispatch arms plus context below);
  ferroplan's covers 30.
- All four facades are rendered by the vendored `beam4pm-process-model-pack`'s
  `templates/beam4pm_engine.*.tmpl` family from those facts — the same manufacturing engine
  that produces the record-type projections documented in `CLAUDE.md`, applied to a second
  fact shape (`bpm:Engine`/`bpm:EngineOp`/`bpm:EngineArg` instead of `bpm:RecordType`/`bpm:Field`).
  No pack template or vocabulary change was needed for the rust4pm/ferroplan conversion — every
  `bpm:EngineArgType`/`bpm:ArgEncoding`/`bpm:ArgMode` member the two new engines needed already
  existed from the petgraph/tract pass.
- `scripts/gate_engine_dispatch_check.sh` is the dispatch-completeness gate for this
  manufacturing path: it reads the manufactured manifest `schema/beam4pm_engine_ops.tsv` (one
  row per op — engine, op, wire_name, kind, dispatch_source) and refuses
  `REFUSED_OP_NOT_DISPATCHED` if a fact names a wire op the crate's own dispatch match has no
  arm for. Real current standing: `73 wire ops dispatched across 4 engines, 0 crate arms
  unexposed` (rust4pm 29/29, ferroplan 30/30, petgraph 10/10, tract 4/4) — every op the four
  crates' own dispatch code handles is now reachable from all three BEAM languages.
  `test/beam4pm_engine_dispatch_gate_test.exs` runs the same check from Mix.
- `docs/reference/beam4pm_hand_authored_source.md`'s GATE AUTHORSHIP table no longer admits
  any `native_engine_facade`-kind file — the six rust4pm/ferroplan facade admissions were
  removed in the same commit that deleted the hand-authored files, the same way the six
  petgraph/tract admissions were removed earlier. The `native_engine_facade` debt ceiling
  stays at 6 in `ontology.ttl` as a historical marker; the real admitted count for that kind
  is 0. (Each engine's wasm-build-dependent *test* files, e.g. `test/beam4pm_rust4pm_test.exs`,
  remain a separate, unrelated hand-authored-input admission from the facade code itself.)

**Implication for this document.** Everything in §1–§8 above — the ABI, the memory-ownership
contract, the facade matrix, the differential-testing story — describes the real wasm
ABI/protocol rust4pm's generated facade still implements; that mechanism didn't change. What
changed is authorship: a correction to any of the four engines' facade behavior now goes through
`ontology.ttl` or the pack's engine templates, never a direct edit to the generated `.ex`/`.erl`/
`.gleam` files — the same source-authority doctrine `CLAUDE.md` states for the record-type
projections. Any sentence elsewhere in §1–§8 that says or implies "hand-authored, no ggen
provenance header" about the rust4pm facade files specifically (as opposed to this document, which
remains hand-authored prose) is describing this document's own state before this update, not the
current repo.
...
- `docs/reference/beam4pm_hand_authored_source.md` — the full GATE AUTHORSHIP admission
  ledger referenced in §9 (no `native_engine_facade` entries remain, for any of the four engines)
- `docs/reference/scripts.md` — `gate_engine_dispatch_check.sh` and every other gate script,
  including the ones that qualify §9's now-fully-manufactured engine facade path
