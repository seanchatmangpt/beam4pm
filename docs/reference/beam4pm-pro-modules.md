...
process-governance, and billing/entitlement reconciliation modules. **Every module below
carries a `GENERATED by ggen` / `GENERATED by ggen_igniter` header and must never be
hand-edited** — each is manufactured from a static EEx/Tera template (`ggen_igniter`'s
"fully-static template" pattern, the same mechanism `beam4pm-pro-revenue-suite.md` documents
for the revenue suite) or from an admitted ontology graph (`BeamPM.Actuation` from
`bpma:AdmittedActuation`, `BeamPM.ProcessGovernor` from `bpmg:ProcessContract`, the two Erlang
records from the admitted `bpm:RecordType` graph). If a module's documented behavior is wrong,
fix the template under
`vendor/ggen-marketplace/packs/beam4pm-process-model-pack/igniter/templates/` (or the relevant
ontology facts) and re-run that family's sync script — never hand-edit the file this doc cites.
Per `CLAUDE.md`'s source-authority doctrine, provenance is signalled per-file by the header
comment, never by directory placement; this doc's line citations below point at each module's
`@spec`/`-spec` line (verified against the current tree) so a reader lands on the contract, not
an arbitrary offset.
...
`lib/beam4pm_actuation.ex:205-267` (the `BeamPM.Actuation` moduledoc; `GymBridge` starts at
line 2, `Session` at line 114)
...
The graph-derived allowlist (`admitted_actuations/0`) currently admits four actuations:
`increment_counter` → gym op `inc` (requires `"counter_ready"`), `k8s_scale_down` → gym op
`scale_down`, `k8s_scale_up` → gym op `scale_up`, `observe_counter` → gym op `noop` (the latter
three require no precondition facts). `k8s_scale_down`/`k8s_scale_up` are admitted for the
`k8s-deployment-scaler` bridge, not the toy-counter bridge used in the example below — an
admitted-but-wrong-bridge action fails through `{:error, {:execution_failed, ...}}`, the same
path any bridge protocol error takes, never a silent no-op.
...
`lib/beam4pm_process_governor.ex:1-117`
...
two graph-declared contracts: `"k8s_scaling_governed"` (`"reset" -> "scaled_up" -> "scaled_down"`
via `k8s_scale_up`/`k8s_scale_down`) and `"toy_counter_governed"` (`"reset" ->
"incremented_once" -> "observed"` via `increment_counter`/`observe_counter`).
...
`lib/beam4pm_receipt_chain.ex:1-160`

Additive, opt-in hash-chaining extension for `beam4pm-brce/v1` receipts (adapted from ex4pm's
`Replay.Chain`), file-based, hashing raw on-disk bytes. Sibling-regenerated with
`BeamPM.Actuation` from the `beam4pm-process-model-pack` igniter recipe (pack v0.1.9 landed the
O(1) tip index below; no ontology.ttl change was required).
...
Measured cost of the index vs. the retained scan oracle (per governed receipt write, min of 5,
`scripts/receipt_chain_sync.sh`'s regeneration commit message): 1.8ms → 0.57ms at N=33 receipts,
56ms → 0.78ms at N=1,000, 0.64s → 0.94ms at N=10,000, 2.2s → 0.96ms at N=30,000 — 3x to 2,300x
cheaper as the directory grows, because `ProcessGovernor`'s `continuous: true` mode never prunes
`receipts_dir`.
...
| Function | Signature | Line |
|---|---|---|
| `new_entitlement_event/1` | `(Map :: map()) -> {ok, entitlement_event()} \| {error, {missing_field, atom()}}` | :108 |
| `new_entitlement_state/1` | `(Map :: map()) -> {ok, entitlement_state()} \| {error, {missing_field, atom()}}` | :153 |
| `initial_entitlement_state/1` | `(EntitlementId :: binary()) -> entitlement_state()` | :215 |
...
- `docs/reference/beam4pm-pro-revenue-suite.md` — the sibling `ggen_igniter` static-template
  manufacturing pattern applied to the revenue-suite modules
