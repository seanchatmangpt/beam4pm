...
Updated 2026-09-05 (verified all three families' generated tests pass for real;
economics activity vocabulary externalized to a per-call runtime argument,
`beam4pm-process-model-pack` v0.1.7), against the v26.8.29 doc package. Overall
standing: ALIVE for the three families' own generated test suites (27/27 real
`mix test` passes, re-verified this update — see the Honesty Ledger section for
the full standing vocabulary and everything this does NOT cover).*
...
- **The three generated test suites are ALIVE, re-verified this update.**
  All three families' rendered files carry the `GENERATED by ggen_igniter`
  header (proof the sync scripts actually ran, not a hand-authored bypass),
  and their generated tests pass for real right now:
  `mix test test/beam4pm_revenue_economics_test.exs
  test/beam4pm_revenue_metering_test.exs
  test/beam4pm_claude_workflow_reactor_test.exs` → `27 tests, 0 failures`
  (11 economics + 9 metering + 7 practice). This covers exactly what those
  three files assert — the fixture-pinned capabilities above, the
  multi-tenant vocabulary case, and the journal-driven reactor DAG. It is
  not a claim about anything outside those files: no rating layer, no
  provider transport, no causal-outcome claim, and no alignment-based
  conformance exist regardless of this test result, per the bullets above.
...
- [BeamPM.Pro Modules Reference](beam4pm-pro-modules.md) — the sibling
  `ggen_igniter` static-template manufacturing pattern applied to the
  commercial-layer modules
