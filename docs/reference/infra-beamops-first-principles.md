...
`packer-beam4pm-image.pkr.hcl.tmpl` and `packer-variables.pkr.hcl.tmpl` (the templates
behind this section) consume `image[0]` -- the first `b4pi:PackerImage` individual after
`ORDER BY(?imageName)` -- not a loop over every admitted individual. See §7 below for the
gate that makes this a checked, disclosed contract rather than a silent limitation.
...
`cloudrun_main_tf.tmpl`, `cloudrun_variables_tf.tmpl`, and `cloudrun_outputs_tf.tmpl` (the
templates behind `infra/gcp/cloudrun/*.tf`) consume `deployments[0]` -- the first
`b4pi:Deployment` individual after `ORDER BY(?serviceName)` -- the same single-individual
contract as §1's Packer templates. See §7 below.
...
## 7. Single-Deployment/single-PackerImage cardinality: gated

Both §1's Packer templates and §2's Cloud Run templates render only **one** individual of
their respective class: `cloudrun_main_tf.tmpl`/`cloudrun_variables_tf.tmpl`/
`cloudrun_outputs_tf.tmpl` all read `deployments[0]` (the row after `ORDER BY(?serviceName)`),
and `packer-beam4pm-image.pkr.hcl.tmpl`/`packer-variables.pkr.hcl.tmpl` read `image[0]`
(`ORDER BY(?imageName)`). This is `beam4pm-pro-infra-pack`'s documented
single-Deployment/single-PackerImage-per-consumer-graph contract, not an oversight -- but
until `gates/030_cardinality.rq` was added, nothing in the pack could see a violation of it:
the pack's pre-existing required-facts gates (`gates/010_required.rq`,
`gates/020_packerimage_required.rq`) are each a per-property presence check scoped to a
single `?s a b4pi:Deployment` / `?s a b4pi:PackerImage`, so a consumer graph asserting a
*second* individual of either class gets zero rows from both -- reading as "nothing
missing" -- while the `[0]`-indexed templates render only the first, ORDER-BY-selected
individual and silently drop every fact belonging to the rest.

`gates/030_cardinality.rq` closes that gap: it counts distinct individuals per class and
refuses (`HAVING (COUNT(DISTINCT ?s) > 1)`) whenever either count exceeds one, turning a
silent truncation into a named refusal. beam4pm's own `ontology.ttl` currently admits
exactly one `b4pi:Deployment` (`b4p:beam4pm_pro_cloudrun`) and one `b4pi:PackerImage`
individual, so this gate passes with zero findings today; it exists to catch drift if a
second individual of either class is ever added, not to report an active problem in this
repo's own graph right now.
...
| Single-Deployment/single-PackerImage cardinality | No (mechanism) | Gated, new (`gates/030_cardinality.rq`, not a book pattern) -- see §7 |
...
- **Also covered by GATE M2's determinism proof.** `scripts/gate_m2_check.sh`'s
  `SEARCH_DIRS` includes `infra/gcp/cloudrun` and `infra/gcp/packer` (added alongside
  `.github/workflows`, closing a real prior gap where manufactured files under either
  directory were invisible to the gate): every `GENERATED by ggen`-marked file there is
  deleted and regenerated from `(ontology.ttl, pack templates)` alone as part of the same
  destructive delete-then-regenerate proof the rest of the manufactured tree gets. This is
  evidence of manufacturing determinism (the files are a pure function of the ontology and
  pack templates, not of anything else on disk), distinct from and additional to the
  Terraform/Packer *syntactic* validation in the bullet above.
- **Single-Deployment/single-PackerImage cardinality is gated, not just documented.** See
  §7: `gates/030_cardinality.rq` in `beam4pm-pro-infra-pack` refuses a consumer graph
  asserting more than one `b4pi:Deployment` or `b4pi:PackerImage` individual, since both
  the Cloud Run and Packer templates render only the first (`ORDER BY`-selected) individual
  and would otherwise silently drop a second one's facts. beam4pm's own graph asserts
  exactly one of each today, so this gate currently passes cleanly.
...
- `vendor/ggen-marketplace/packs/beam4pm-pro-infra-pack/gates/030_cardinality.rq` -- the cardinality gate described in §7
